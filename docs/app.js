...
function init(){if(typeof firebase==='undefined'){throw new Error('Firebase SDK no cargado')};if(!firebase.apps.length)firebase.initializeApp(firebaseConfig);db=firebase.firestore()}
...
async function login(){
  try{
    ...
    await refresh();
  }catch(e){console.log(e);q("msg").innerText="Error Firebase / configuración. Verificá conexión y permisos."}
}
...
/* ===== V3.3.21 - Nombre oficial del sistema ===== */
(function(){
  const APP_VERSION_V2='3.3.21';
...
/* ===== V3.3.21 - Version y menu lateral robustos ===== */
(function(){
...
/* ===== V3.3.21 - Anti-cache y normalizacion final menu/version ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION = VERSION;
...
/* ===== V3.3.21 - Correcciones GPS, clima, alertas y seguimiento ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Ajustes finales clima, KM y alertas ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Ajuste final: menu, clima compacto, KM y alertas alineadas ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Correccion funcional final: menu, clima, KM y alertas compactas ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Vista Alertas compacta: boton Verificar visible ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Torre de Control: Últimas alertas formato linea unica ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Tránsitos: alertas a la derecha y todas las alertas / Seguimiento vuelve a base 3.3.21 ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Iconos por tipo de alerta en Tránsitos y Seguimiento ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: diseño aprobado Flotas -> Pasos -> Destinos ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: formato operativo compacto Flotas -> Pasos -> Destinos ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: filtro por embarque y orden Flotas -> Pasos -> Destinos ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: filtro embarques activos + layout compacto corregido ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: tarjetas compactas + datos operativos + paso real oficial ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: actualizar funcional, tarjetas compactas y Paso oficial ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: columnas Flotas | Paso+Destinos, tarjetas compactas ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - SOLO vista Clima: reorganizacion final compacta + paso oficial bajo demanda ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Clima: layout compacto definitivo y Paso con consulta online ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Vista Alertas: resumen con graficos + tabla compacta funcional ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Alertas: graficos en una linea, campana completa, contador funcional ===== */
(function(){
  const VERSION_ALERTS_2035 = "3.3.21";
  const $ = (id)=>document.getElementById(id);
...
/* ===== V3.3.21 - Menu lateral: badge y efecto de campana segun pendientes ===== */
(function(){
...
/* ===== V3.3.21 - Normalizacion final del menu lateral ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Refuerzo campana menu lateral ===== */
(function(){
...
/* ===== V3.3.21 - contador real y badge compacto de menu ===== */
(function(){
  const APP_VERSION_MENU_2173 = '3.3.21';
...
/* ===== V3.3.21 - Menu base limpio: normalizadores anteriores removidos ===== */
/* ===== V3.3.21 - Nuevo modulo Embarques ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
    ...
    if(tbody){
      tbody.innerHTML=rows.map(e=>{
        ...
        return `<tr>
          ...
          <td>${c}</td>
          <td class="embarqueActionsCellV208"><div class="embarqueActionsV208"><button class="iconBtn view" onclick="window.openEmbarqueDetailV244('${esc2(e.numero)}')">👁️</button><button class="iconBtn edit" onclick="window.openCargoFormV244('${esc2(e.numero)}')">+ Carga</button><button type="button" class="deleteEmbarqueBtnV275" title="Eliminar embarque" onclick="event.stopPropagation();window.confirmDeleteEmbarqueV275&&window.confirmDeleteEmbarqueV275('${esc2(e.numero)}')">🗑</button></div></td>
        </tr>`;
...
/* ===== V3.3.21 - Embarques: combos desde colecciones Firestore y estructura real ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
  ...
  window.renderEmbarquesV244=async function(){
    ...
    if(tbody){
      tbody.innerHTML=rows.map(e=>{
        ...
        return `<tr><td><b>Emb. ${E(e.numero)}</b></td><td>${E(S(e.cliente))}</td><td>${E(S(e.origen))}</td><td>${E(S(e.destino))}</td><td>${E(fmt(e.fecha))}</td><td>${E([...e.flotas].join(', ')||'-')}</td><td>${c}</td><td class="embarqueActionsCellV208"><div class="embarqueActionsV208"><button class="iconBtn view" onclick="window.openEmbarqueDetailV244('${E(e.numero)}')">👁️</button><button class="iconBtn edit" onclick="window.openCargoFormV244('${E(e.numero)}')">+ Carga</button><button type="button" class="deleteEmbarqueBtnV275" title="Eliminar embarque" onclick="event.stopPropagation();window.confirmDeleteEmbarqueV275&&window.confirmDeleteEmbarqueV275('${E(e.numero)}')">🗑</button></div></td></tr>`;
      }).join('') || '<tr><td colspan="8" class="emptyRow">No hay embarques para mostrar.</td></tr>';
    }
  };
...
/* ===== V3.3.21 - Librerias externas: PDF y mapa ===== */
(function(){
  const VERSION='3.3.21';
  ...
  window.ensurePdfEngineV314=async function(){
    ...
    try{await loadScriptV314('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js');}catch(e){console.warn('jsPDF CDN no disponible',e);}
    return !!(window.jspdf&&window.jspdf.jsPDF);
  };
  window.ensureLeafletV314=async function(){
    if(window.L)return true;
    try{loadCssV314('https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'); await loadScriptV314('https://unpkg.com/leaflet@1.9.4/dist/leaflet.js');}catch(e){console.warn('Leaflet CDN no disponible',e);}
    return !!window.L;
  };
  const retryMap=async()=>{try{if(typeof window.initSeguimientoMap==='function'){await window.ensureLeafletV314(); if(window.trs&&typeof window.renderMapa==='function')window.renderMapa();}}catch(e){console.warn('retry map',e);}};
  window.addEventListener('load',()=>setTimeout(retryMap,800));
})();

/* ===== v3.3.21 - PDF profesional con logo original ELTA para Vista Tránsitos ===== */
(function(){
  const APP_VERSION_PDF = '3.3.21';
  ...
  window.downloadTransitPdf = async function(embarque,fleet){
    ...
    if(window.ensurePdfEngineV314) await window.ensurePdfEngineV314();
  ...
  document.querySelectorAll('.loginFooter span, .headerTitle span').forEach(el=>{if(/Versi[oó]n/i.test(el.textContent||''))el.textContent='Versión 3.3.21';});
})();


/* ===== V3.3.21 - Fix botones Actualizar global + version unificada ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Configuracion redisenada sin JSON y sin pestana Embarques ===== */
(function(){
  const APP_VERSION_CFG = "3.3.21";
...
/* ===== V3.3.21 - SOLO vista Clima: filtro embarque corrige destino exacto del embarque ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - FIX DEFINITIVO SOLO CLIMA: filtro por embarque trae destino/paso aunque no haya flota ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Datos reales del usuario logueado en sidebar ===== */
(function(){
  const APP_VERSION_LOGGED_USER_FIX = "3.3.21";
...
/* ===== V3.3.21 - Clima compacto + PDF OEA + correcciones puntuales ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  function $id(id){return document.getElementById(id);}
  ...
  window.downloadOeaPdf=async function(embarque,flotaNo){
    ...
    if(window.ensurePdfEngineV314) await window.ensurePdfEngineV314(); const jsPDF=window.jspdf?.jsPDF; if(!jsPDF){alert('No se pudo cargar jsPDF.');return;}
...
/* ===== V3.3.21 - Version + flotas del combo desde usuarios role=flota ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - FIX DEFINITIVO: version, clima compacto y combo flotas desde Firebase ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Embarques: boton Ver cargas en Agregar carga ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - Recuperar password, ajustes Configuracion, Embarques y KM Alertas ===== */
(function(){
  const APP_VERSION_FIX='3.3.21';
...
/* ===== V3.3.21 - Password visible en Configuracion y version unificada ===== */
(function(){
  const APP_VERSION_273='3.3.21';
  ...
  // setInterval(setVersionLabelsV273,1000); // desactivado v3.3.21
})();


/* ===== V3.3.21 - Version lock definitivo ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION = VERSION;
  window.APP_VERSION_V2 = VERSION;
  ...
  // setInterval(applyVersionLock, 800); // desactivado v3.3.21
})();


/* ===== V3.3.21 - Embarques: eliminar registro con confirmacion ===== */
(function(){
  const VERSION='3.3.21';
  const S=v=>String(v==null?'':v).trim();
  const E=v=>S(v).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  ...
  function enhanceDeleteButtons(){ /* v3.3.21 neutralizado: eliminar ya nace en el render */ return; }
...
/* ===== V3.3.21 - Embarques: papelera sin fondo ===== */
(function(){
  const css=document.createElement('style');
...
/* ===== V3.3.21 - MENU LATERAL ESTABLE: sin intervalos ni reescrituras repetidas ===== */
(function(){
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V3.3.21 - ENTREGAS: ROUTING UNICO SIN REFRESH ===== */
(function(){
  'use strict';
  const VERSION='3.3.21';
  window.ELTA_APP_VERSION=VERSION;
  ...
  function stages(row){let mid=[];const d=countryOf(row.destino); if(d==='chile')mid=['Uspallata','Los Andes']; else if(d==='paraguay')mid=['Clorinda','Puerto José Falcón','Depósito Fiscal Paraguay']; else if(d==='uruguay')mid=['Gualeguaychú','Fray Bentos']; const out=[S(row.origen)||'Origen',...mid,S(row.destino)||'Lugar de entrega'].filter(Boolean).slice(0,6); return window.filtrarHitosEntrega187?window.filtrarHitosEntrega187(out):out;}
...
/* ===== v3.3.21 - Entregas: abreviatura en origen del render ===== */
...
/* ===== v3.3.21 - CSS final linea desde circulo Entregas ===== */
(function(){
  const CSS = '\n/* ===== v3.3.21 - Entregas: linea vertical nace desde el circulo =====\n   SOLO afecta la vista #entrega. No modifica icono de camion ni otros modulos. */\n\n#entrega .entregaStep.hasDate::before{\n  content:"" !important;\n  position:absolute !important;\n  left:50% !important;\n  top:32px !important;\n  height:33px !important;\n  border-left:2px dotted rgba(226,232,240,.72) !important;\n  transform:translateX(-50%) !important;\n  z-index:2 !important;\n}\n\n#entrega .entregaStep::after{\n  top:65px !important;\n  z-index:4 !important;\n}\n\n#entrega .entregaStep.hasDate[style*="left:0%"]::before{\n  left:6px !important;\n  transform:none !important;\n}\n\n#entrega .entregaStep.hasDate[style*="left:100%"]::before{\n  left:auto !important;\n  right:6px !important;\n  transform:none !important;\n}\n\n#entrega .entregaStep:not(.hasDate)::before{\n  content:none !important;\n  display:none !important;\n}\n';
...
/* ===== v3.3.21 - Entregas: avance por registros + bandera Tierra del Fuego =====
...
`;
  ...
  document.addEventListener('DOMContentLoaded',()=>{injectTdfCss();});
  window.addEventListener('load',()=>{injectTdfCss();});
  setTimeout(()=>{injectTdfCss();},300);
})();




/* ===== v3.3.21 - Entregas: timeline por registros reales corregido =====
   Solo afecta la vista Entregas. No modifica diseño ni otros módulos. */
(function(){
  const CSS = `
/* ===== v3.3.21 - Entregas: extremos y registros reales =====
...
`;
  ...
  function attach(){ /* v3.3.21: wrapper v162 desactivado para evitar segundo render de Entregas */ return; }
...
/* ===== v3.3.21 - Entregas: hitos desde Aduanas + camión por último registro ===== */
(function(){
const CSS=`
/* v3.3.21 - Entregas: aduanas y avance real */
...
function attach(){/* v3.3.21: wrapper v163 desactivado para evitar segundo render de Entregas */return}
...
/* ===== v3.3.21 - Entregas: abreviatura visual de hitos =====
...
/* ===== v3.3.21 - Entregas: hitos desde colección aduana por campo destino =====
...
(function(){
  ...
  function aplicarTimelineAduana167(){
    ...
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      try{
        ...
        const namesFiltrados187 = window.filtrarHitosEntrega187?window.filtrarHitosEntrega187(names):names;
        const stages = namesFiltrados187.map((name,i)=>({
          name,
  ...
  async function run167(){ /* v3.3.21 desactivado: causaba segundo render heredado de Entregas */ return Promise.resolve(); }
function attach167(){ /* v3.3.21 desactivado */ return; }
document.addEventListener('DOMContentLoaded',()=>{});
...
/* ===== v3.3.21 - Entregas: botón Actualizar refresca timeline =====
...
(function(){
  ...
  if(typeof oldRenderEntrega168 === 'function' && !oldRenderEntrega168.__wireRefresh168_disabled223){
    window.renderEntrega = async function(){
      const r = await oldRenderEntrega168.apply(this, arguments);
      wireEntregaRefresh168();
      return r;
    };
    window.renderEntrega.__wireRefresh168_disabled223 = true;
  }
...
/* ===== v3.3.21 - Entregas: Actualizar reconstruye la línea como al reingresar =====
   Solo afecta el botón Actualizar de Entregas. No modifica estética ni otros módulos. */
(function(){
  ...
  if(typeof oldRenderEntrega169 === 'function' && !oldRenderEntrega169.__wireRefresh169_disabled223){
    window.renderEntrega = async function(){
      const r = await oldRenderEntrega169.apply(this, arguments);
      wire169();
      return r;
    };
    window.renderEntrega.__wireRefresh169_disabled223 = true;
  }

  const oldTab169 = window.tab;
  if(typeof oldTab169 === 'function' && !oldTab169.__wireRefresh169_disabled223){
    window.tab = function(id){
      const r = oldTab169.apply(this, arguments);
      if(id === 'entrega') setTimeout(wire169, 150);
      return r;
    };
    window.tab.__wireRefresh169_disabled223 = true;
  }
...
/* ===== v3.3.21 - Entregas: Actualizar sin parpadeo =====
...
/* ===== v3.3.21 - Entregas: Actualizar in-place sin reconstruir tarjeta =====
   Solo afecta el botón Actualizar de Entregas.
...
(function(){
  ...
  function updateCard(card){
    ...
    const namesFiltrados187=window.filtrarHitosEntrega187?window.filtrarHitosEntrega187(names):names;
    const stages=namesFiltrados187.map((name,i)=>({name,time:i===0?inicioTime:(i===names.length-1?cierreTime:''),kind:i===0?'start':(i===names.length-1?'end':'aduana')}));
    collectRecords(t).forEach(r=>{
...
/* ===== v3.3.21 - Entregas: Actualizar aislado, sin render viejo =====
   Solo afecta el botón Actualizar de Entregas.
...
(function(){
  ...
  const oldRender=window.renderEntrega;
  if(typeof oldRender==='function' && !oldRender.__wireOnly172_disabled223){
    window.renderEntrega=async function(){
      const r=await oldRender.apply(this, arguments);
      wire();
      return r;
    };
    window.renderEntrega.__wireOnly172_disabled223=true;
  }
...
/* ===== v3.3.21 - Version final + Entregas boton aislado dominante ===== */
(function(){
  const VERSION_FINAL_173 = '3.3.21';
  ...
  const oldRender = window.renderEntrega;
  if(typeof oldRender === 'function' && !oldRender.__v173_disabled223){
    window.renderEntrega = async function(){
      const r = await oldRender.apply(this, arguments);
      wire173();
      return r;
    };
    window.renderEntrega.__v173_disabled223 = true;
  }
})();
...
(function(){
  ...
  if(typeof oldRender174 === 'function' && !oldRender174.__wire174_disabled223){
    window.renderEntrega = async function(){
      const r = await oldRender174.apply(this, arguments);
      wire174();
      return r;
    };
    window.renderEntrega.__wire174_disabled223 = true;
  }
...
/* ===== v3.3.21 - Reporte PDF: hitos desde colección aduana =====
...
/* ===== v3.3.21 - Forzado final de versión visible y PDF ===== */
(function(){
  const VERSION_FINAL_177 = '3.3.21';
...
/* ===== v3.3.21 - Alertas: sin KM heredado, sin observers =====
   Base v3.3.21. Solo afecta Alertas. No toca Tránsitos, Entregas, PDF ni estética. */
(function(){
  const VERSION_ALERTAS_183='3.3.21';
...
/* ===== v3.3.21 - Alertas: columnas alineadas sin KM =====
   Solo ajusta la tabla de detalle de Alertas. No toca otros modulos. */
(function(){
  const VERSION_ALERTAS_185='3.3.21';
...
/* ===== v3.3.21 - Entregas: evitar flash de hito inicio largo =====
   Solo afecta el texto visual del hito de inicio CLZ/CLC. */
(function(){
  const VERSION_ENTREGA_186='3.3.21';
  function apply186(){
    document.querySelectorAll('#entrega .entregaStep b').forEach(el=>{
...
/* ===== v3.3.21 - Entregas: quitar país como hito intermedio =====
   Solo afecta la línea de tiempo de Entregas. No toca Alertas, Tránsitos, PDF ni Firebase. */
(function(){
  const VERSION_ENTREGA_187='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_187;
  window.APP_VERSION_V2=VERSION_ENTREGA_187;

  window.filtrarHitosEntrega187 = function(sts){
    try{
      const arr = Array.isArray(sts) ? sts.slice() : [];
      if(arr.length <= 2) return arr;
      const isPais = (v)=>/^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(
        String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
      );
      return arr.filter((h,i)=> i===0 || i===arr.length-1 || !isPais(h));
    }catch(e){ return sts; }
  };

  function isPais187(txt){
    return /^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(
      String(txt||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    );
  }

  function limpiarPaisIntermedio187(){
    document.querySelectorAll('#entrega .entregaTimeline').forEach(tl=>{
      ...
      steps.forEach((step,i)=>{
        if(i>0 && i<steps.length-1){
          ...
          const txt=b?.getAttribute('data-full-hito') || b?.getAttribute('title') || b?.textContent || '';
          if(isPais187(txt)) step.remove();
        }
      });
    });
  }

  const oldRenderEntrega187=window.renderEntrega;
  if(typeof oldRenderEntrega187==='function' && !oldRenderEntrega187.__sinPais187){
    window.renderEntrega=async function(){
      const r=await oldRenderEntrega187.apply(this,arguments);
      limpiarPaisIntermedio187();
      requestAnimationFrame(limpiarPaisIntermedio187);
      return r;
    };
    window.renderEntrega.__sinPais187=true;
  }

  const oldTab187=window.tab;
  if(typeof oldTab187==='function' && !oldTab187.__sinPais187){
    window.tab=function(id){
      const r=oldTab187.apply(this,arguments);
      if(id==='entrega'){ limpiarPaisIntermedio187(); requestAnimationFrame(limpiarPaisIntermedio187); }
      return r;
    };
    window.tab.__sinPais187=true;
  }

  document.addEventListener('DOMContentLoaded',limpiarPaisIntermedio187);
  window.addEventListener('load',limpiarPaisIntermedio187);
})();




/* ===== v3.3.21 - Entregas: render único sin postproceso heredado =====
   Solo afecta Entregas. No toca Alertas, Tránsitos, PDF, Firebase ni estética. */
(function(){
  const VERSION_ENTREGA_188='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_188;
  window.APP_VERSION_V2=VERSION_ENTREGA_188;

  function isCountry188(v){
    return /^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(
      String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    );
  }

  function limpiarPaisIntermedio188(){
    document.querySelectorAll('#entrega .entregaTimeline').forEach(tl=>{
      const steps=[...tl.querySelectorAll('.entregaStep')];
      if(steps.length<=2) return;
      steps.forEach((step,i)=>{
        if(i>0 && i<steps.length-1){
          const b=step.querySelector('b');
          const txt=b?.getAttribute('data-full-hito') || b?.getAttribute('title') || b?.textContent || '';
          if(isCountry188(txt)) step.remove();
        }
      });
    });
  }

  const oldRenderEntrega188=window.renderEntrega;
  if(typeof oldRenderEntrega188==='function' && !oldRenderEntrega188.__sinLegacy188){
    window.renderEntrega=async function(){
      const r=await oldRenderEntrega188.apply(this,arguments);
      limpiarPaisIntermedio188();
      return r;
    };
    window.renderEntrega.__sinLegacy188=true;
  }

  const oldTab188=window.tab;
  if(typeof oldTab188==='function' && !oldTab188.__sinLegacy188){
    window.tab=function(id){
      const r=oldTab188.apply(this,arguments);
      if(id==='entrega') limpiarPaisIntermedio188();
      return r;
    };
    window.tab.__sinLegacy188=true;
  }
})();
...
/* ===== v3.3.21 - Entregas: render único, sin wrappers heredados =====
   Solo afecta Entregas. No toca Alertas, Tránsitos, PDF, Firebase ni estética. */
(function(){
  const VERSION_ENTREGA_189='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_189;
  window.APP_VERSION_V2=VERSION_ENTREGA_189;

  // No usar más los post-procesadores que reescriben la línea de tiempo.
  if(window.renderEntrega && window.renderEntrega.__v162) window.renderEntrega.__v162=false;
  if(window.renderEntrega && window.renderEntrega.__v163) window.renderEntrega.__v163=false;

  function isCountry189(v){
    return /^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(
      String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    );
  }
    ...
    document.querySelectorAll('#entrega .entregaTimeline').forEach(tl=>{
      ...
      steps.forEach((step,i)=>{
        if(i>0 && i<steps.length-1){
          const b=step.querySelector('b');
          const txt=b?.getAttribute('data-full-hito') || b?.getAttribute('title') || b?.textContent || '';
          if(isCountry189(txt)) step.remove();
        }
      });
    });
  }

  const oldRender189=window.renderEntrega;
  if(typeof oldRender189==='function' && !oldRender189.__renderUnico189){
    window.renderEntrega=async function(){
      const r=await oldRender189.apply(this,arguments);
      limpiar189();
      return r;
    };
    window.renderEntrega.__renderUnico189=true;
  }
})();
...
/* ===== v3.3.21 - Entregas: render definitivo unico =====
...
(function(){
  const VERSION_ENTREGAS_190 = '3.3.21';
  window.ELTA_APP_VERSION = VERSION_ENTREGAS_190;
  window.APP_VERSION_V2 = VERSION_ENTREGAS_190;

  const esc190 = v => String(v ?? '').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const clean190 = v => {
    if(v === undefined || v === null) return '';
    if(typeof v === 'object'){
      if(v && typeof v.toDate === 'function') return v;
      for(const k of ['nombre','name','label','title','descripcion','description','aduana','aduanaId','destino','origen','pais','country','provincia','tipo','estado','level','localidad','ciudad']){
        const c = clean190(v[k]);
        if(c) return c;
      }
      return '';
    }
  ...
  const norm190 = v => clean190(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
  const get190 = (o, keys) => {
    for(const k of keys){
      const v = o && o[k];
      const c = clean190(v);
  ...
  function elapsed190(start){
    const d = toDate190(start);
    if(!d) return '-';
    let ms = Date.now() - d.getTime();
    if(ms < 0) ms = 0;
    const m = Math.floor(ms/60000), days = Math.floor(m/1440), h = Math.floor((m%1440)/60), mm = m%60;
  ...
  function country190(v){
    const s = norm190(v);
    if(/(tierra del fuego|ushuaia|rio grande|tolhuin|antartida|islas del atlantico sur)/.test(s)) return 'tdf';
    if(/(chile|stli|toyota chile|los andes|libertadores|\bcl\b)/.test(s)) return 'chile';
    if(/(paraguay|asuncion|puerto jose falcon|falcon|garden paraguay|\bpy\b)/.test(s)) return 'paraguay';
    if(/(uruguay|montevideo|fray bentos|sevel uruguay|\buy\b)/.test(s)) return 'uruguay';
    if(/(brasil|brazil|curitiba|sao paulo|\bbr\b)/.test(s)) return 'brasil';
    if(/(argentina|zarate|zárate|cordoba|córdoba|clz|clc|\bar\b|arg)/.test(s)) return 'argentina';
    return '';
  }
  function code190(v){
    const c = country190(v);
    return c === 'argentina' ? 'AR' : c === 'chile' ? 'CL' : c === 'paraguay' ? 'PY' : c === 'uruguay' ? 'UY' : c === 'brasil' ? 'BR' : c === 'tdf' ? 'TDF' : '--';
  }
  function flagCls190(v){
    const c = country190(v);
    return c === 'argentina' ? 'ar' : c === 'chile' ? 'cl' : c === 'paraguay' ? 'py' : c === 'uruguay' ? 'uy' : c === 'brasil' ? 'br' : c === 'tdf' ? 'tdf' : 'xx';
  }
  function flagHtml190(v){
    const code = code190(v), cls = flagCls190(v);
    return `<span class="countryFlag flag-${esc190(cls)}" title="${esc190(code)}" aria-label="${esc190(code)}"></span>`;
  }
  function abbr190(v){
    const n = norm190(v), original = clean190(v);
    if(/^clz\b/.test(n) || n.includes('centro logistico zarate')) return 'CLZ';
    if(/^clc\b/.test(n) || n.includes('centro logistico cordoba')) return 'CLC';
    return original;
  }
  function status190(t,e){
    const s = norm190([get190(t,['estado','status']), get190(e,['estado','status'])].join(' '));
    if(s.includes('final') || s.includes('cerr') || t?.closed || t?.cierre) return {txt:'CERRADO', cls:'done'};
    if(s.includes('aduana')) return {txt:'ADUANA', cls:'customs'};
    if(t && Object.keys(t).length) return {txt:'EN TRÁNSITO', cls:'open'};
    return {txt:'PLANIFICADO', cls:'planned'};
  }
  function embNo190(e){
    return clean190(get190(e,['numero','embarque','idEmbarque','codigo']) || e?.id);
  }
  function flota190(t){
    return clean190(get190(t,['flota','fleet','usuario','user']) || t?.user?.fleet);
  }
  function route190(t){ return t?.route || t?.ruta || {}; }
  function lastTime190(t){
    const arr = []
      .concat(Array.isArray(t?.updates) ? t.updates : [])
      .concat(Array.isArray(t?.gps) ? t.gps : [])
      .concat(Array.isArray(t?.posiciones) ? t.posiciones : [])
      .concat(Array.isArray(t?.registros) ? t.registros : []);
    arr.sort((a,b)=>(toDate190(get190(b,['time','fecha','createdAt','updatedAt','ts']))?.getTime()||0)-(toDate190(get190(a,['time','fecha','createdAt','updatedAt','ts']))?.getTime()||0));
    return get190(arr[0]||{},['time','fecha','createdAt','updatedAt','ts']) || get190(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']);
  }
  function collectRecords190(t){
    const out = [];
    const push = r => {
      if(!r || typeof r !== 'object') return;
      const n = clean190(get190(r,['hito','nombre','name','label','title','aduana','aduanaId','checkpoint','puesto','evento','tipo','estado','level','localidad','ciudad']));
      const tm = get190(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora']);
      if(n && hasDate190(tm) && !/^(updates?|gps|registros?|logs?|tracking|posiciones?)$/i.test(n)) out.push({name:n,time:tm});
    };
    ['aduanas','aduana','registros','hitos','eventos','novedades','checkpoints','updates'].forEach(k=>{
      const v = t && t[k];
      if(Array.isArray(v)) v.forEach(push);
    ...
    const seen = new Set();
    return out.sort((a,b)=>(toDate190(a.time)?.getTime()||0)-(toDate190(b.time)?.getTime()||0)).filter(r=>{
      const k = norm190(r.name)+'|'+(toDate190(r.time)?.getTime()||'');
      if(seen.has(k)) return false;
      seen.add(k);
      return true;
    });
  ...
  function latestTime190(t){
    const candidates = [];
    const add = v => { if(hasDate190(v)) candidates.push(v); };
    add(lastTime190(t));
    collectRecords190(t).forEach(r => add(r.time));
    add(get190(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn']));
    add(get190(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']));
    candidates.sort((a,b)=>(toDate190(b)?.getTime()||0)-(toDate190(a)?.getTime()||0));
    return candidates[0] || '';
  }

  function match190(a,b){
    a = norm190(a); b = norm190(b);
    return a && b && (a === b || a.includes(b) || b.includes(a));
  }
  function destinoAduana190(v){
    const s = norm190(v);
    if(s === 'chille') return 'chile';
    if(s === 'brazil') return 'brasil';
    if(/tierra del fuego/.test(s)) return 'tdf';
    return s;
  }
  function nombreAduana190(a){
    return clean190(a && (a.nombre || a.name || a.aduana || a.aduanaId || a.label || a.title || a.id));
  }
  function hitosAduana190(destino, origen){
    const c = country190(destino);
    const docs = Array.isArray(window.aduanas) ? window.aduanas : [];
    const list = docs
      .filter(a => !(a && a.activo === false) && (destinoAduana190(a && a.destino) === c || country190(a && a.destino) === c))
      .map(a => ({name:nombreAduana190(a), orden:Number(a && a.orden !== undefined ? a.orden : 9999)}))
      .filter(x => x.name && !/^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(norm190(x.name)))
      .sort((a,b)=>a.orden-b.orden || norm190(a.name).localeCompare(norm190(b.name)))
      .map(x=>x.name);
    const uniq = [];
    list.forEach(h=>{ if(!uniq.some(x=>norm190(x)===norm190(h))) uniq.push(h); });
    if(uniq.length) return [origen || 'Inicio', ...uniq, destino || 'Cierre'];

    // Fallback sólo si Firebase no trae aduanas. Sin país como hito.
    if(c === 'chile') return [origen || 'Inicio', 'Uspallata', 'Los Andes', destino || 'Cierre'];
    if(c === 'paraguay') return [origen || 'Inicio', 'Clorinda', 'Puerto José Falcón', 'Depósito Fiscal Paraguay', destino || 'Cierre'];
    if(c === 'uruguay') return [origen || 'Inicio', 'Gualeguaychú', 'Fray Bentos', destino || 'Cierre'];
    return [origen || 'Inicio', destino || 'Cierre'];
  }
  async function readCol190(name){
    try{
      const dbc = window.db || (typeof db !== 'undefined' ? db : null);
      if(dbc && typeof dbc.collection === 'function'){
        const s = await dbc.collection(name).get();
        return s.docs.map(d=>({id:d.id,_docId:d.id,...d.data()}));
      }
    }catch(e){ console.warn('Entregas v190 read', name, e); }
    try{
      if(window.firebase && window.firebase.firestore){
        const s = await window.firebase.firestore().collection(name).get();
        return s.docs.map(d=>({id:d.id,_docId:d.id,...d.data()}));
  ...
  async function reload190(){
    const [tr, em, ad] = await Promise.all([readCol190('transitos'), readCol190('embarque'), readCol190('aduana')]);
    if(Array.isArray(tr)){ window.trs = tr; try{ trs = tr; }catch(_e){} }
    if(Array.isArray(em)){ window.embarques = em; try{ embarques = em; }catch(_e){} }
    if(Array.isArray(ad)){ window.aduanas = ad; }
  }
  function buildRows190(){
    const tr = Array.isArray(window.trs) ? window.trs : [];
    const em = Array.isArray(window.embarques) ? window.embarques : [];
    const by = new Map();

    em.forEach(e=>{
      const fl = clean190(get190(e,['flota','fleet','unidad','usuario']) || get190(e?.user,['fleet']));
      if(!fl) return;
      const key = fl;
      const rt = route190(e);
      const t = tr.find(x => flota190(x) === fl) || {};
      const n = embNo190(e);
      const row = by.get(key) || {flota:fl, embarques:[], t:t, e:e};
      if(n && !row.embarques.includes(n)) row.embarques.push(n);
      row.t = Object.keys(t).length ? t : row.t;
      row.e = e;
      row.numero = n || row.numero || '';
      row.cliente = clean190(get190(e,['cliente','client']) || rt.cliente || t.cliente || row.cliente);
      row.origen = clean190(get190(e,['origen','origin']) || rt.origen || t.origen || row.origen || 'Centro Logístico Zárate');
      row.destino = clean190(get190(e,['destino','destination']) || rt.destino || t.destino || row.destino || '');
      row.fecha = get190(e,['fecha','fechaHora','createdAt','start']) || get190(t,['start','inicio','createdAt']) || row.fecha || '';
      by.set(key,row);
    });
    ...
    tr.forEach(t=>{
      ...
      const key = fl;
      const rt = route190(t);
      const row = by.get(key) || {flota:fl, embarques:[], t:t, e:{}};
      row.t = t;
      row.cliente = row.cliente || clean190(rt.cliente || t.cliente);
      row.origen = row.origen || clean190(rt.origen || t.origen || 'Centro Logístico Zárate');
      row.destino = row.destino || clean190(rt.destino || t.destino || '');
      row.fecha = row.fecha || get190(t,['start','inicio','createdAt']);
      if(t.embarque && !row.embarques.includes(String(t.embarque))) row.embarques.push(String(t.embarque));
      by.set(key,row);
    });

    return [...by.values()].sort((a,b)=>String(a.flota).localeCompare(String(b.flota),'es',{numeric:true}));
  }
  function stages190(row){
    const t = row.t || {}, e = row.e || {};
    const origen = clean190(row.origen) || clean190(get190(e,['origen','origin'])) || clean190(get190(t,['origen','origin'])) || 'Inicio';
    const destino = clean190(row.destino) || clean190(get190(e,['destino','destination'])) || clean190(get190(t,['destino','destination'])) || 'Cierre';
    const inicio = get190(t,['start','inicio','fechaInicio','createdAt']) || row.fecha || get190(e,['fecha','createdAt','fechaHora']);
    const cierre = get190(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn']) || get190(e,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn']) || (status190(t,e).cls==='done' ? (latestTime190(t) || row.fecha || get190(e,['fecha','createdAt','fechaHora'])) : '');

    const names = hitosAduana190(destino, origen);
    const st = names.map((name,i)=>({name, time:i===0?inicio:(i===names.length-1?cierre:''), kind:i===0?'start':(i===names.length-1?'end':'aduana')}));
    collectRecords190(t).forEach(r=>{
      let idx = st.findIndex((s,i)=>i>0 && i<st.length-1 && match190(r.name,s.name));
      if(idx >= 0 && !hasDate190(st[idx].time)) st[idx].time = r.time;
    });
    return st.filter((x,i,arr)=> i===0 || i===arr.length-1 || !/^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(norm190(x.name)));
  }
  function completed190(st){
    let idx = 0;
    st.forEach((s,i)=>{ if(hasDate190(s.time)) idx = i; });
    return Math.min(idx, st.length-1);
  }
  function card190(row){
    const state = status190(row.t,row.e);
    const st = stages190(row);
    const pi = state.cls === 'done' ? Math.max(0, st.length-1) : completed190(st);
    const max = Math.max(1, st.length-1);
    const pct = Math.max(0, Math.min(100, (pi/max)*100));
    const upd = fmt190(latestTime190(row.t) || lastTime190(row.t) || row.fecha);
    const el = elapsed190(row.fecha || get190(row.t,['start','inicio','createdAt']) || lastTime190(row.t));
    const next = clean190((st.find((s,i)=>i>pi) || st[pi] || st[0] || {}).name) || '-';

    return `<article class="entregaCard">
      <div class="entregaTop">
        <div class="entregaFleet"><span class="entregaFleetIcon">🚚</span><div><h3>Flota ${esc190(row.flota)}</h3><span class="entregaStatus ${state.cls}">${esc190(state.txt)}</span><small>Emb. ${esc190((row.embarques||[]).join(', ')||row.numero||'-')}</small></div></div>
        <div class="entregaMeta"><span><i>🗓️</i> Última act.</span><b>${esc190(upd)}</b></div>
        <div class="entregaMeta"><span><i>⏱️</i> Tiempo transcurrido</span><b>${esc190(el)}</b></div>
        <div class="entregaMeta"><span><i>🚩</i> Próximo hito</span><b>${esc190(abbr190(next))}</b></div>
        <div class="entregaMeta"><span><i>📡</i> Estado</span><b class="state ${state.cls}">${esc190(state.txt)}</b></div>
        <div class="entregaFlags entregaFlagsOnly"><div><b>${esc190(code190(row.origen))}</b>${flagHtml190(row.origen)}<i>›</i><b>${esc190(code190(row.destino))}</b>${flagHtml190(row.destino)}</div></div>
      </div>
      <div class="entregaTimeline" style="--entrega-progress:${pct/100}">
        <div class="entregaLine"><i style="width:${pct}%"></i></div>
        ${st.map((s,i)=>{
          const d = fmt190(s.time), has = d && d !== '-';
          const left = (i/max)*100;
          return `<div class="entregaStep ${i===0?'timelineStart':''} ${i===st.length-1?'timelineEnd':''} ${i<=pi?'done':''} ${i===pi?'current':''} ${has&&i>0?'hasDate':''}" style="left:${left}%"><b data-full-hito="${esc190(s.name)}" title="${esc190(s.name)}">${esc190(abbr190(s.name))}</b><em>${esc190(d)}</em></div>`;
        }).join('')}
        <div class="entregaVehicle" style="left:calc(72px + (100% - 144px) * ${pct/100})"><img class="entregaCarrierImg" src="assets/camion-automobilero.png" alt="Camión automobilero"></div>
      </div>
      <div class="entregaBottom"><span><b>Actualizado:</b> ${esc190(upd)}</span><span><b>Tiempo transcurrido:</b> ${esc190(el)}</span><span><b>Próximo hito:</b> ${esc190(abbr190(next))}</span><span><b>Estado:</b> ${esc190(state.txt)}</span></div>
    </article>`;
  }
  function ensurePanel190(){
    if(window.ensureEntregaPanelStable) try{ window.ensureEntregaPanelStable(); }catch(e){}
    const sec = document.getElementById('entrega');
    if(!sec) return;
    if(!document.getElementById('entregaCards')){
      sec.insertAdjacentHTML('beforeend','<div id="entregaCards" class="entregaCards"></div>');
    }
    const btn = sec.querySelector('.entregaRefreshBtn');
    if(btn){
      btn.onclick = async function(ev){
        ...
        try{ await window.renderEntrega(true); }finally{ btn.disabled=false; btn.textContent=old||'Actualizar'; }
        return false;
      };
    }
  }
  function fillFilters190(rows){
    const defs=[['entregaFiltroEmbarque','embarques','Embarque'],['entregaFiltroFlota','flota','Todas las flotas'],['entregaFiltroCliente','cliente','Todos los clientes'],['entregaFiltroDestino','destino','Todos los destinos']];
    defs.forEach(([id,key,all])=>{
      const sel=document.getElementById(id);
      if(!sel) return;
      const cur=sel.value;
      let vals=[];
      rows.forEach(r=>{let v=r[key]; if(Array.isArray(v)) vals.push(...v); else if(v) vals.push(v);});
      vals=[...new Set(vals.filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'es',{numeric:true}));
      sel.innerHTML=`<option value="">${all}</option>`+vals.map(v=>`<option value="${esc190(v)}">${key==='embarques'?'Emb. ':''}${esc190(v)}</option>`).join('');
      sel.value=vals.includes(cur)?cur:'';
      sel.onchange=()=>window.renderEntrega(false);
    });
    const es=document.getElementById('entregaFiltroEstado');
    if(es) es.onchange=()=>window.renderEntrega(false);
  }
  window.renderEntrega = async function(force=false){
    ensurePanel190();
    if(force) await reload190();
    let rows = buildRows190();
    fillFilters190(rows);
    const fc=norm190(document.getElementById('entregaFiltroCliente')?.value);
    const fe=norm190(document.getElementById('entregaFiltroEmbarque')?.value);
    const fd=norm190(document.getElementById('entregaFiltroDestino')?.value);
    const ff=norm190(document.getElementById('entregaFiltroFlota')?.value);
    const fs=norm190(document.getElementById('entregaFiltroEstado')?.value);
    rows = rows.filter(r=>(!fc||norm190(r.cliente)===fc)&&(!fe||(r.embarques||[]).some(e=>norm190(e)===fe))&&(!fd||norm190(r.destino)===fd)&&(!ff||norm190(r.flota)===ff)&&(!fs||(fs==='cerrado'?status190(r.t,r.e).cls==='done':status190(r.t,r.e).cls!=='done')));
    const box = document.getElementById('entregaCards');
    if(box) box.innerHTML = rows.length ? rows.map(card190).join('') : '<div class="glassPanel entregaEmpty">No hay flotas para el filtro seleccionado.</div>';
  };
  window.renderEntrega.__renderDefinitivo190 = true;

  const oldTab190 = window.tab;
  if(typeof oldTab190 === 'function' && !oldTab190.__entrega190){
    window.tab = function(id){
      const r = oldTab190.apply(this, arguments);
      if(id === 'entrega') window.renderEntrega(false);
      return r;
    };
    window.tab.__entrega190 = true;
  }

  document.addEventListener('DOMContentLoaded',()=>{ ensurePanel190(); if(document.getElementById('entrega')?.classList.contains('active')) window.renderEntrega(false); });
  window.addEventListener('load',()=>{ ensurePanel190(); if(document.getElementById('entrega')?.classList.contains('active')) window.renderEntrega(false); });
})();




/* ===== v3.3.21 - Entregas: marcar hito final cuando el tránsito está cerrado =====
   Solo afecta la línea de tiempo de Entregas. */
(function(){
  const VERSION_ENTREGA_191='3.3.21';
...
(function(){
  ...
  function fixClosedTimeline192(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      ...
      last.classList.add('done','current','hasDate','timelineEnd');

      const line=tl.querySelector('.entregaLine i');
      if(line) line.style.width='100%';

      tl.style.setProperty('--entrega-progress','1');

      const veh=tl.querySelector('.entregaVehicle');
      if(veh){
        veh.style.left='calc(72px + (100% - 144px) * 1)';
        veh.style.transform='translateX(-50%)';
      }

      const lastDate=last.querySelector('em');
      if(lastDate && (!lastDate.textContent || lastDate.textContent.trim()==='-')){
        const upd=(card.querySelector('.entregaBottom span:first-child')?.textContent||'').replace(/Actualizado:\s*/i,'').trim();
        if(upd) lastDate.textContent=upd;
      }
    });
  }

  const oldRender192=window.renderEntrega;
  if(typeof oldRender192==='function' && !oldRender192.__cierreFinal192){
    window.renderEntrega=async function(){
      const r=await oldRender192.apply(this,arguments);
      fixClosedTimeline192();
      requestAnimationFrame(fixClosedTimeline192);
      setTimeout(fixClosedTimeline192,80);
      return r;
    };
    window.renderEntrega.__cierreFinal192=true;
  }

  const oldTab192=window.tab;
  if(typeof oldTab192==='function' && !oldTab192.__cierreFinal192){
    window.tab=function(id){
      const r=oldTab192.apply(this,arguments);
      if(id==='entrega'){
        fixClosedTimeline192();
        requestAnimationFrame(fixClosedTimeline192);
        setTimeout(fixClosedTimeline192,80);
    ...
    window.tab.__cierreFinal192=true;
  }

  document.addEventListener('DOMContentLoaded',()=>setTimeout(fixClosedTimeline192,120));
  window.addEventListener('load',()=>setTimeout(fixClosedTimeline192,120));
})();




/* ===== v3.3.21 - Entregas: cierre gana contra render heredado tardío =====
   Solo afecta la vista Entregas. Corrección finita, sin observer ni intervalos. */
(function(){
  const VERSION_ENTREGA_193='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_193;
  window.APP_VERSION_V2=VERSION_ENTREGA_193;

  function cardClosed193(card){
    const stateText = [
      card.querySelector('.entregaStatus')?.textContent || '',
      card.querySelector('.entregaMeta .state')?.textContent || '',
      card.querySelector('.entregaBottom')?.textContent || '',
      card.textContent || ''
    ].join(' ').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    return /\bcerrado\b/.test(stateText);
  }

  function markClosed193(card){
    if(!card || !cardClosed193(card)) return;
    const tl = card.querySelector('.entregaTimeline');
    if(!tl) return;
    ...
    last.classList.add('done','hasDate','timelineEnd');
    ...
    const lastEm = last.querySelector('em');
    if(lastEm && (!lastEm.textContent || lastEm.textContent.trim()==='-')){
      const upd = (card.querySelector('.entregaBottom span:first-child')?.textContent || '')
        .replace(/Actualizado:\s*/i,'')
        .trim();
      if(upd) lastEm.textContent = upd;
    }

    const metaNext = card.querySelectorAll('.entregaMeta b')[2];
    if(metaNext && /toyota chile|garden paraguay|sevel uruguay|cierre|destino/i.test(metaNext.textContent || '')){
      metaNext.textContent = 'Tránsito finalizado';
    }

    const bottom = card.querySelector('.entregaBottom');
    if(bottom){
      bottom.querySelectorAll('span').forEach(span=>{
        if(/^\s*Pr[oó]ximo hito\s*:/i.test(span.textContent || '')){
          const b = span.querySelector('b');
          if(b) span.innerHTML = '<b>Próximo hito:</b> Tránsito finalizado';
        }
      });
    }
  }

  function fixClosed193(){
    document.querySelectorAll('#entrega .entregaCard').forEach(markClosed193);
  }

  function scheduleClosedFix193(){
    fixClosed193();
    requestAnimationFrame(fixClosed193);
    [60,140,300,600,1000,1600].forEach(ms=>setTimeout(fixClosed193,ms));
  }

  const oldRender193 = window.renderEntrega;
  if(typeof oldRender193 === 'function' && !oldRender193.__cierrePost193){
    window.renderEntrega = async function(){
      const r = await oldRender193.apply(this, arguments);
      scheduleClosedFix193();
      return r;
    };
    window.renderEntrega.__cierrePost193 = true;
  }

  const oldTab193 = window.tab;
  if(typeof oldTab193 === 'function' && !oldTab193.__cierrePost193){
    window.tab = function(id){
      const r = oldTab193.apply(this, arguments);
      if(id === 'entrega') scheduleClosedFix193();
      return r;
    };
    window.tab.__cierrePost193 = true;
  }

  document.addEventListener('DOMContentLoaded',()=>setTimeout(scheduleClosedFix193,200));
  window.addEventListener('load',()=>setTimeout(scheduleClosedFix193,200));
})();




/* ===== v3.3.21 - Entregas: cierre 100% verde =====
   Solo afecta Entregas. Cuando el tránsito está cerrado:
   línea completa, todos los círculos verdes y camión al último hito. */
(function(){
  const VERSION_ENTREGA_194='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_194;
  window.APP_VERSION_V2=VERSION_ENTREGA_194;

  function isClosed194(card){
    const txt=[
      card.querySelector('.entregaStatus')?.textContent||'',
      card.querySelector('.entregaMeta .state')?.textContent||'',
      card.querySelector('.entregaBottom')?.textContent||''
  ...
  function markClosed194(card){
    if(!card || !isClosed194(card)) return;
    card.classList.add('closedTransit');
    ...
    if(last){
      last.classList.add('done','hasDate','timelineEnd');
      last.classList.remove('current');
      const em=last.querySelector('em');
      if(em && (!em.textContent || em.textContent.trim()==='-')){
        const upd=(card.querySelector('.entregaBottom span:first-child')?.textContent||'').replace(/Actualizado:\s*/i,'').trim();
        if(upd) em.textContent=upd;
      }
    }
  ...
  function applyClosed194(){
    document.querySelectorAll('#entrega .entregaCard').forEach(markClosed194);
  }

  const oldRender194=window.renderEntrega;
  if(typeof oldRender194==='function' && !oldRender194.__cierreVerde194){
    window.renderEntrega=async function(){
      const r=await oldRender194.apply(this,arguments);
      applyClosed194();
      requestAnimationFrame(applyClosed194);
      setTimeout(applyClosed194,120);
      setTimeout(applyClosed194,360);
      setTimeout(applyClosed194,900);
      return r;
    };
    window.renderEntrega.__cierreVerde194=true;
  }

  const oldTab194=window.tab;
  if(typeof oldTab194==='function' && !oldTab194.__cierreVerde194){
    window.tab=function(id){
      const r=oldTab194.apply(this,arguments);
      if(id==='entrega'){
        applyClosed194();
        requestAnimationFrame(applyClosed194);
        setTimeout(applyClosed194,120);
    ...
    window.tab.__cierreVerde194=true;
  }

  function installObserver194(){
    const root=document.getElementById('entregaCards');
    if(!root || root.__closedObserver194) return;
    let pending=false;
    root.__closedObserver194=new MutationObserver(()=>{
      if(pending) return;
      pending=true;
      requestAnimationFrame(()=>{pending=false; applyClosed194();});
    });
    root.__closedObserver194.observe(root,{childList:true,subtree:true});
  }

  document.addEventListener('DOMContentLoaded',()=>{installObserver194(); setTimeout(applyClosed194,150);});
  window.addEventListener('load',()=>{installObserver194(); setTimeout(applyClosed194,150);});
  setTimeout(()=>{installObserver194(); applyClosed194();},600);
})();




/* ===== v3.3.21 - Entregas: botón Actualizar con render único =====
   Solo afecta el botón Actualizar de Entregas.
   Evita que un handler heredado vuelva a pintar datos anteriores, especialmente Última act. */
(function(){
  const VERSION_ENTREGA_195='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_195;
  window.APP_VERSION_V2=VERSION_ENTREGA_195;
  let busy195=false;
  let lastData195=null;

  async function readCol195(name){
    try{
      const dbc = window.db || (typeof db !== 'undefined' ? db : null);
      if(dbc && typeof dbc.collection === 'function'){
        const snap = await dbc.collection(name).get();
        return snap.docs.map(d=>({id:d.id,_docId:d.id,...d.data()}));
      }
    }catch(e){ console.warn('Entregas v195 read', name, e); }
    try{
      if(window.firebase && window.firebase.firestore){
        const snap = await window.firebase.firestore().collection(name).get();
        return snap.docs.map(d=>({id:d.id,_docId:d.id,...d.data()}));
      }
    }catch(e){}
    return null;
  ...
  async function reloadData195(){
    const [tr, em, ad] = await Promise.all([
      readCol195('transitos'),
      readCol195('embarque'),
      readCol195('aduana')
    ]);
    if(Array.isArray(tr)){ window.trs=tr; try{ trs=tr; }catch(_e){} }
    if(Array.isArray(em)){ window.embarques=em; try{ embarques=em; }catch(_e){} }
    if(Array.isArray(ad)){ window.aduanas=ad; }
    lastData195={tr,em,ad,ts:Date.now()};
  }

  function applyClosed195(){
    if(typeof window.__applyClosedEntrega194 === 'function'){
      try{ window.__applyClosedEntrega194(); }catch(e){}
    }
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      const txt=[
        card.querySelector('.entregaStatus')?.textContent||'',
      ...
      if(line) line.style.width='100%';
      const steps=[...tl.querySelectorAll('.entregaStep')];
      steps.forEach(s=>{s.classList.add('done');s.classList.remove('current');});
      const last=steps[steps.length-1];
      if(last) last.classList.add('done','hasDate','timelineEnd');
      const veh=tl.querySelector('.entregaVehicle');
      if(veh){veh.style.left='calc(72px + (100% - 144px) * 1)';veh.style.transform='translateX(-50%)';}
    });
  }

  async function finalRefresh195(btn){
    if(busy195) return false;
    busy195=true;
    const b=btn || document.querySelector('#entrega .entregaRefreshBtn');
    const old=b ? b.textContent : 'Actualizar';
    if(b){ b.disabled=true; b.textContent='Actualizando...'; }
    try{
      ...
      /* v3.3.21: renders tardíos heredados desactivados */
    }finally{
      setTimeout(()=>{
        if(b){ b.disabled=false; b.textContent=old || 'Actualizar'; }
        busy195=false;
  ...
  // Reemplaza funciones viejas usadas por los handlers anteriores.
  window.forceEntregaRefresh195=finalRefresh195;
  window.forceEntregaRefresh174=finalRefresh195;
  window.forceEntregaIsolatedRefresh173=finalRefresh195;
  window.forceEntregaIsolatedRefresh172=finalRefresh195;
  window.forceEntregaInlineRefresh171=finalRefresh195;
  window.forceEntregaCleanRefresh170=finalRefresh195;
  window.forceEntregaHardRefresh169=finalRefresh195;
  window.forceRefreshEntrega168=finalRefresh195;

  document.addEventListener('click',function(ev){
    const btn=ev.target && ev.target.closest ? ev.target.closest('#entrega .entregaRefreshBtn') : null;
    if(!btn) return;
    ev.preventDefault();
    ev.stopPropagation();
    if(ev.stopImmediatePropagation) ev.stopImmediatePropagation();
  ...
  document.addEventListener('DOMContentLoaded',()=>setTimeout(wire195,100));
  window.addEventListener('load',()=>setTimeout(wire195,100));
  setInterval(wire195,900);
})();




/* ===== v3.3.21 - Entregas: fecha correcta de cierre y última actualización =====
   Solo afecta Entregas. Toma la fecha más nueva de registros del tránsito para cerrado. */
(function(){
  const VERSION_ENTREGA_196='3.3.21';
  window.ELTA_APP_VERSION=VERSION_ENTREGA_196;
  window.APP_VERSION_V2=VERSION_ENTREGA_196;

  function parseDate196(txt){
    if(!txt) return null;
    // Soporta formatos dd/mm, hh:mm usados visualmente; se usa solo para no borrar fechas.
    return String(txt).trim();
  }

  function cardClosed196(card){
    const txt=[
      card.querySelector('.entregaStatus')?.textContent||'',
      card.querySelector('.entregaMeta .state')?.textContent||'',
      card.querySelector('.entregaBottom')?.textContent||''
  ...
  function fixClosedDates196(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      if(!cardClosed196(card)) return;
      const metaUpd=card.querySelector('.entregaMeta:nth-of-type(1) b');
      const bottomUpd=card.querySelector('.entregaBottom span:first-child');
      const last=card.querySelector('.entregaTimeline .entregaStep:last-of-type em');
      const currentLast=last ? parseDate196(last.textContent) : '';
      const currentUpd=metaUpd ? parseDate196(metaUpd.textContent) : '';
      // Si el último hito tiene fecha y Última act. quedó vieja, usar la fecha del último hito.
      if(currentLast && currentLast !== '-' && metaUpd && currentUpd !== currentLast){
        metaUpd.textContent=currentLast;
        if(bottomUpd) bottomUpd.innerHTML='<b>Actualizado:</b> '+currentLast;
      }
    });
  }

  const oldRender196=window.renderEntrega;
  if(typeof oldRender196==='function' && !oldRender196.__fechaCierre196){
    window.renderEntrega=async function(){
      const r=await oldRender196.apply(this,arguments);
      fixClosedDates196();
      requestAnimationFrame(fixClosedDates196);
      setTimeout(fixClosedDates196,160);
      setTimeout(fixClosedDates196,500);
      return r;
    };
    window.renderEntrega.__fechaCierre196=true;
  }

  const oldTab196=window.tab;
  if(typeof oldTab196==='function' && !oldTab196.__fechaCierre196){
    window.tab=function(id){
      const r=oldTab196.apply(this,arguments);
      if(id==='entrega'){
        fixClosedDates196();
        requestAnimationFrame(fixClosedDates196);
        setTimeout(fixClosedDates196,160);
    ...
    window.tab.__fechaCierre196=true;
  }
})();
...
/* ===== v3.3.21 - Forzar version visible ===== */
(function(){
  const V='3.3.21';
...
/* ===== v3.3.21 - Embarques only: sin render heredado =====
...
(function(){
  ...
  function cleanEmbarques208(){
    const tbody=document.getElementById('embarquesTbody');
    if(!tbody) return;
    [...tbody.querySelectorAll('tr')].forEach(tr=>{
      if(tr.querySelector('.emptyRow')) return;
      const first=(tr.querySelector('td')?.textContent||'').trim();
      if(!/^Emb\.\s*[A-Za-z0-9_-]+/i.test(first)){
        tr.remove();
        return;
      }
      while(tr.children.length>8) tr.removeChild(tr.lastElementChild);
      const action=tr.querySelector('td:last-child');
      if(action){
        const dels=[...action.querySelectorAll('.deleteEmbarqueBtnV275')];
        dels.slice(1).forEach(x=>x.remove());
      }
    });
  }

  const oldRender208=window.renderEmbarquesV244;
  if(typeof oldRender208==='function' && !oldRender208.__v208){
    window.renderEmbarquesV244=async function(){
      const r=await oldRender208.apply(this,arguments);
      cleanEmbarques208();
      setVersion208();
      return r;
    };
    window.renderEmbarquesV244.__v208=true;
  }

  document.addEventListener('DOMContentLoaded',()=>{setVersion208(); setTimeout(cleanEmbarques208,200);});
  window.addEventListener('load',()=>{setVersion208(); setTimeout(cleanEmbarques208,200);});
})();




/* ===== v3.3.21 - Eliminar embarque real + hitos intermedios Entregas =====
   Corrige:
...
(function(){
  ...
  const S=v=>String(v??'').trim();
  const N=v=>S(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');

  async function readCol209(name){
    try{
      const dbc=window.db || (typeof db!=='undefined'?db:null);
      if(dbc && typeof dbc.collection==='function'){
        const snap=await dbc.collection(name).get();
        return snap.docs.map(d=>({id:d.id,_docId:d.id,ref:d.ref,...d.data()}));
      }
    }catch(e){console.warn('v209 read',name,e);}
    try{
      if(window.firebase && window.firebase.firestore){
        const snap=await window.firebase.firestore().collection(name).get();
        return snap.docs.map(d=>({id:d.id,_docId:d.id,ref:d.ref,...d.data()}));
  ...
  async function deleteDocAndCargas209(ref){
    if(!ref) return false;
    try{
      const snap=await ref.collection('cargas').get();
      if(snap && snap.docs && snap.docs.length){
        const dbc=window.db || (typeof db!=='undefined'?db:null);
        const batch=dbc && dbc.batch ? dbc.batch() : null;
        if(batch){
    ...
    }catch(e){console.warn('v209 cargas delete',e);}
    try{
      await ref.delete();
      return true;
    }catch(e){
      console.warn('v209 doc delete failed; marking deleted',e);
      try{
        await ref.set({activo:false,deleted:true,deletedAt:new Date().toISOString()},{merge:true});
        return true;
      }catch(err){console.warn('v209 mark deleted failed',err);}
    }
    return false;
  }

  async function deleteEmbarqueReal209(numero){
    numero=S(numero);
    ...
    if(dbc && typeof dbc.collection==='function'){
      const col=dbc.collection('embarque');

      // 1) Intento por ID de documento.
      try{
        const ref=col.doc(numero);
        const snap=await ref.get();
        if(snap && snap.exists) deleted = (await deleteDocAndCargas209(ref)) || deleted;
      }catch(e){console.warn('v209 delete by doc id',e);}

      // 2) Intento por campos usados en distintas versiones.
      for(const field of ['numero','embarque','idEmbarque','codigo']){
        try{
          const qs=await col.where(field,'==',numero).get();
          if(qs && qs.docs){
            for(const d of qs.docs){
              deleted = (await deleteDocAndCargas209(d.ref)) || deleted;
            }
          }
        }catch(e){console.warn('v209 delete by field',field,e);}
      }
    }

    // 3) Limpieza local siempre.
    try{
      ['elta_cargas_embarque_v245','elta_cargas_embarque_v244'].forEach(k=>{
        try{
          const arr=JSON.parse(localStorage.getItem(k)||'[]')||[];
          localStorage.setItem(k,JSON.stringify(arr.filter(c=>S(c?.embarque)!==numero)));
        }catch(e){}
      });
      let del=[];
      try{del=JSON.parse(localStorage.getItem('elta_deleted_embarques_v275')||'[]')||[];}catch(e){del=[];}
      if(!del.includes(numero)) del.push(numero);
      localStorage.setItem('elta_deleted_embarques_v275',JSON.stringify(del));
    }catch(e){}

    try{
      if(Array.isArray(window.embarques)) window.embarques=window.embarques.filter(e=>S(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId)!==numero);
    }catch(e){}
    try{
      if(typeof embarques!=='undefined' && Array.isArray(embarques)) embarques=embarques.filter(e=>S(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId)!==numero);
    }catch(e){}

    return deleted || true;
  }

  // Reemplaza la confirmacion anterior: ahora no muestra exito hasta terminar el borrado real/local.
  window.confirmDeleteEmbarqueV275=function(numero){
    numero=S(numero);
    if(!numero) return;
    const ok=confirm('¿Confirmar eliminación del embarque '+numero+'?');
    if(!ok) return;

    deleteEmbarqueReal209(numero).then(async()=>{
      try{ if(typeof refresh==='function') await refresh(); }catch(e){}
      try{ if(typeof window.renderEmbarquesV244==='function') await window.renderEmbarquesV244(); }catch(e){}
      alert('Embarque '+numero+' eliminado correctamente.');
    }).catch(e=>{
      console.error(e);
  ...
  // ENTREGAS: asegurar aduana antes de render. Si no existe window.aduanas, se carga.
  async function ensureAduanas209(){
    try{
      if(Array.isArray(window.aduanas) && window.aduanas.length) return;
      const ad=await readCol209('aduana');
      if(Array.isArray(ad)){
        window.aduanas=ad;
        try{aduanas=ad;}catch(e){}
      }
    }catch(e){console.warn('v209 ensure aduanas',e);}
  }

  const oldRenderEntrega209=window.renderEntrega;
  if(typeof oldRenderEntrega209==='function' && !oldRenderEntrega209.__aduanas209){
    window.renderEntrega=async function(){
      await ensureAduanas209();
      const r=await oldRenderEntrega209.apply(this,arguments);
      return r;
    };
    window.renderEntrega.__aduanas209=true;
  }

  const oldTab209=window.tab;
  if(typeof oldTab209==='function' && !oldTab209.__aduanas209){
    window.tab=function(id){
      const r=oldTab209.apply(this,arguments);
      if(id==='entrega'){ ensureAduanas209().then(()=>{try{window.renderEntrega&&window.renderEntrega(false);}catch(e){}}); }
      return r;
    };
    window.tab.__aduanas209=true;
  }

  document.addEventListener('DOMContentLoaded',()=>{ensureAduanas209();});
  window.addEventListener('load',()=>{ensureAduanas209();});
})();




/* ===== v3.3.21 - Limpieza definitiva Entregas + Embarques =====
...
(function(){
  ...
  const S=v=>String(v??'').trim();
  const N=v=>S(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  const get=(o,ks)=>{for(const k of ks){const v=o&&o[k]; if(v!==undefined&&v!==null&&String(v).trim()!=='')return v;} return '';};
  const arr=name=>{try{if(Array.isArray(window[name]))return window[name]; if(Array.isArray(globalThis[name]))return globalThis[name];}catch(e){} return [];};

  function setVersion210(){
    try{
      document.title='ELTA ITS - Versión '+VERSION_210;
      document.querySelectorAll('span,small,p,div,footer').forEach(el=>{
        if(el.childElementCount===0 && /Versi[oó]n\s+\d+\.\d+\.\d+/i.test(el.textContent||'')){
          el.textContent=(el.textContent||'').replace(/Versi[oó]n\s+\d+\.\d+\.\d+/gi,'Versión '+VERSION_210);
        }
      });
    }catch(e){}
  }
  ...
  function hasDate(v){return !!toDate(v);}
  function fmt(v, year=false){
    const d=toDate(v);
    if(!d) return S(v)||'-';
    const opt=year?{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}:{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'};
    return d.toLocaleString('es-AR',opt);
  }
  function elapsed(a,b){
    const d1=toDate(a), d2=toDate(b)||new Date();
    if(!d1)return '-';
    let ms=Math.max(0,d2-d1);
    const m=Math.floor(ms/60000), dd=Math.floor(m/1440), h=Math.floor((m%1440)/60), mm=m%60;
    return `${dd} d ${h} h ${mm} m`;
  }
  async function readCol(name){
    try{
      const dbc=window.db || (typeof db!=='undefined'?db:null);
    ...
    }catch(e){console.warn('v210 read',name,e);}
    try{
      if(window.firebase&&window.firebase.firestore){
        const snap=await window.firebase.firestore().collection(name).get();
        return snap.docs.map(d=>({id:d.id,_docId:d.id,ref:d.ref,...d.data()}));
  ...
  async function ensureData(force=false){
    const promises=[
      (force||!arr('embarques').length)?readCol('embarque'):Promise.resolve(null),
      (force||!arr('trs').length)?readCol('transitos'):Promise.resolve(null),
      (force||!arr('aduanas').length)?readCol('aduana'):Promise.resolve(null),
      (force||!arr('clientes').length)?readCol('clientes'):Promise.resolve(null),
      (force||!arr('origenes').length)?readCol('origenes'):Promise.resolve(null),
      (force||!arr('destinos').length)?readCol('destinos'):Promise.resolve(null)
    ];
    const [emb,tr,ad,cli,ori,des]=await Promise.all(promises);
    if(Array.isArray(emb)&&emb.length>=0){window.embarques=emb; try{embarques=emb;}catch(e){}}
    if(Array.isArray(tr)&&tr.length>=0){window.trs=tr; try{trs=tr;}catch(e){}}
    if(Array.isArray(ad)&&ad.length>=0){window.aduanas=ad; try{aduanas=ad;}catch(e){}}
    if(Array.isArray(cli)&&cli.length>=0){window.clientes=cli; try{clientes=cli;}catch(e){}}
    if(Array.isArray(ori)&&ori.length>=0){window.origenes=ori; try{origenes=ori;}catch(e){}}
    if(Array.isArray(des)&&des.length>=0){window.destinos=des; try{destinos=des;}catch(e){}}
  }

  // ---------------- ENTREGAS ----------------
  function ensureEntregaPanel(){
    if(typeof window.ensureEntregaPanelStable==='function'){try{window.ensureEntregaPanelStable();}catch(e){}}
    let sec=$('entrega');
    if(!sec){
      sec=document.createElement('section');
      sec.id='entrega';
      sec.className='panel entregaPanel';
      const emb=$('embarques');
      (emb?.parentNode||document.querySelector('main')||document.body).insertBefore(sec, emb?emb.nextSibling:null);
    }
    if(!$('entregaCards')){
      sec.innerHTML=`<div class="sectionTitle entregaHeader"><div><h2>Entregas</h2><p>Seguimiento de flotas en tránsito</p></div><button type="button" class="entregaRefreshBtn">Actualizar</button></div>
      <div class="entregaToolbar glassPanel entregaToolbarCompact">
        <label class="entregaFilterLabel"><span>Embarque</span><select id="entregaFiltroEmbarque"><option value="">Embarque</option></select></label>
        <label class="entregaFilterLabel"><span>Flota</span><select id="entregaFiltroFlota"><option value="">Todas las flotas</option></select></label>
        <label class="entregaFilterLabel"><span>Cliente</span><select id="entregaFiltroCliente"><option value="">Todos los clientes</option></select></label>
  ...
  function country(v){
    const s=N(v);
    if(/tierra del fuego|ushuaia|rio grande|tolhuin/.test(s))return 'tdf';
    if(/chile|stli|toyota chile|los andes|libertadores|\bcl\b/.test(s))return 'chile';
    if(/paraguay|asuncion|puerto jose falcon|falcon|garden paraguay|\bpy\b/.test(s))return 'paraguay';
    if(/uruguay|montevideo|fray bentos|sevel uruguay|\buy\b/.test(s))return 'uruguay';
    if(/brasil|brazil|curitiba|sao paulo|\bbr\b/.test(s))return 'brasil';
  ...
  function code(v){const c=country(v);return c==='argentina'?'AR':c==='chile'?'CL':c==='paraguay'?'PY':c==='uruguay'?'UY':c==='brasil'?'BR':c==='tdf'?'TDF':'--';}
  function flagCls(v){const c=country(v);return c==='argentina'?'ar':c==='chile'?'cl':c==='paraguay'?'py':c==='uruguay'?'uy':c==='brasil'?'br':c==='tdf'?'tdf':'xx';}
  function flagHtml(v){return `<span class="countryFlag flag-${esc(flagCls(v))}" title="${esc(code(v))}" aria-label="${esc(code(v))}"></span>`;}
  function abbr(v){
    const n=N(v), o=S(v);
    if(/^clz\b/.test(n)||n.includes('centro logistico zarate'))return 'CLZ';
    if(/^clc\b/.test(n)||n.includes('centro logistico cordoba'))return 'CLC';
    if(/^tz\b/.test(n)||n.includes('puerto zarate'))return 'TZ - PUERTO ZARATE';
    return o;
  }
  function status(t,e){
    const s=N([get(t,['estado','status']),get(e,['estado','status'])].join(' '));
    if(s.includes('final')||s.includes('cerr')||t?.closed||t?.cierre)return {txt:'CERRADO',cls:'done'};
    if(s.includes('aduana'))return {txt:'EN ADUANA',cls:'customs'};
    if(t&&Object.keys(t).length)return {txt:'EN TRÁNSITO',cls:'open'};
    return {txt:'PLANIFICADO',cls:'planned'};
  }
  function embNum(e){return S(get(e,['numero','embarque','idEmbarque','codigo'])||e?.id||e?._docId);}
  function flota(t){return S(get(t,['flota','fleet','usuario','user'])||get(t?.user,['fleet','flota']));}
  function route(t){return (t&&(t.route||t.ruta||t.recorrido))||{};}
  function startTime(t,e){return get(t,['start','inicio','fechaInicio','createdAt'])||get(e,['fecha','fechaHora','createdAt']);}
  function latestTime(t){
    const vals=[], add=v=>{if(hasDate(v))vals.push(v);};
    add(get(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']));
    ['updates','gps','posiciones','registros','aduanas','hitos','eventos','novedades','checkpoints'].forEach(k=>{
      const v=t&&t[k], a=Array.isArray(v)?v:(v&&typeof v==='object'?Object.values(v):[]);
      a.forEach(r=>add(get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora'])));
    });
    vals.sort((a,b)=>(toDate(b)?.getTime()||0)-(toDate(a)?.getTime()||0));
    return vals[0]||'';
  }
  function closeTime(t,e){return get(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn'])||get(e,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn'])||latestTime(t);}
  function records(t){
    const out=[];
    const push=r=>{
      if(!r||typeof r!=='object')return;
      const name=S(get(r,['hito','nombre','name','label','title','aduana','aduanaId','checkpoint','puesto','evento','tipo','estado','level','localidad','ciudad']));
      const time=get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora']);
      if(name&&hasDate(time)&&!/^(updates?|gps|registros?|logs?|tracking|posiciones?)$/i.test(name))out.push({name,time});
    };
    ['aduanas','aduana','registros','hitos','eventos','novedades','checkpoints','updates'].forEach(k=>{
      const v=t&&t[k];
      if(Array.isArray(v))v.forEach(push); else if(v&&typeof v==='object')Object.values(v).forEach(push);
    });
    const seen=new Set();
    return out.sort((a,b)=>(toDate(a.time)?.getTime()||0)-(toDate(b.time)?.getTime()||0)).filter(r=>{const k=N(r.name)+'|'+(toDate(r.time)?.getTime()||''); if(seen.has(k))return false; seen.add(k); return true;});
  }
  function aduanaHitos(destino,origen){
    const c=country(destino);
    const mapDest=d=>{const s=N(d); if(s==='chille')return 'chile'; if(s==='brazil')return 'brasil'; return s;};
    const docs=arr('aduanas');
    const list=docs.filter(a=>a&&a.activo!==false&&(mapDest(a.destino)===c||country(a.destino)===c))
      .map(a=>({name:S(a.nombre||a.name||a.aduana||a.aduanaId||a.label||a.title||a.id),orden:Number(a.orden??9999)}))
      .filter(x=>x.name&&!/^(argentina|chile|uruguay|paraguay|brasil|brazil)$/i.test(N(x.name)))
      .sort((a,b)=>a.orden-b.orden||N(a.name).localeCompare(N(b.name))).map(x=>x.name);
    const uniq=[]; list.forEach(h=>{if(!uniq.some(x=>N(x)===N(h)))uniq.push(h);});
    if(uniq.length)return [origen||'Origen',...uniq,destino||'Cierre'];
    if(c==='chile')return [origen||'Origen','Uspallata','Los Andes',destino||'Cierre'];
    if(c==='paraguay')return [origen||'Origen','Clorinda','Puerto José Falcón','Depósito Fiscal Paraguay',destino||'Cierre'];
    if(c==='uruguay')return [origen||'Origen','Gualeguaychú','Fray Bentos',destino||'Cierre'];
    return [origen||'Origen',destino||'Cierre'];
  }
  function match(a,b){a=N(a);b=N(b);return a&&b&&(a===b||a.includes(b)||b.includes(a));}
  function entregaRows(){
    const by=new Map();
    arr('embarques').forEach(e=>{
      const f=S(get(e,['flota','fleet','unidad','usuario'])||get(e?.user,['fleet']));
      const n=embNum(e), key=f||n; if(!key)return;
      const row=by.get(key)||{flota:f,embarques:[],e,t:{}};
      if(n&&!row.embarques.includes(n))row.embarques.push(n);
      row.e=e; row.numero=n||row.numero; row.cliente=S(get(e,['cliente','client'])||row.cliente);
      row.origen=S(get(e,['origen','origin'])||row.origen);
      row.destino=S(get(e,['destino','destination'])||row.destino);
      row.fecha=get(e,['fecha','fechaHora','createdAt'])||row.fecha;
      by.set(key,row);
    });
    arr('trs').forEach(t=>{
      const f=flota(t), n=S(t&&t.embarque), key=f||n; if(!key)return;
      const row=by.get(key)||{flota:f,embarques:[],e:{}};
      row.t=t; row.flota=f||row.flota; if(n&&!row.embarques.includes(n))row.embarques.push(n);
      const r=route(t); row.cliente=row.cliente||S(r.cliente||t.cliente);
      row.origen=row.origen||S(r.origen||t.origen);
      row.destino=row.destino||S(r.destino||t.destino);
      row.fecha=row.fecha||startTime(t,{});
      by.set(key,row);
    });
    return [...by.values()].sort((a,b)=>String(a.flota).localeCompare(String(b.flota),'es',{numeric:true}));
  }
  function stages(row){
    const t=row.t||{}, e=row.e||{};
    const origen=row.origen||S(get(e,['origen','origin']))||S(get(t,['origen','origin']))||'Origen';
    const destino=row.destino||S(get(e,['destino','destination']))||S(get(t,['destino','destination']))||'Cierre';
    const names=aduanaHitos(destino,origen);
    const st=names.map((name,i)=>({name,time:i===0?startTime(t,e):(i===names.length-1?closeTime(t,e):''),kind:i===0?'start':(i===names.length-1?'end':'aduana')}));
    records(t).forEach(r=>{const idx=st.findIndex((s,i)=>i>0&&i<st.length-1&&match(r.name,s.name)); if(idx>=0&&!hasDate(st[idx].time))st[idx].time=r.time;});
    return st;
  }
  function entregaCard(row){
    const st=stages(row), stt=status(row.t,row.e);
    let idx=0; st.forEach((s,i)=>{if(hasDate(s.time))idx=i;});
    if(stt.cls==='done')idx=st.length-1;
    const max=Math.max(1,st.length-1), pct=idx/max*100;
    const upd=fmt(stt.cls==='done'?closeTime(row.t,row.e):(latestTime(row.t)||row.fecha),true);
    const start=st[0]?.time||row.fecha, end=stt.cls==='done'?closeTime(row.t,row.e):null;
    const next=stt.cls==='done'?'Tránsito finalizado':(st.find((s,i)=>i>idx)?.name||st[idx]?.name||'-');
    return `<article class="entregaCard ${stt.cls==='done'?'closedTransit':''}">
      <div class="entregaTop">
        <div class="entregaFleet"><span class="entregaFleetIcon">🚚</span><div><h3>Flota ${esc(row.flota||'-')}</h3><span class="entregaStatus ${stt.cls}">${esc(stt.txt)}</span><small>Emb. ${esc((row.embarques||[]).join(', ')||row.numero||'-')}</small></div></div>
        <div class="entregaMeta"><span><i>🗓️</i> Última act.</span><b>${esc(upd)}</b></div>
        <div class="entregaMeta"><span><i>⏱️</i> Tiempo transcurrido</span><b>${esc(elapsed(start,end))}</b></div>
        <div class="entregaMeta"><span><i>🚩</i> Próximo hito</span><b>${esc(abbr(next))}</b></div>
        <div class="entregaMeta"><span><i>📡</i> Estado</span><b class="state ${stt.cls}">${esc(stt.txt)}</b></div>
        <div class="entregaFlags entregaFlagsOnly"><div><b>${esc(code(row.origen))}</b>${flagHtml(row.origen)}<i>›</i><b>${esc(code(row.destino))}</b>${flagHtml(row.destino)}</div></div>
      </div>
      <div class="entregaTimeline" style="--entrega-progress:${pct/100}">
        <div class="entregaLine"><i style="width:${pct}%"></i></div>
        ${st.map((s,i)=>{const d=fmt(s.time,false),has=d&&d!=='-'; const left=i/max*100; return `<div class="entregaStep ${i===0?'timelineStart':''} ${i===st.length-1?'timelineEnd':''} ${i<=idx?'done':''} ${i===idx&&stt.cls!=='done'?'current':''} ${has&&i>0?'hasDate':''}" style="left:${left}%"><b data-full-hito="${esc(s.name)}" title="${esc(s.name)}">${esc(abbr(s.name))}</b><em>${esc(d)}</em></div>`;}).join('')}
        <div class="entregaVehicle" style="left:calc(72px + (100% - 144px) * ${pct/100})"><img class="entregaCarrierImg" src="assets/camion-automobilero.png" alt="Camión automobilero"></div>
      </div>
      <div class="entregaBottom"><span><b>Actualizado:</b> ${esc(upd)}</span><span><b>Tiempo transcurrido:</b> ${esc(elapsed(start,end))}</span><span><b>Próximo hito:</b> ${esc(abbr(next))}</span><span><b>Estado:</b> ${esc(stt.txt)}</span></div>
    </article>`;
  }
  function fillSelect(id,vals,all,cb){
    const sel=$(id); if(!sel)return;
    const cur=sel.value;
    const list=[...new Set(vals.filter(Boolean).map(S))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
    sel.innerHTML=`<option value="">${esc(all)}</option>`+list.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
    if(list.includes(cur))sel.value=cur;
    if(cb)sel.onchange=cb;
  }

  window.renderEntrega=async function(force=false){
    ensureEntregaPanel();
    await ensureData(force);
    let rows=entregaRows();
    fillSelect('entregaFiltroEmbarque',rows.flatMap(r=>r.embarques||[]),'Embarque',()=>window.renderEntrega(false));
    fillSelect('entregaFiltroFlota',rows.map(r=>r.flota),'Todas las flotas',()=>window.renderEntrega(false));
    fillSelect('entregaFiltroCliente',rows.map(r=>r.cliente),'Todos los clientes',()=>window.renderEntrega(false));
    fillSelect('entregaFiltroDestino',rows.map(r=>r.destino),'Todos los destinos',()=>window.renderEntrega(false));
    const fe=N($('entregaFiltroEmbarque')?.value), ff=N($('entregaFiltroFlota')?.value), fc=N($('entregaFiltroCliente')?.value), fd=N($('entregaFiltroDestino')?.value), fs=N($('entregaFiltroEstado')?.value);
    if($('entregaFiltroEstado')) $('entregaFiltroEstado').onchange=()=>window.renderEntrega(false);
    rows=rows.filter(r=>(!fe||(r.embarques||[]).some(e=>N(e)===fe))&&(!ff||N(r.flota)===ff)&&(!fc||N(r.cliente)===fc)&&(!fd||N(r.destino)===fd)&&(!fs||(fs==='cerrado'?status(r.t,r.e).cls==='done':status(r.t,r.e).cls!=='done')));
    const box=$('entregaCards');
    if(box)box.innerHTML=rows.length?rows.map(entregaCard).join(''):'<div class="glassPanel entregaEmpty">No hay flotas para el filtro seleccionado.</div>';
  };
  window.renderEntrega.__v210Clean=true;

  // ---------------- EMBARQUES ----------------
  function deletedSet(){
    try{return new Set(JSON.parse(localStorage.getItem('elta_deleted_embarques_v275')||'[]').map(S));}catch(e){return new Set();}
  }
  function cargoStore(){
    let out=[]; ['elta_cargas_embarque_v245','elta_cargas_embarque_v244'].forEach(k=>{try{const a=JSON.parse(localStorage.getItem(k)||'[]'); if(Array.isArray(a))out=out.concat(a);}catch(e){}});
    return out;
  }
  function embRows(){
    const del=deletedSet(), by=new Map();
    arr('embarques').forEach(e=>{
      const n=S(get(e,['numero','embarque','idEmbarque','codigo'])||e.id||e._docId); if(!n||del.has(n)||e.deleted===true||e.activo===false)return;
      const r=by.get(n)||{numero:n,flotas:new Set()};
      r.cliente=S(get(e,['cliente','client'])||r.cliente); r.origen=S(get(e,['origen','origin'])||r.origen); r.destino=S(get(e,['destino','destination'])||r.destino); r.fecha=get(e,['fecha','fechaHora','createdAt','updatedAt'])||r.fecha;
      const f=S(get(e,['flota','fleet','unidad'])); if(f)r.flotas.add(f); if(Array.isArray(e.flotas))e.flotas.forEach(x=>x&&r.flotas.add(S(x)));
      by.set(n,r);
    });
    arr('trs').forEach(t=>{
      const n=S(t&&t.embarque); if(!n||del.has(n))return;
      const r=by.get(n)||{numero:n,flotas:new Set()}; const rt=route(t);
      r.cliente=r.cliente||S(rt.cliente||t.cliente); r.origen=r.origen||S(rt.origen||t.origen); r.destino=r.destino||S(rt.destino||t.destino); r.fecha=r.fecha||startTime(t,{});
      const f=flota(t); if(f)r.flotas.add(f); by.set(n,r);
    });
    return [...by.values()].map(r=>({...r,flotas:[...r.flotas]})).sort((a,b)=>String(b.fecha||'').localeCompare(String(a.fecha||'')));
  }
  function dateIn(v,d,h){const dt=toDate(v); if(!dt)return true; if(d&&dt<new Date(d+'T00:00:00'))return false; if(h&&dt>new Date(h+'T23:59:59'))return false; return true;}
  function fillEmb(id,vals,all){
    const sel=$(id); if(!sel)return;
    const cur=sel.value;
    const list=[...new Set(vals.filter(Boolean).map(S))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
    sel.innerHTML=`<option value="">${esc(all)}</option>`+list.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
    if(list.includes(cur))sel.value=cur;
    sel.onchange=()=>window.renderEmbarquesV244(false);
  }
  window.renderEmbarquesV244=async function(force=false){
    await ensureData(force);
    const all=embRows();
    fillEmb('embFilterCliente',all.map(r=>r.cliente).concat(arr('clientes').map(x=>S(x.nombre||x.name||x.id||x))),'Todos los clientes');
    fillEmb('embFilterOrigen',all.map(r=>r.origen).concat(arr('origenes').map(x=>S(x.nombre||x.name||x.id||x))),'Todos los orígenes');
    fillEmb('embFilterDestino',all.map(r=>r.destino).concat(arr('destinos').map(x=>S(x.nombre||x.name||x.id||x))),'Todos los destinos');
    fillEmb('embCliente',arr('clientes').map(x=>S(x.nombre||x.name||x.id||x)),'Seleccionar cliente');
    fillEmb('embOrigen',arr('origenes').map(x=>S(x.nombre||x.name||x.id||x)),'Seleccionar origen');
    fillEmb('embDestino',arr('destinos').map(x=>S(x.nombre||x.name||x.id||x)),'Seleccionar destino');
    ['embFilterDesde','embFilterHasta'].forEach(id=>{const x=$(id); if(x)x.type='date';});
    const fc=N($('embFilterCliente')?.value), fo=N($('embFilterOrigen')?.value), fd=N($('embFilterDestino')?.value), de=$('embFilterDesde')?.value||'', ha=$('embFilterHasta')?.value||'';
    const rows=all.filter(r=>(!fc||N(r.cliente)===fc)&&(!fo||N(r.origen)===fo)&&(!fd||N(r.destino)===fd)&&dateIn(r.fecha,de,ha));
    const cg=cargoStore(), tbody=$('embarquesTbody');
    if(tbody)tbody.innerHTML=rows.map(e=>{const c=cg.filter(x=>String(x.embarque)===String(e.numero)).length; return `<tr data-embarque="${esc(e.numero)}"><td><b>Emb. ${esc(e.numero)}</b></td><td>${esc(e.cliente||'-')}</td><td>${esc(e.origen||'-')}</td><td>${esc(e.destino||'-')}</td><td>${esc(fmt(e.fecha,false))}</td><td>${esc(e.flotas.join(', ')||'-')}</td><td>${esc(c)}</td><td class="embarqueActionsCellV210"><div class="embarqueActionsV210"><button class="iconBtn edit" type="button" onclick="event.stopPropagation();window.openCargoFormV244&&window.openCargoFormV244('${esc(e.numero)}')">+ Carga</button><button class="deleteEmbarqueBtnV275" type="button" title="Eliminar embarque" onclick="event.stopPropagation();window.confirmDeleteEmbarqueV275&&window.confirmDeleteEmbarqueV275('${esc(e.numero)}')">🗑</button></div></td></tr>`;}).join('')||'<tr><td colspan="8" class="emptyRow">No hay embarques para mostrar.</td></tr>';
  };
  window.renderEmbarquesV244.__v210Clean=true;
  ...
  async function deleteByRef(ref){
    ...
    try{await ref.delete();return true;}catch(e){try{await ref.set({deleted:true,activo:false,deletedAt:new Date().toISOString()},{merge:true});return true;}catch(_e){}}
    return false;
  }
  async function deleteEmbarque(numero){
    numero=S(numero); if(!numero)return false;
    const dbc=window.db || (typeof db!=='undefined'?db:null);
    if(dbc&&typeof dbc.collection==='function'){
      const col=dbc.collection('embarque');
      try{const snap=await col.doc(numero).get(); if(snap&&snap.exists)await deleteByRef(snap.ref);}catch(e){}
      for(const f of ['numero','embarque','idEmbarque','codigo']){
        try{const qs=await col.where(f,'==',numero).get(); for(const d of qs.docs)await deleteByRef(d.ref);}catch(e){}
      }
    }
    try{
      ['elta_cargas_embarque_v245','elta_cargas_embarque_v244'].forEach(k=>{try{const a=JSON.parse(localStorage.getItem(k)||'[]')||[]; localStorage.setItem(k,JSON.stringify(a.filter(c=>S(c?.embarque)!==numero)));}catch(e){}});
      const d=deletedSet(); d.add(numero); localStorage.setItem('elta_deleted_embarques_v275',JSON.stringify([...d]));
    }catch(e){}
    if(Array.isArray(window.embarques))window.embarques=window.embarques.filter(e=>S(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId)!==numero);
    try{if(typeof embarques!=='undefined'&&Array.isArray(embarques))embarques=embarques.filter(e=>S(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId)!==numero);}catch(e){}
    await window.renderEmbarquesV244(false);
    return true;
  }
  window.confirmDeleteEmbarqueV275=function(numero){
    numero=S(numero); if(!numero)return;
    if(!confirm('¿Confirmar eliminación del embarque '+numero+'?'))return;
    deleteEmbarque(numero).then(()=>alert('Embarque '+numero+' eliminado correctamente.')).catch(e=>{console.error(e);alert('No se pudo eliminar el embarque.');});
  };

  // ---------------- TAB SIN RENDER HEREDADO PARA ESTOS DOS MODULOS ----------------
  const legacyTab210=window.tab;
  window.tab=function(id){
    if(id==='entrega'||id==='embarques'){
      document.querySelectorAll('.sideNav button').forEach(b=>b.classList.remove('active'));
      document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));
      [...document.querySelectorAll('.sideNav button')].find(b=>(b.dataset.menuId===id)||((b.getAttribute('onclick')||'').includes("'"+id+"'"))||((b.getAttribute('onclick')||'').includes('"'+id+'"')))?.classList.add('active');
      $(id)?.classList.add('active');
      setVersion210();
      if(id==='entrega')window.renderEntrega(false);
      if(id==='embarques')window.renderEmbarquesV244(false);
      return;
    }
    return typeof legacyTab210==='function'?legacyTab210.apply(this,arguments):undefined;
  };
  window.tab.__v210Clean=true;

  document.addEventListener('click',ev=>{
    const b=ev.target&&ev.target.closest?ev.target.closest('#entrega button,#embarques button'):null;
    if(!b)return;
    if(/actualizar/i.test(b.textContent||b.title||'')){
      ev.preventDefault(); ev.stopPropagation(); if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();
      const old=b.textContent; b.disabled=true; b.textContent='Actualizando...';
      const p=b.closest('#entrega')?window.renderEntrega(true):window.renderEmbarquesV244(true);
      Promise.resolve(p).finally(()=>{b.disabled=false;b.textContent=old||'Actualizar';});
      return false;
    }
  },true);

  document.addEventListener('DOMContentLoaded',()=>{setVersion210();ensureEntregaPanel();});
  window.addEventListener('load',()=>{setVersion210();ensureEntregaPanel();});
})();




/* ===== v3.3.21 - Forzado final de version visible ===== */
(function(){
  const V='3.3.21';
  window.ELTA_APP_VERSION=V;
  window.APP_VERSION_V2=V;
  ...
  document.addEventListener('DOMContentLoaded',()=>{forceVersion211();setTimeout(forceVersion211,300);setTimeout(forceVersion211,1000);});
  window.addEventListener('load',()=>{forceVersion211();setTimeout(forceVersion211,300);setTimeout(forceVersion211,1000);});
})();




/* ===== v3.3.21 - Confirmación eliminar + hitos Entregas por embarque =====
   Ajuste puntual:
...
(function(){
  ...
  function setVersion212(){
    try{
      document.title='ELTA ITS - Versión '+VERSION_212;
      document.querySelectorAll('span,small,p,div,footer').forEach(el=>{
        if(el.childElementCount===0 && /Versi[oó]n\s+\d+\.\d+\.\d+/i.test(el.textContent||'')){
          el.textContent=(el.textContent||'').replace(/Versi[oó]n\s+\d+\.\d+\.\d+/gi,'Versión '+VERSION_212);
        }
      });
    }catch(e){}
  }
  ...
  function elapsed(a,b){
    const d1=toDate(a),d2=toDate(b)||new Date();
    if(!d1)return '-';
    const m=Math.floor(Math.max(0,d2-d1)/60000), dd=Math.floor(m/1440), h=Math.floor((m%1440)/60), mm=m%60;
    return `${dd} d ${h} h ${mm} m`;
  }
  async function readCol212(name){
    try{
      const dbc=window.db || (typeof db!=='undefined'?db:null);
      if(dbc&&typeof dbc.collection==='function'){
        const snap=await dbc.collection(name).get();
    ...
    }catch(e){console.warn('v212 read',name,e);}
    return [];
  }
  async function ensureData212(force=false){
    const [emb,tr,ad]=await Promise.all([
      (force||!arr('embarques').length)?readCol212('embarque'):Promise.resolve(null),
      (force||!arr('trs').length)?readCol212('transitos'):Promise.resolve(null),
      (force||!arr('aduanas').length)?readCol212('aduana'):Promise.resolve(null)
    ]);
    if(Array.isArray(emb)){window.embarques=emb;try{embarques=emb;}catch(e){}}
    if(Array.isArray(tr)){window.trs=tr;try{trs=tr;}catch(e){}}
    if(Array.isArray(ad)){window.aduanas=ad;try{aduanas=ad;}catch(e){}}
  }

  // ---------- Embarques: modal de confirmación ----------
  function deleteModal212(numero){
    document.querySelectorAll('.deleteConfirmOverlayV212').forEach(x=>x.remove());
    const overlay=document.createElement('div');
    overlay.className='deleteConfirmOverlayV212';
    overlay.innerHTML=`<div class="deleteConfirmBoxV212">
      <h2>Eliminar embarque</h2>
      <p>¿Realmente desea eliminar este embarque?</p>
      <div class="deleteInfoV212"><b>Embarque:</b> ${esc(numero)}</div>
      <p class="deleteHintV212">Esta acción eliminará el embarque y actualizará el listado.</p>
      <div class="deleteActionsV212">
        <button type="button" class="cancelDeleteV212">Cancelar</button>
        <button type="button" class="confirmDeleteV212">Eliminar</button>
      </div>
    </div>`;
    overlay.querySelector('.cancelDeleteV212').onclick=()=>overlay.remove();
    overlay.addEventListener('click',ev=>{if(ev.target===overlay)overlay.remove();});
    overlay.querySelector('.confirmDeleteV212').onclick=async()=>{
      const btn=overlay.querySelector('.confirmDeleteV212');
      btn.disabled=true; btn.textContent='Eliminando...';
      try{
        if(typeof window.__deleteEmbarqueV210==='function'){
          await window.__deleteEmbarqueV210(numero);
        }else{
          // Usa la función vigente; si no existe, llama al flujo anterior después de cerrar modal.
          const old=window.confirmDeleteEmbarqueV275;
          overlay.remove();
          if(old&&old.__modalV212) return;
          if(typeof old==='function') old(numero);
          return;
        }
        overlay.remove();
        alert('Embarque '+numero+' eliminado correctamente.');
  ...
  // Guardar una referencia directa al borrado real si el bloque anterior lo dejó accesible solo adentro.
  // Si no existe, se crea una implementación equivalente.
  if(!window.__deleteEmbarqueV210){
    window.__deleteEmbarqueV210=async function(numero){
      numero=S(numero);
      const dbc=window.db || (typeof db!=='undefined'?db:null);
      async function delRef(ref){
        if(!ref)return false;
        try{const snap=await ref.collection('cargas').get(); for(const d of (snap.docs||[])) await d.ref.delete();}catch(e){}
        try{await ref.delete();return true;}catch(e){try{await ref.set({deleted:true,activo:false,deletedAt:new Date().toISOString()},{merge:true});return true;}catch(_e){}}
        return false;
      }
      if(dbc&&typeof dbc.collection==='function'){
        const col=dbc.collection('embarque');
        try{const snap=await col.doc(numero).get(); if(snap&&snap.exists)await delRef(snap.ref);}catch(e){}
        for(const f of ['numero','embarque','idEmbarque','codigo']){
          try{const qs=await col.where(f,'==',numero).get(); for(const d of (qs.docs||[])) await delRef(d.ref);}catch(e){}
        }
      }
      try{
        ['elta_cargas_embarque_v245','elta_cargas_embarque_v244'].forEach(k=>{
          try{const a=JSON.parse(localStorage.getItem(k)||'[]')||[]; localStorage.setItem(k,JSON.stringify(a.filter(c=>S(c?.embarque)!==numero)));}catch(e){}
        });
        let del=[]; try{del=JSON.parse(localStorage.getItem('elta_deleted_embarques_v275')||'[]')||[];}catch(e){}
        if(!del.map(S).includes(numero))del.push(numero);
        localStorage.setItem('elta_deleted_embarques_v275',JSON.stringify(del));
      }catch(e){}
      if(Array.isArray(window.embarques))window.embarques=window.embarques.filter(e=>S(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId)!==numero);
      try{if(typeof embarques!=='undefined'&&Array.isArray(embarques))embarques=embarques.filter(e=>S(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId)!==numero);}catch(e){}
      try{if(typeof window.renderEmbarquesV244==='function')await window.renderEmbarquesV244(false);}catch(e){}
      return true;
    };
  }
  ...
  window.confirmDeleteEmbarqueV275=function(numero){
    numero=S(numero);
    if(!numero)return;
    deleteModal212(numero);
  };
  window.confirmDeleteEmbarqueV275.__modalV212=true;

  // ---------- Entregas: render final con join correcto por embarque ----------
  function embNum(e){return S(get(e,['numero','embarque','idEmbarque','codigo'])||e?.id||e?._docId);}
  function flota(t){return S(get(t,['flota','fleet','usuario','user'])||get(t?.user,['fleet','flota']));}
  function route(t){return (t&&(t.route||t.ruta||t.recorrido))||{};}
  function country(v){
  ...
  function flagCls(v){const c=country(v);return c==='argentina'?'ar':c==='chile'?'cl':c==='paraguay'?'py':c==='uruguay'?'uy':c==='brasil'?'br':c==='tdf'?'tdf':'xx';}
  function flagHtml(v){return `<span class="countryFlag flag-${esc(flagCls(v))}" title="${esc(code(v))}" aria-label="${esc(code(v))}"></span>`;}
  function abbr(v){const n=N(v),o=S(v); if(/^clz\b/.test(n)||n.includes('centro logistico zarate'))return 'CLZ'; if(/^clc\b/.test(n)||n.includes('centro logistico cordoba'))return 'CLC'; if(n.includes('puerto zarate'))return 'TZ - PUERTO ZARATE'; return o;}
  function startTime(t,e){return get(t,['start','inicio','fechaInicio','createdAt'])||get(e,['fecha','fechaHora','createdAt']);}
  function latestTime(t){
    const vals=[],add=v=>{if(hasDate(v))vals.push(v);};
    add(get(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']));
  ...
  function closeTime(t,e){return get(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn'])||get(e,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn'])||latestTime(t);}
  function status(t,e){const s=N([get(t,['estado','status']),get(e,['estado','status'])].join(' ')); if(s.includes('final')||s.includes('cerr')||t?.closed||t?.cierre)return {txt:'CERRADO',cls:'done'}; if(s.includes('aduana'))return {txt:'EN ADUANA',cls:'customs'}; if(t&&Object.keys(t).length)return {txt:'EN TRÁNSITO',cls:'open'}; return {txt:'PLANIFICADO',cls:'planned'};}
  function records(t){
    const out=[]; const push=r=>{if(!r||typeof r!=='object')return; const name=S(get(r,['hito','nombre','name','label','title','aduana','aduanaId','checkpoint','puesto','evento','tipo','estado','level','localidad','ciudad'])); const time=get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora']); if(name&&hasDate(time)&&!/^(updates?|gps|registros?|logs?|tracking|posiciones?)$/i.test(name))out.push({name,time});};
    ['aduanas','aduana','registros','hitos','eventos','novedades','checkpoints','updates'].forEach(k=>{const v=t&&t[k]; if(Array.isArray(v))v.forEach(push); else if(v&&typeof v==='object')Object.values(v).forEach(push);});
    const seen=new Set(); return out.sort((a,b)=>(toDate(a.time)?.getTime()||0)-(toDate(b.time)?.getTime()||0)).filter(r=>{const k=N(r.name)+'|'+(toDate(r.time)?.getTime()||''); if(seen.has(k))return false; seen.add(k); return true;});
  }
  function aduanaNames(destino,origen){
    const c=country(destino);
    const docs=arr('aduanas');
    const mapDest=d=>{const s=N(d); if(s==='chille')return 'chile'; if(s==='brazil')return 'brasil'; return s;};
    const list=docs.filter(a=>a&&a.activo!==false&&(mapDest(a.destino)===c||country(a.destino)===c))
      ...
      .sort((a,b)=>a.orden-b.orden||N(a.name).localeCompare(N(b.name))).map(x=>x.name);
    const uniq=[]; list.forEach(h=>{if(!uniq.some(x=>N(x)===N(h)))uniq.push(h);});
    if(uniq.length)return [origen,...uniq,destino];
    if(c==='chile')return [origen,'Uspallata','Los Andes',destino];
    if(c==='paraguay')return [origen,'Clorinda','Puerto José Falcón','Depósito Fiscal Paraguay',destino];
    if(c==='uruguay')return [origen,'Gualeguaychú','Fray Bentos',destino];
    return [origen,destino];
  }
  function match(a,b){a=N(a);b=N(b);return a&&b&&(a===b||a.includes(b)||b.includes(a));}

  function entregaRows212(){
    const embByNum=new Map();
    arr('embarques').forEach(e=>{const n=embNum(e); if(n)embByNum.set(n,e);});

    const rows=[];
    arr('trs').forEach(t=>{
      const f=flota(t), n=S(t&&t.embarque), e=embByNum.get(n)||{};
      const rt=route(t);
      const origen=S(get(e,['origen','origin'])) || S(rt.origen||t.origen) || 'Origen';
      const destino=S(get(e,['destino','destination'])) || S(rt.destino||t.destino) || 'Cierre';
      rows.push({
        flota:f,
        embarques:n?[n]:[],
        numero:n,
        t,e,
        cliente:S(get(e,['cliente','client']))||S(rt.cliente||t.cliente),
        origen,destino,
        fecha:startTime(t,e)
      });
    });

    // Embarques sin tránsito
    arr('embarques').forEach(e=>{
      const n=embNum(e);
      if(!n || rows.some(r=>(r.embarques||[]).includes(n)))return;
      rows.push({
        flota:S(get(e,['flota','fleet','unidad','usuario'])||get(e?.user,['fleet'])),
        embarques:n?[n]:[],
        numero:n,
        t:{},e,
        cliente:S(get(e,['cliente','client'])),
        origen:S(get(e,['origen','origin']))||'Origen',
        destino:S(get(e,['destino','destination']))||'Cierre',
        fecha:get(e,['fecha','fechaHora','createdAt'])
      });
    });
    return rows.sort((a,b)=>String(a.flota).localeCompare(String(b.flota),'es',{numeric:true}));
  }
  function buildStages(row){
    const names=aduanaNames(row.destino,row.origen);
    const st=names.map((name,i)=>({name,time:i===0?startTime(row.t,row.e):(i===names.length-1?closeTime(row.t,row.e):''),kind:i===0?'start':(i===names.length-1?'end':'aduana')}));
    records(row.t).forEach(r=>{const idx=st.findIndex((s,i)=>i>0&&i<st.length-1&&match(r.name,s.name)); if(idx>=0&&!hasDate(st[idx].time))st[idx].time=r.time;});
    return st;
  }
  function card212(row){
    const st=buildStages(row), stt=status(row.t,row.e);
    let idx=0; st.forEach((s,i)=>{if(hasDate(s.time))idx=i;});
    if(stt.cls==='done')idx=st.length-1;
    const max=Math.max(1,st.length-1), pct=idx/max*100;
    const upd=fmt(stt.cls==='done'?closeTime(row.t,row.e):(latestTime(row.t)||row.fecha),true);
  ...
  const oldRenderEntrega212=window.renderEntrega;
  window.renderEntrega=async function(force=false){
    if(typeof window.ensureEntregaPanelStable==='function')try{window.ensureEntregaPanelStable();}catch(e){}
    await ensureData212(force);
    let rows=entregaRows212();
    ...
    fill('entregaFiltroEmbarque',rows.flatMap(r=>r.embarques||[]),'Embarque');
    fill('entregaFiltroFlota',rows.map(r=>r.flota),'Todas las flotas');
    fill('entregaFiltroCliente',rows.map(r=>r.cliente),'Todos los clientes');
    fill('entregaFiltroDestino',rows.map(r=>r.destino),'Todos los destinos');
    const fe=N($('entregaFiltroEmbarque')?.value), ff=N($('entregaFiltroFlota')?.value), fc=N($('entregaFiltroCliente')?.value), fd=N($('entregaFiltroDestino')?.value), fs=N($('entregaFiltroEstado')?.value);
    if($('entregaFiltroEstado'))$('entregaFiltroEstado').onchange=()=>window.renderEntrega(false);
    rows=rows.filter(r=>(!fe||(r.embarques||[]).some(e=>N(e)===fe))&&(!ff||N(r.flota)===ff)&&(!fc||N(r.cliente)===fc)&&(!fd||N(r.destino)===fd)&&(!fs||(fs==='cerrado'?status(r.t,r.e).cls==='done':status(r.t,r.e).cls!=='done')));
    const box=$('entregaCards');
    if(box)box.innerHTML=rows.length?rows.map(card212).join(''):'<div class="glassPanel entregaEmpty">No hay flotas para el filtro seleccionado.</div>';
  };
  window.renderEntrega.__v212Join=true;

  const oldTab212=window.tab;
  window.tab=function(id){
    if(id==='entrega'){
      document.querySelectorAll('.sideNav button').forEach(b=>b.classList.remove('active'));
      document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));
    ...
    return typeof oldTab212==='function'?oldTab212.apply(this,arguments):undefined;
  };
  window.tab.__v212Join=true;

  document.addEventListener('DOMContentLoaded',()=>{setVersion212();});
  window.addEventListener('load',()=>{setVersion212();});
})();




/* ===== v3.3.21 - Ajustes Embarques + Entregas =====
...
(function(){
  ...
  function setVersion213(){
    try{
      document.title='ELTA ITS - Versión '+VERSION_213;
      document.querySelectorAll('span,small,p,div,footer').forEach(el=>{
        if(el.childElementCount===0 && /Versi[oó]n\s+\d+\.\d+\.\d+/i.test(el.textContent||'')){
          el.textContent=(el.textContent||'').replace(/Versi[oó]n\s+\d+\.\d+\.\d+/gi,'Versión '+VERSION_213);
        }
      });
    }catch(e){}
  }

  async function readCol213(name){
    try{
      const dbc=window.db || (typeof db!=='undefined'?db:null);
      if(dbc&&typeof dbc.collection==='function'){
        const snap=await dbc.collection(name).get();
    ...
    }catch(e){console.warn('v213 read',name,e);}
    return [];
  }
  async function ensureData213(force=false){
    const [emb,tr,ad]=await Promise.all([
      (force||!arr('embarques').length)?readCol213('embarque'):Promise.resolve(null),
      (force||!arr('trs').length)?readCol213('transitos'):Promise.resolve(null),
      (force||!arr('aduanas').length)?readCol213('aduana'):Promise.resolve(null)
    ]);
    if(Array.isArray(emb)){window.embarques=emb;try{embarques=emb;}catch(e){}}
    if(Array.isArray(tr)){window.trs=tr;try{trs=tr;}catch(e){}}
    if(Array.isArray(ad)){window.aduanas=ad;try{aduanas=ad;}catch(e){}}
  ...
  function toDate(v){
    try{ if(!v)return null; if(v&&typeof v.toDate==='function')v=v.toDate(); const d=new Date(v); return isNaN(d)?null:d; }catch(e){return null;}
  }
  function hasDate(v){return !!toDate(v);}
  function fmt(v,year=false){
    const d=toDate(v);
  ...
  /* -------- Modal eliminar embarque compacto -------- */
  function deleteModal213(numero){
    document.querySelectorAll('.deleteConfirmOverlayV212,.deleteConfirmOverlayV213').forEach(x=>x.remove());
    const overlay=document.createElement('div');
    overlay.className='deleteConfirmOverlayV213';
    overlay.innerHTML=`<div class="deleteConfirmBoxV213">
      <h2>Eliminar embarque</h2>
      <p>¿Realmente desea eliminar este embarque?</p>
      <div class="deleteInfoV213"><b>Embarque:</b> ${esc(numero)}</div>
      <p class="deleteHintV213">Esta acción eliminará el embarque y actualizará el listado.</p>
      <div class="deleteActionsV213">
        <button type="button" class="cancelDeleteV213">Cancelar</button>
        <button type="button" class="confirmDeleteV213">Eliminar</button>
      </div>
    </div>`;
    overlay.querySelector('.cancelDeleteV213').onclick=()=>overlay.remove();
    overlay.addEventListener('click',ev=>{if(ev.target===overlay)overlay.remove();});
    overlay.querySelector('.confirmDeleteV213').onclick=async()=>{
      const btn=overlay.querySelector('.confirmDeleteV213');
      btn.disabled=true; btn.textContent='Eliminando...';
      try{
        if(typeof window.__deleteEmbarqueV210==='function') await window.__deleteEmbarqueV210(numero);
        else {
          let old=window.confirmDeleteEmbarqueV275;
          overlay.remove();
          if(old && !old.__modalV213) old(numero);
          return;
        }
        overlay.remove();
        alert('Embarque '+numero+' eliminado correctamente.');
  ...
  window.confirmDeleteEmbarqueV275=function(numero){
    numero=S(numero); if(!numero)return;
    deleteModal213(numero);
  };
  window.confirmDeleteEmbarqueV275.__modalV213=true;

  /* -------- Embarques: flota desde cargas -------- */
  function cargoStore213(){
    let out=[];
    ['elta_cargas_embarque_v245','elta_cargas_embarque_v244'].forEach(k=>{
      try{const a=JSON.parse(localStorage.getItem(k)||'[]'); if(Array.isArray(a))out=out.concat(a);}catch(e){}
    });
    return out;
  }
  const oldRenderEmb213=window.renderEmbarquesV244;
  if(typeof oldRenderEmb213==='function' && !oldRenderEmb213.__flotasCargas213){
    window.renderEmbarquesV244=async function(){
      const r=await oldRenderEmb213.apply(this,arguments);
      try{
        const cargas=cargoStore213();
        const tbody=$('embarquesTbody');
        if(tbody){
          [...tbody.querySelectorAll('tr')].forEach(tr=>{
            if(tr.querySelector('.emptyRow'))return;
            const first=(tr.querySelector('td')?.textContent||'').trim();
            const m=first.match(/^Emb\.\s*([A-Za-z0-9_-]+)/i);
            if(!m)return;
            const numero=m[1];
            const flotas=[...new Set(cargas.filter(c=>S(c.embarque)===numero).map(c=>S(c.flota)).filter(Boolean))];
            const tds=tr.querySelectorAll('td');
            if(flotas.length && tds[5] && (!S(tds[5].textContent) || S(tds[5].textContent)==='-')){
              tds[5].textContent=flotas.join(', ');
            }
          });
        }
      }catch(e){console.warn('v213 flotas cargas',e);}
      setVersion213();
      return r;
    };
    window.renderEmbarquesV244.__flotasCargas213=true;
  }

  /* -------- Entregas: render correcto, programados y anti-render heredado -------- */
  function embNum(e){return S(get(e,['numero','embarque','idEmbarque','codigo'])||e?.id||e?._docId);}
  function flota(t){return S(get(t,['flota','fleet','usuario','user'])||get(t?.user,['fleet','flota']));}
  function route(t){return (t&&(t.route||t.ruta||t.recorrido))||{};}
  function country(v){
    ...
    if(/uruguay|montevideo|fray bentos|sevel uruguay|\buy\b/.test(s))return 'uruguay';
    if(/brasil|brazil|curitiba|sao paulo|\bbr\b/.test(s))return 'brasil';
    if(/argentina|zarate|zárate|cordoba|córdoba|clz|clc|tz|puerto zarate|\bar\b|arg/.test(s))return 'argentina';
    return '';
  }
  function code(v){const c=country(v);return c==='argentina'?'AR':c==='chile'?'CL':c==='paraguay'?'PY':c==='uruguay'?'UY':c==='brasil'?'BR':c==='tdf'?'TDF':'--';}
  function flagCls(v){const c=country(v);return c==='argentina'?'ar':c==='chile'?'cl':c==='paraguay'?'py':c==='uruguay'?'uy':c==='brasil'?'br':c==='tdf'?'tdf':'xx';}
  function flagHtml(v){return `<span class="countryFlag flag-${esc(flagCls(v))}" title="${esc(code(v))}" aria-label="${esc(code(v))}"></span>`;}
  function abbr(v){const n=N(v),o=S(v); if(/^clz\b/.test(n)||n.includes('centro logistico zarate'))return 'CLZ'; if(/^clc\b/.test(n)||n.includes('centro logistico cordoba'))return 'CLC'; if(n.includes('puerto zarate')||/^tz\b/.test(n))return 'TZ - PUERTO ZARATE'; return o;}
  function startTime(t,e){return get(t,['start','inicio','fechaInicio','createdAt'])||get(e,['fecha','fechaHora','createdAt']);}
  function latestTime(t){
    const vals=[],add=v=>{if(hasDate(v))vals.push(v);};
    add(get(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']));
  ...
  function status(t,e){
    const s=N([get(t,['estado','status']),get(e,['estado','status'])].join(' '));
    if(s.includes('final')||s.includes('cerr')||t?.closed||t?.cierre)return {txt:'CERRADO',cls:'done',filter:'cerrado'};
    if(t&&Object.keys(t).length)return {txt:'EN TRÁNSITO',cls:'open',filter:'en transito'};
    return {txt:'PROGRAMADO',cls:'planned',filter:'programado'};
  }
  function records(t){
    const out=[]; const push=r=>{if(!r||typeof r!=='object')return; const name=S(get(r,['hito','nombre','name','label','title','aduana','aduanaId','checkpoint','puesto','evento','tipo','estado','level','localidad','ciudad'])); const time=get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora']); if(name&&hasDate(time)&&!/^(updates?|gps|registros?|logs?|tracking|posiciones?)$/i.test(name))out.push({name,time});};
    ['aduanas','aduana','registros','hitos','eventos','novedades','checkpoints','updates'].forEach(k=>{const v=t&&t[k]; if(Array.isArray(v))v.forEach(push); else if(v&&typeof v==='object')Object.values(v).forEach(push);});
  ...
  function entregaRows213(){
    ...
    arr('trs').forEach(t=>{
      ...
      const origen=S(get(e,['origen','origin']))||S(rt.origen||t.origen)||'Origen';
      const destino=S(get(e,['destino','destination']))||S(rt.destino||t.destino)||'Cierre';
      rows.push({flota:f,embarques:n?[n]:[],numero:n,t,e,cliente:S(get(e,['cliente','client']))||S(rt.cliente||t.cliente),origen,destino,fecha:startTime(t,e)});
    });
    arr('embarques').forEach(e=>{
      const n=embNum(e); if(!n || rows.some(r=>(r.embarques||[]).includes(n)))return;
      rows.push({flota:S(get(e,['flota','fleet','unidad','usuario'])||get(e?.user,['fleet'])),embarques:n?[n]:[],numero:n,t:{},e,cliente:S(get(e,['cliente','client'])),origen:S(get(e,['origen','origin']))||'Origen',destino:S(get(e,['destino','destination']))||'Cierre',fecha:get(e,['fecha','fechaHora','createdAt'])});
    });
    return rows.sort((a,b)=>String(a.flota).localeCompare(String(b.flota),'es',{numeric:true}));
  }
  function stages213(row){
    const names=aduanaNames(row.destino,row.origen);
    const st=names.map((name,i)=>({name,time:i===0?startTime(row.t,row.e):(i===names.length-1?closeTime(row.t,row.e):''),kind:i===0?'start':(i===names.length-1?'end':'aduana')}));
    records(row.t).forEach(r=>{const idx=st.findIndex((s,i)=>i>0&&i<st.length-1&&match(r.name,s.name)); if(idx>=0&&!hasDate(st[idx].time))st[idx].time=r.time;});
    return st;
  }
  function card213(row){
    const st=stages213(row), stt=status(row.t,row.e);
    let idx=0; st.forEach((s,i)=>{if(hasDate(s.time))idx=i;});
    if(stt.cls==='done')idx=st.length-1;
    const max=Math.max(1,st.length-1), pct=idx/max*100;
    const upd=fmt(stt.cls==='done'?closeTime(row.t,row.e):(latestTime(row.t)||row.fecha),true);
  ...
  async function renderEntrega213(force=false){
    if(typeof window.ensureEntregaPanelStable==='function')try{window.ensureEntregaPanelStable();}catch(e){}
    await ensureData213(force);
    const selEstado=$('entregaFiltroEstado');
    if(selEstado && ![...selEstado.options].some(o=>o.value==='programado')){
      selEstado.insertAdjacentHTML('beforeend','<option value="programado">Programados</option>');
    }
    let rows=entregaRows213();
    function fill(id,vals,all){
      const sel=$(id); if(!sel)return;
      const cur=sel.value;
      const list=[...new Set(vals.filter(Boolean).map(S))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
      sel.innerHTML=`<option value="">${esc(all)}</option>`+list.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
      if(list.includes(cur))sel.value=cur;
      sel.onchange=()=>renderEntrega213(false);
    }
    fill('entregaFiltroEmbarque',rows.flatMap(r=>r.embarques||[]),'Embarque');
    fill('entregaFiltroFlota',rows.map(r=>r.flota),'Todas las flotas');
    fill('entregaFiltroCliente',rows.map(r=>r.cliente),'Todos los clientes');
    fill('entregaFiltroDestino',rows.map(r=>r.destino),'Todos los destinos');
    if(selEstado) selEstado.onchange=()=>renderEntrega213(false);
    const fe=N($('entregaFiltroEmbarque')?.value), ff=N($('entregaFiltroFlota')?.value), fc=N($('entregaFiltroCliente')?.value), fd=N($('entregaFiltroDestino')?.value), fs=N($('entregaFiltroEstado')?.value);
    rows=rows.filter(r=>{
      const st=status(r.t,r.e);
      return (!fe||(r.embarques||[]).some(e=>N(e)===fe))&&(!ff||N(r.flota)===ff)&&(!fc||N(r.cliente)===fc)&&(!fd||N(r.destino)===fd)&&(!fs||st.filter===fs);
    });
    const box=$('entregaCards');
    if(box) box.innerHTML=rows.length?rows.map(card213).join(''):'<div class="glassPanel entregaEmpty">No hay flotas para el filtro seleccionado.</div>';
    setVersion213();
  }

  window.renderEntrega=renderEntrega213;
  window.renderEntrega.__v213Final=true;

  function needsEntregaFix213(){
    const box=$('entregaCards');
    if(!box) return false;
    const txt=N(box.textContent||'');
    return /\binicio\b/.test(txt) || (/\bcierre\b/.test(txt) && !/(clz|clc|uspallata|los andes|clorinda|fray bentos|puerto jose falcon|dep[oó]sito fiscal)/.test(txt));
  }
  let fixing213=false;
  function scheduleEntregaFix213(force=false){
    if(fixing213) return;
    fixing213=true;
    setTimeout(async()=>{
      try{
        if(force || needsEntregaFix213()) await renderEntrega213(false);
      }catch(e){console.warn('v213 entrega fix',e);}
      finally{fixing213=false;}
    },80);
  }

  const oldTab213=window.tab;
  window.tab=function(id){
    if(id==='entrega'){
      const r=oldTab213&&oldTab213.apply(this,arguments);
      renderEntrega213(false);
      [120,350,900].forEach(ms=>setTimeout(()=>scheduleEntregaFix213(true),ms));
      return r;
    }
    return oldTab213&&oldTab213.apply(this,arguments);
  };
  window.tab.__v213Final=true;

  function installEntregaObserver213(){
    const box=$('entregaCards');
    if(!box || box.__obs213) return;
    box.__obs213=new MutationObserver(()=>scheduleEntregaFix213(false));
    box.__obs213.observe(box,{childList:true,subtree:true,characterData:true});
  }
  document.addEventListener('DOMContentLoaded',()=>{setVersion213();setTimeout(installEntregaObserver213,500);});
  window.addEventListener('load',()=>{setVersion213();setTimeout(installEntregaObserver213,500);});
})();




/* ===== v3.3.21 - Proyecto limpio deploy =====
...
(function(){
  ...
  function ensureProgramados214(){
    const sel=$('entregaFiltroEstado');
    if(sel && ![...sel.options].some(o=>o.value==='programado')){
      sel.insertAdjacentHTML('beforeend','<option value="programado">Programados</option>');
    }
  }
  function cargoStore214(){
    let out=[];
    ['elta_cargas_embarque_v245','elta_cargas_embarque_v244'].forEach(k=>{
      try{const a=JSON.parse(localStorage.getItem(k)||'[]'); if(Array.isArray(a))out=out.concat(a);}catch(e){}
    });
  ...
  function fillFlotasEmbarques214(){
    try{
      const cargas=cargoStore214();
      const tbody=$('embarquesTbody');
      if(!tbody)return;
      [...tbody.querySelectorAll('tr')].forEach(tr=>{
        if(tr.querySelector('.emptyRow'))return;
  ...
  function needsEntregaFix214(){
    const box=$('entregaCards');
    if(!box)return false;
    const txt=N(box.textContent||'');
    return /\binicio\b/.test(txt) || (/\bcierre\b/.test(txt) && !/(clz|clc|tz|uspallata|los andes|clorinda|fray bentos|puerto jose falcon|dep[oó]sito fiscal)/.test(txt));
  }
  let fixing=false;
  function repairEntrega214(force=false){
    if(fixing)return;
    fixing=true;
    setTimeout(async()=>{
      try{
        ensureProgramados214();
        if((force||needsEntregaFix214()) && typeof window.renderEntrega==='function'){
          await window.renderEntrega(false);
        }
  ...
  const oldEmb=window.renderEmbarquesV244;
  if(typeof oldEmb==='function' && !oldEmb.__v214Deploy){
    window.renderEmbarquesV244=async function(){
  ...
  if(typeof oldTab==='function' && !oldTab.__v214Deploy){
    window.tab=function(id){
      ...
      if(id==='embarques'){
        [120,350].forEach(ms=>setTimeout(fillFlotasEmbarques214,ms));
      }
      return r;
    };
    window.tab.__v214Deploy=true;
  }
  function installObs(){
    const box=$('entregaCards');
    if(box && !box.__obs214){
      box.__obs214=new MutationObserver(()=>repairEntrega214(false));
...
/* ===== v3.3.21 - Corrección final línea de tiempo Entregas =====
   ...
   - Construye hitos desde origen + aduanas por destino + destino.
   - Programado queda como estaba.
...
(function(){
  ...
  const N=v=>S(v).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const get=(o,ks)=>{for(const k of ks){const v=o&&o[k]; if(v!==undefined&&v!==null&&String(v).trim()!==''&&String(v)!=='[object Object]')return v;} return '';};
  const arr=name=>{try{if(Array.isArray(window[name]))return window[name]; if(Array.isArray(globalThis[name]))return globalThis[name];}catch(e){} return [];};
  ...
  function toDate215(v){
    try{
      ...
      if(typeof v==='object'){
        const sec = v.seconds ?? v._seconds ?? v.s ?? v.epochSeconds;
        if(sec!==undefined){
          const ms=Number(sec)*1000 + Math.floor(Number(v.nanoseconds ?? v._nanoseconds ?? 0)/1e6);
          const d=new Date(ms);
          return isNaN(d)?null:d;
        }
        if(v.date) return toDate215(v.date);
        if(v.fecha) return toDate215(v.fecha);
        if(v.value) return toDate215(v.value);
        return null;
      }
      const txt=String(v).trim();
      if(!txt || txt==='[object Object]' || txt==='-') return null;
      const d=new Date(txt);
      return isNaN(d)?null:d;
    }catch(e){return null;}
  }
  function hasDate215(v){return !!toDate215(v);}
  function fmt215(v,year=false){
    const d=toDate215(v);
    if(!d) return '-';
    const opt=year?{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}:{day:'numeric',month:'numeric',hour:'2-digit',minute:'2-digit'};
    return d.toLocaleString('es-AR',opt);
  }
  function elapsed215(a,b){
    const d1=toDate215(a), d2=toDate215(b)||new Date();
    if(!d1)return '-';
    const m=Math.floor(Math.max(0,d2-d1)/60000), dd=Math.floor(m/1440), h=Math.floor((m%1440)/60), mm=m%60;
    return `${dd} d ${h} h ${mm} m`;
  }

  async function readCol215(name){
    try{
      const dbc=window.db || (typeof db!=='undefined'?db:null);
      if(dbc&&typeof dbc.collection==='function'){
        const snap=await dbc.collection(name).get();
    ...
    }catch(e){console.warn('v215 read',name,e);}
    return [];
  }
  async function ensureData215(force=false){
    const [emb,tr,ad]=await Promise.all([
      (force||!arr('embarques').length)?readCol215('embarque'):Promise.resolve(null),
      (force||!arr('trs').length)?readCol215('transitos'):Promise.resolve(null),
      (force||!arr('aduanas').length)?readCol215('aduana'):Promise.resolve(null)
    ]);
    if(Array.isArray(emb)){window.embarques=emb;try{embarques=emb;}catch(e){}}
    if(Array.isArray(tr)){window.trs=tr;try{trs=tr;}catch(e){}}
    if(Array.isArray(ad)){window.aduanas=ad;try{aduanas=ad;}catch(e){}}
  ...
  function embNum(e){return S(get(e,['numero','embarque','idEmbarque','codigo'])||e?.id||e?._docId);}
  function flota(t){return S(get(t,['flota','fleet','unidad','usuario','user'])||get(t?.user,['fleet','flota']));}
  function route(t){return (t&&(t.route||t.ruta||t.recorrido))||{};}
  ...
  function abbr(v){
    ...
    if(/^clz\b/.test(n)||n.includes('centro logistico zarate'))return 'CLZ';
    if(/^clc\b/.test(n)||n.includes('centro logistico cordoba'))return 'CLC';
    if(n.includes('puerto zarate')||/^tz\b/.test(n))return 'TZ - PUERTO ZARATE';
    if(n.includes('rio grande'))return 'RIO GRANDE - TIERRA DEL FUEGO';
    return o;
  }
  function startTime(t,e){
    return get(t,['start','inicio','fechaInicio','createdAt','fechaSalida','salida']) ||
           get(e,['fecha','fechaHora','createdAt','inicio','fechaInicio']);
  }
  function closeTime(t,e){
    return get(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin']) ||
           get(e,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin']);
  }
  function latestTime(t){
    const vals=[], add=v=>{const d=toDate215(v); if(d)vals.push(d);};
    add(get(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']));
    ['updates','gps','posiciones','registros','aduanas','hitos','eventos','novedades','checkpoints','aduana_registros','aduanaRegistros'].forEach(k=>{
      const v=t&&t[k], a=Array.isArray(v)?v:(v&&typeof v==='object'?Object.values(v):[]);
      a.forEach(r=>add(get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora','ingreso','liberacion'])));
    });
    vals.sort((a,b)=>b-a);
    return vals[0]||'';
  }
  function status215(t,e){
    const s=N([get(t,['estado','status']),get(e,['estado','status'])].join(' '));
    if(s.includes('final')||s.includes('cerr')||t?.closed||t?.cierre||t?.fechaCierre)return {txt:'CERRADO',cls:'done',filter:'cerrado'};
    if(t&&Object.keys(t).length)return {txt:'EN TRÁNSITO',cls:'open',filter:'en transito'};
    return {txt:'PROGRAMADO',cls:'planned',filter:'programado'};
  }

  function regName(r){
    return S(get(r,['hito','nombre','name','label','title','aduana','aduanaId','checkpoint','puesto','evento','tipo','estado','level','localidad','ciudad']));
  }
  function regTime(r){
    return get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora','ingreso','liberacion','fechaIngreso','fechaLiberacion']);
  }
  function records215(t){
    const out=[], push=r=>{
      if(!r||typeof r!=='object')return;
      const name=regName(r), time=regTime(r);
      if(name && toDate215(time) && !/^(updates?|gps|registros?|logs?|tracking|posiciones?)$/i.test(name)) out.push({name,time});
    };
    ['aduanas','aduana','registros','hitos','eventos','novedades','checkpoints','updates','aduana_registros','aduanaRegistros'].forEach(k=>{
      const v=t&&t[k];
      if(Array.isArray(v))v.forEach(push);
      else if(v&&typeof v==='object')Object.values(v).forEach(push);
    });
    const seen=new Set();
    return out.sort((a,b)=>(toDate215(a.time)?.getTime()||0)-(toDate215(b.time)?.getTime()||0)).filter(r=>{
      const k=N(r.name)+'|'+(toDate215(r.time)?.getTime()||'');
      if(seen.has(k))return false; seen.add(k); return true;
    });
  }

  function aduanaNames215(destino,origen){
    const c=country(destino);
    const docs=arr('aduanas');
    const mapDest=d=>{const s=N(d); if(s==='chille')return 'chile'; if(s==='brazil')return 'brasil'; return s;};
    const list=docs.filter(a=>a&&a.activo!==false&&(mapDest(a.destino)===c||country(a.destino)===c))
    ...
    if(c==='paraguay')return [origen,'Clorinda','Puerto José Falcón','Depósito Fiscal Paraguay',destino];
    if(c==='uruguay')return [origen,'Gualeguaychú','Fray Bentos',destino];
    if(c==='tdf')return [origen,'Rio Grande - Tierra del Fuego',destino];
    return [origen,destino];
  }
  function match215(a,b){a=N(a);b=N(b);return a&&b&&(a===b||a.includes(b)||b.includes(a));}

  function entregaRows215(){
    const embByNum=new Map();
    arr('embarques').forEach(e=>{const n=embNum(e); if(n)embByNum.set(n,e);});
    const rows=[];
    arr('trs').forEach(t=>{
      ...
      const destino=S(get(e,['destino','destination']))||S(rt.destino||t.destino)||'Cierre';
      rows.push({
        flota:f, embarques:n?[n]:[], numero:n, t,e,
        cliente:S(get(e,['cliente','client']))||S(rt.cliente||t.cliente),
        origen,destino,fecha:startTime(t,e)
      });
    });
    arr('embarques').forEach(e=>{
      const n=embNum(e);
  ...
  function buildStages215(row){
    const stt=status215(row.t,row.e);
    const names=aduanaNames215(row.destino,row.origen);
    const recs=records215(row.t);
    const st=names.map((name,i)=>{
      let time='';
      if(i===0) time=startTime(row.t,row.e);
      else if(i===names.length-1) time=(stt.cls==='done'?closeTime(row.t,row.e):'');
      return {name,time,kind:i===0?'start':(i===names.length-1?'end':'aduana')};
    });
    recs.forEach(r=>{
      const idx=st.findIndex((s,i)=>i>0&&i<st.length-1&&match215(r.name,s.name));
      if(idx>=0&&!hasDate215(st[idx].time)) st[idx].time=r.time;
    });
    return st;
  }

  function card215(row){
    const st=buildStages215(row), stt=status215(row.t,row.e);
    let idx=0;
    st.forEach((s,i)=>{ if(hasDate215(s.time) && (i<st.length-1 || stt.cls==='done')) idx=i; });
    if(stt.cls==='done')idx=st.length-1;
    if(stt.cls==='planned')idx=0;
    const max=Math.max(1,st.length-1), pct=idx/max*100;
    const updateRaw=stt.cls==='done' ? closeTime(row.t,row.e) : (latestTime(row.t)||startTime(row.t,row.e)||row.fecha);
    const upd=fmt215(updateRaw,true);
    const start=st[0]?.time||row.fecha, end=stt.cls==='done'?closeTime(row.t,row.e):null;
    const next=stt.cls==='done'?'Tránsito finalizado':(st.find((s,i)=>i>idx)?.name||st[idx]?.name||'-');
    return `<article class="entregaCard ${stt.cls==='done'?'closedTransit':''}">
      <div class="entregaTop">
        <div class="entregaFleet"><span class="entregaFleetIcon">🚚</span><div><h3>Flota ${esc(row.flota||'-')}</h3><span class="entregaStatus ${stt.cls}">${esc(stt.txt)}</span><small>Emb. ${esc((row.embarques||[]).join(', ')||row.numero||'-')}</small></div></div>
        <div class="entregaMeta"><span><i>🗓️</i> Última act.</span><b>${esc(upd)}</b></div>
        <div class="entregaMeta"><span><i>⏱️</i> Tiempo transcurrido</span><b>${esc(elapsed215(start,end))}</b></div>
        <div class="entregaMeta"><span><i>🚩</i> Próximo hito</span><b>${esc(abbr(next))}</b></div>
        <div class="entregaMeta"><span><i>📡</i> Estado</span><b class="state ${stt.cls}">${esc(stt.txt)}</b></div>
        <div class="entregaFlags entregaFlagsOnly"><div><b>${esc(code(row.origen))}</b>${flagHtml(row.origen)}<i>›</i><b>${esc(code(row.destino))}</b>${flagHtml(row.destino)}</div></div>
      </div>
      <div class="entregaTimeline" style="--entrega-progress:${pct/100}">
        <div class="entregaLine"><i style="width:${pct}%"></i></div>
        ${st.map((s,i)=>{const d=fmt215(s.time,false),has=d&&d!=='-'; const left=i/max*100; const done=(stt.cls==='done'||i<=idx)&&!(stt.cls==='planned'&&i>0); return `<div class="entregaStep ${i===0?'timelineStart':''} ${i===st.length-1?'timelineEnd':''} ${done?'done':''} ${i===idx&&stt.cls!=='done'?'current':''} ${has&&i>0?'hasDate':''}" style="left:${left}%"><b data-full-hito="${esc(s.name)}" title="${esc(s.name)}">${esc(abbr(s.name))}</b><em>${esc(d)}</em></div>`;}).join('')}
        <div class="entregaVehicle" style="left:calc(72px + (100% - 144px) * ${pct/100})"><img class="entregaCarrierImg" src="assets/camion-automobilero.png" alt="Camión automobilero"></div>
      </div>
      <div class="entregaBottom"><span><b>Actualizado:</b> ${esc(upd)}</span><span><b>Tiempo transcurrido:</b> ${esc(elapsed215(start,end))}</span><span><b>Próximo hito:</b> ${esc(abbr(next))}</span><span><b>Estado:</b> ${esc(stt.txt)}</span></div>
    </article>`;
  }

  function ensureProgramados215(){
    const sel=$('entregaFiltroEstado');
    if(sel&&!Array.from(sel.options).some(o=>o.value==='programado')){
      sel.insertAdjacentHTML('beforeend','<option value="programado">Programados</option>');
    }
  }
  function fill215(id,vals,all){
    const sel=$(id); if(!sel)return;
    const cur=sel.value;
    const list=[...new Set(vals.filter(Boolean).map(S))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
    sel.innerHTML=`<option value="">${esc(all)}</option>`+list.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
    if(list.includes(cur))sel.value=cur;
    sel.onchange=()=>renderEntrega215(false);
  }
  async function renderEntrega215(force=false){
    if(typeof window.ensureEntregaPanelStable==='function')try{window.ensureEntregaPanelStable();}catch(e){}
    await ensureData215(force);
    ensureProgramados215();
    let rows=entregaRows215();
    fill215('entregaFiltroEmbarque',rows.flatMap(r=>r.embarques||[]),'Embarque');
    fill215('entregaFiltroFlota',rows.map(r=>r.flota),'Todas las flotas');
    fill215('entregaFiltroCliente',rows.map(r=>r.cliente),'Todos los clientes');
    fill215('entregaFiltroDestino',rows.map(r=>r.destino),'Todos los destinos');
    ensureProgramados215();
    if($('entregaFiltroEstado'))$('entregaFiltroEstado').onchange=()=>renderEntrega215(false);
    const fe=N($('entregaFiltroEmbarque')?.value), ff=N($('entregaFiltroFlota')?.value), fc=N($('entregaFiltroCliente')?.value), fd=N($('entregaFiltroDestino')?.value), fs=N($('entregaFiltroEstado')?.value);
    rows=rows.filter(r=>{
      const st=status215(r.t,r.e);
      return (!fe||(r.embarques||[]).some(e=>N(e)===fe))&&(!ff||N(r.flota)===ff)&&(!fc||N(r.cliente)===fc)&&(!fd||N(r.destino)===fd)&&(!fs||st.filter===fs);
    });
    const box=$('entregaCards');
  ...
  window.renderEntrega.__v215Timeline=true;

  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__v215Timeline){
    window.tab=function(id){
      const r=oldTab.apply(this,arguments);
      if(id==='entrega'){
  ...
  function installObserver215(){
    const box=$('entregaCards');
    if(!box||box.__obs215)return;
    let pending=false;
    box.__obs215=new MutationObserver(()=>{
      if(pending)return;
      pending=true;
      setTimeout(()=>{pending=false; if(/\[object Object\]|\bINICIO\b/i.test(box.textContent||''))renderEntrega215(false);},120);
    });
...
/* ===== v3.3.21 - Bloqueo del último hito Entregas =====
   Mantiene la lógica de v3.3.21, pero evita que un render heredado posterior
...
(function(){
  ...
  function isClosedCard(card){
    const status=(card.querySelector('.entregaStatus,.state')?.textContent || card.textContent || '').toUpperCase();
    return /\bCERRADO\b|FINALIZADO|FINALIZADA/.test(status);
  }

  function isProgrammedCard(card){
    const status=(card.querySelector('.entregaStatus,.state')?.textContent || card.textContent || '').toUpperCase();
    return /\bPROGRAMADO\b/.test(status);
  }

  function hasRealDate(step){
    const em=(step.querySelector('em')?.textContent || '').trim();
    return !!em && em !== '-' && !/\[object Object\]/i.test(em);
  }

  function stabilizeEntregaCard216(card){
    const timeline=card.querySelector('.entregaTimeline');
    if(!timeline) return;

    const steps=[...timeline.querySelectorAll('.entregaStep')];
    if(steps.length < 2) return;

    const line=timeline.querySelector('.entregaLine i');
    const vehicle=timeline.querySelector('.entregaVehicle');
    const last=steps[steps.length-1];
    const closed=isClosedCard(card);
    const programmed=isProgrammedCard(card);

    // Quitar restos visuales heredados de object Object si aparecen por un render posterior.
    card.querySelectorAll('em,b,span').forEach(el=>{
    ...
    if(closed){
      ...
      if(line) line.style.width='100%';
      if(vehicle){
        vehicle.style.left='calc(72px + (100% - 144px) * 1)';
        vehicle.style.transform='translateX(-50%)';
      }
      return;
    }
    ...
    if(programmed){
      steps.forEach((st,i)=>{
        st.classList.toggle('done', i===0);
        st.classList.toggle('current', i===0);
        if(i>0){
          st.classList.remove('hasDate');
        }
      });
      if(line) line.style.width='0%';
      if(vehicle){
        vehicle.style.left='calc(72px + (100% - 144px) * 0)';
        vehicle.style.transform='translateX(-50%)';
      }
      return;
    }

    // En tránsito: el destino final NO se marca como alcanzado hasta cierre.
    last.classList.remove('done','current','hasDate');
    const lastEm=last.querySelector('em');
    if(lastEm) lastEm.textContent='-';

    let idx=0;
    steps.forEach((st,i)=>{
      if(i < steps.length-1 && (st.classList.contains('done') || st.classList.contains('current') || hasRealDate(st))){
        idx=i;
      }
    });
    idx=Math.max(0, Math.min(idx, steps.length-2));

    steps.forEach((st,i)=>{
      if(i < steps.length-1){
        st.classList.toggle('done', i<=idx);
        st.classList.toggle('current', i===idx);
      }
    });
    ...
    if(vehicle){
      vehicle.style.left='calc(72px + (100% - 144px) * '+(pct/100)+')';
      vehicle.style.transform='translateX(-50%)';
    }
  }

  function stabilizeEntregas216(){
    try{
      const box=$('entregaCards');
      if(!box) return;
  ...
  function stabilizeBurst216(){
    [0,80,180,420,900,1500].forEach(ms=>setTimeout(stabilizeEntregas216,ms));
  }

  const oldRenderEntrega216=window.renderEntrega;
  if(typeof oldRenderEntrega216==='function' && !oldRenderEntrega216.__v216LastHito){
    window.renderEntrega=async function(){
      const r=await oldRenderEntrega216.apply(this,arguments);
      stabilizeBurst216();
      return r;
    };
    window.renderEntrega.__v216LastHito=true;
  }

  const oldTab216=window.tab;
  if(typeof oldTab216==='function' && !oldTab216.__v216LastHito){
    window.tab=function(id){
  ...
  function installObserver216(){
    const box=$('entregaCards');
    if(!box || box.__obs216) return;
    let lock=false;
    box.__obs216=new MutationObserver(()=>{
      if(lock) return;
      lock=true;
      setTimeout(()=>{lock=false;stabilizeEntregas216();},60);
    });
...
/* ===== v3.3.21 - Entregas: bloqueo fuerte render heredado ultimo hito + modal rojo ===== */
(function(){
  const V='3.3.21';
  window.ELTA_APP_VERSION=V;
  window.APP_VERSION_V2=V;
  ...
  function cardStatus217(card){
    const s=((card.querySelector('.entregaStatus')?.textContent||'')+' '+(card.querySelector('.state')?.textContent||'')+' '+(card.textContent||'')).toUpperCase();
    if(/\bCERRADO\b|FINALIZADO|FINALIZADA/.test(s)) return 'closed';
    if(/\bPROGRAMADO\b/.test(s)) return 'programmed';
    return 'open';
  }

  function hardLockEntregaTimeline217(card){
    try{
      const timeline=card.querySelector('.entregaTimeline');
      if(!timeline) return;
      const steps=[...timeline.querySelectorAll('.entregaStep')];
      ...
      const status=cardStatus217(card);
      const line=timeline.querySelector('.entregaLine i');
      const vehicle=timeline.querySelector('.entregaVehicle');
      const last=steps[steps.length-1];

      // Nunca dejar object Object visible.
      card.querySelectorAll('em,b,span').forEach(el=>{
        if(/\[object Object\]/i.test(el.textContent||'')) el.textContent='-';
      });

      if(status === 'closed' || card.classList.contains('closedTransit')){
        card.classList.add('closedTransit');
        steps.forEach(st=>{
          st.classList.add('done');
          st.classList.remove('current','legacyLastHito','timelineLockedPending');
          st.removeAttribute('data-locked-pending');
        });
        if(line) line.style.setProperty('width','100%','important');
        if(vehicle){
          vehicle.style.setProperty('left','calc(72px + (100% - 144px) * 1)','important');
          vehicle.style.setProperty('transform','translateX(-50%)','important');
        }
        return;
      }

      card.classList.remove('closedTransit');

      if(status === 'programmed'){
        steps.forEach((st,i)=>{
          st.classList.toggle('done', i===0);
          st.classList.toggle('current', i===0);
          st.classList.remove('legacyLastHito');
          if(i>0) st.classList.remove('hasDate');
        });
        if(line) line.style.setProperty('width','0%','important');
        if(vehicle){
          vehicle.style.setProperty('left','calc(72px + (100% - 144px) * 0)','important');
          vehicle.style.setProperty('transform','translateX(-50%)','important');
        }
        return;
      }

      // EN TRANSITO: el ultimo hito SIEMPRE queda pendiente, blanco, sin fecha.
      last.classList.remove('done','current','hasDate');
      last.classList.add('timelineLockedPending');
      last.setAttribute('data-locked-pending','1');
      const lastEm=last.querySelector('em');
      if(lastEm) lastEm.textContent='-';
      ...
      steps.forEach((st,i)=>{
        if(i < steps.length-1){
          const em=txt(st.querySelector('em'));
          const hasDate=em && em !== '-' && !/\[object Object\]/i.test(em);
          if(st.classList.contains('done') || st.classList.contains('current') || hasDate) idx=i;
        }
      });
      idx=Math.max(0, Math.min(idx, steps.length-2));
      ...
      steps.forEach((st,i)=>{
        if(i < steps.length-1){
          st.classList.toggle('done', i<=idx);
          st.classList.toggle('current', i===idx);
          st.classList.remove('timelineLockedPending');
          st.removeAttribute('data-locked-pending');
        }
      });

      const pct=(idx/Math.max(1, steps.length-1))*100;
      if(line) line.style.setProperty('width', pct + '%', 'important');
      if(vehicle){
        vehicle.style.setProperty('left','calc(72px + (100% - 144px) * '+(pct/100)+')','important');
        vehicle.style.setProperty('transform','translateX(-50%)','important');
      }
    }catch(e){console.warn('v217 hard lock',e);}
  }

  function hardLockAllEntregas217(){
    const box=$('entregaCards');
    if(!box) return;
  ...
  function burst217(){
    [0,40,90,160,300,600,1000,1800,3000].forEach(ms=>setTimeout(hardLockAllEntregas217,ms));
  }

  const prevRenderEntrega217=window.renderEntrega;
  if(typeof prevRenderEntrega217==='function' && !prevRenderEntrega217.__v217HardLock){
    window.renderEntrega=async function(){
      const r=await prevRenderEntrega217.apply(this,arguments);
      burst217();
      return r;
    };
    window.renderEntrega.__v217HardLock=true;
  }

  const prevTab217=window.tab;
  if(typeof prevTab217==='function' && !prevTab217.__v217HardLock){
    window.tab=function(id){
  ...
  function installObs217(){
    const box=$('entregaCards');
    if(!box || box.__obs217) return;
    let pending=false;
    box.__obs217=new MutationObserver(()=>{
      if(pending) return;
      pending=true;
      requestAnimationFrame(()=>{
        pending=false;
        hardLockAllEntregas217();
      });
    });
    box.__obs217.observe(box,{childList:true,subtree:true,attributes:true,characterData:true,attributeFilter:['class','style']});
  }

  // Captura clicks de actualizar en Entregas para aplicar bloqueo despues de cualquier refresco.
  document.addEventListener('click',ev=>{
    const b=ev.target&&ev.target.closest?ev.target.closest('#entrega button'):null;
    if(!b) return;
    if(/actualizar/i.test(b.textContent||b.title||'')){
      setTimeout(burst217,120);
    }
  },true);
...
/* ===== v3.3.21 - Bloqueo del render heredado que escribe INICIO =====
...
(function(){
  ...
  function abbrOrigin218(v){
    const n=N(v), o=S(v);
    if(/^clz\b/.test(n) || n.includes('centro logistico zarate')) return 'CLZ';
    if(/^clc\b/.test(n) || n.includes('centro logistico cordoba')) return 'CLC';
    if(/^tz\b/.test(n) || n.includes('puerto zarate')) return 'TZ - PUERTO ZARATE';
    if(/^uspallata\b/.test(n)) return 'USPALLATA';
    return o && !/^\[object object\]$/i.test(o) ? o : '';
  }

  function getCardEmbarque218(card){
    const small=[...card.querySelectorAll('small')].map(x=>x.textContent||'').find(t=>/Emb\./i.test(t));
    const m=(small||'').match(/Emb\.\s*([A-Za-z0-9_-]+)/i);
    return m ? m[1] : '';
  }

  function getOriginFromData218(card){
    try{
      const emb=getCardEmbarque218(card);
      let e=null;
      if(emb && Array.isArray(window.embarques)){
        e=window.embarques.find(x=>String(x?.numero||x?.embarque||x?.idEmbarque||x?.codigo||x?.id||x?._docId||'')===String(emb));
      }
      let origin=S(e?.origen||e?.origin||'');
      if(origin) return abbrOrigin218(origin);

      const flags=card.querySelector('.entregaFlags')?.textContent || '';
      // Fallback por país no es suficiente, por eso se prioriza el embarque.
      const currentFirst=card.querySelector('.entregaStep.timelineStart b')?.getAttribute('data-full-hito') || '';
      if(currentFirst && !/inicio/i.test(currentFirst)) return abbrOrigin218(currentFirst);
    }catch(e){}
    return '';
  }

  function lockFirstHito218(card){
    try{
      const first=card.querySelector('.entregaStep.timelineStart');
      if(!first) return;
      const b=first.querySelector('b');
      if(!b) return;

      const current=S(b.textContent);
      const full=S(b.getAttribute('data-full-hito'));
      const bad = /^inicio$/i.test(current) || /^inicio$/i.test(full) || /^\[object object\]$/i.test(current);
      if(!bad) return;

      const origin=getOriginFromData218(card);
      if(!origin) return;

      b.textContent=origin;
      b.setAttribute('data-full-hito', origin);
      b.setAttribute('title', origin);
      first.classList.add('timelineOriginLocked218');
      first.setAttribute('data-origin-locked','1');

      const em=first.querySelector('em');
      if(em && /\[object Object\]/i.test(em.textContent||'')) em.textContent='-';
    }catch(e){console.warn('v218 lock first hito',e);}
  }

  function lockAllFirstHitos218(){
    const box=$('entregaCards');
    if(!box) return;
  ...
  function burst218(){
    [0,30,80,150,300,600,1000,1800,3000].forEach(ms=>setTimeout(lockAllFirstHitos218,ms));
  }

  const oldRenderEntrega218=window.renderEntrega;
  if(typeof oldRenderEntrega218==='function' && !oldRenderEntrega218.__v218InicioLock){
    window.renderEntrega=async function(){
      const r=await oldRenderEntrega218.apply(this,arguments);
      burst218();
      return r;
    };
    window.renderEntrega.__v218InicioLock=true;
  }

  const oldTab218=window.tab;
  if(typeof oldTab218==='function' && !oldTab218.__v218InicioLock){
    window.tab=function(id){
  ...
  function installObserver218(){
    const box=$('entregaCards');
    if(!box || box.__obs218) return;
    let pending=false;
    box.__obs218=new MutationObserver(()=>{
      if(pending) return;
      pending=true;
      requestAnimationFrame(()=>{
        pending=false;
        lockAllFirstHitos218();
      });
    });
    box.__obs218.observe(box,{childList:true,subtree:true,attributes:true,characterData:true,attributeFilter:['class','style','data-full-hito','title']});
  }
...
/* ===== v3.3.21 - Entregas: render final estable sin refresh heredado =====
...
(function(){
  ...
  const get=(o,ks)=>{
    for(const k of ks){
      const v=o&&o[k];
      if(v!==undefined&&v!==null&&String(v).trim()!==''&&String(v)!=='[object Object]') return v;
    }
    return '';
  };
  ...
  function toDate(v){
    try{
      ...
      if(typeof v==='object'){
        ...
        for(const k of ['fecha','date','value','timestamp','createdAt','time']){
          if(v[k]) return toDate(v[k]);
        }
        return null;
      }
      const txt=String(v).trim();
  ...
  async function readCol(name){
    ...
    }catch(e){console.warn('v219 read',name,e);}
    return [];
  }
  async function ensureData(force=false){
    const [emb,tr,ad]=await Promise.all([
  ...
  function findStart(t,e,numero){
    const cargas=cargoStore().filter(c=>S(c.embarque)===S(numero));
    const candidates=[
      get(t,['start','inicio','fechaInicio','createdAt','fechaSalida','salida','startedAt','inicioTransito','fechaInicioTransito']),
      get(e,['fecha','fechaHora','createdAt','inicio','fechaInicio','fechaSalida','salida']),
      ...cargas.map(c=>get(c,['createdAt','fecha','fechaHora','inicio','timestamp']))
    ];
    for(const c of candidates){ if(toDate(c)) return c; }
    return candidates.find(Boolean)||'';
  }
  function findClose(t,e){
    const candidates=[
      get(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin','endedAt']),
      get(e,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin'])
    ];
    for(const c of candidates){ if(toDate(c)) return c; }
    return '';
  ...
  function latestTime(t){
    const vals=[], add=v=>{const d=toDate(v); if(d)vals.push(d);};
    add(get(t,['updatedAt','actualizadoEn','lastUpdate','fechaActualizacion']));
    ['updates','gps','posiciones','registros','aduanas','hitos','eventos','novedades','checkpoints','aduana_registros','aduanaRegistros'].forEach(k=>{
      const v=t&&t[k], a=Array.isArray(v)?v:(v&&typeof v==='object'?Object.values(v):[]);
      a.forEach(r=>add(get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora','ingreso','liberacion','fechaIngreso','fechaLiberacion'])));
    });
    vals.sort((a,b)=>b-a);
    return vals[0]||'';
  }
  ...
  function regTime(r){return get(r,['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora','ingreso','liberacion','fechaIngreso','fechaLiberacion']);}
  function regName(r){return S(get(r,['hito','nombre','name','label','title','aduana','aduanaId','checkpoint','puesto','evento','tipo','estado','level','localidad','ciudad']));}
  function records(t){
    const out=[], push=r=>{
      if(!r||typeof r!=='object')return;
      const name=regName(r), time=regTime(r);
      if(name&&toDate(time)&&!/^(updates?|gps|registros?|logs?|tracking|posiciones?)$/i.test(name)) out.push({name,time});
    };
    ['aduanas','aduana','registros','hitos','eventos','novedades','checkpoints','updates','aduana_registros','aduanaRegistros'].forEach(k=>{
      const v=t&&t[k];
      if(Array.isArray(v))v.forEach(push); else if(v&&typeof v==='object')Object.values(v).forEach(push);
    ...
    const seen=new Set();
    return out.sort((a,b)=>(toDate(a.time)?.getTime()||0)-(toDate(b.time)?.getTime()||0)).filter(r=>{
      const k=N(r.name)+'|'+(toDate(r.time)?.getTime()||'');
      if(seen.has(k))return false; seen.add(k); return true;
    });
  }
  function aduanaNames(destino,origen){
  ...
  function entregaRows(){
    ...
    arr('trs').forEach(t=>{
      ...
      const origen=S(get(e,['origen','origin']))||S(rt.origen||t.origen)||'Origen';
      const destino=S(get(e,['destino','destination']))||S(rt.destino||t.destino)||'Cierre';
      rows.push({flota:f,embarques:n?[n]:[],numero:n,t,e,cliente:S(get(e,['cliente','client']))||S(rt.cliente||t.cliente),origen,destino,fecha:findStart(t,e,n)});
    });
    arr('embarques').forEach(e=>{
      const n=embNum(e);
      if(!n || rows.some(r=>(r.embarques||[]).includes(n)))return;
      rows.push({flota:S(get(e,['flota','fleet','unidad','usuario'])||get(e?.user,['fleet'])),embarques:n?[n]:[],numero:n,t:{},e,cliente:S(get(e,['cliente','client'])),origen:S(get(e,['origen','origin']))||'Origen',destino:S(get(e,['destino','destination']))||'Cierre',fecha:findStart({},e,n)});
    });
    return rows.sort((a,b)=>String(a.flota).localeCompare(String(b.flota),'es',{numeric:true}));
  }
  function stages(row){
    const stt=status(row.t,row.e);
    const names=aduanaNames(row.destino,row.origen);
    const start=findStart(row.t,row.e,row.numero);
    const close=findClose(row.t,row.e);
    const st=names.map((name,i)=>{
      let time='';
      if(i===0) time=start;
      else if(i===names.length-1) time=(stt.cls==='done'?close:'');
      return {name,time,kind:i===0?'start':(i===names.length-1?'end':'aduana')};
    });
    records(row.t).forEach(r=>{
      const idx=st.findIndex((s,i)=>i>0&&i<st.length-1&&match(r.name,s.name));
      if(idx>=0&&!toDate(st[idx].time)) st[idx].time=r.time;
    });
    return st;
  }
  function card(row){
    const st=stages(row), stt=status(row.t,row.e);
    let idx=0;
    st.forEach((s,i)=>{ if(toDate(s.time)&&(i<st.length-1||stt.cls==='done')) idx=i; });
    if(stt.cls==='done')idx=st.length-1;
    if(stt.cls==='planned')idx=0;
    const max=Math.max(1,st.length-1), pct=idx/max*100;
    const close=findClose(row.t,row.e);
    const start=st[0]?.time||row.fecha;
    const updateRaw=stt.cls==='done'?close:(latestTime(row.t)||start||row.fecha);
    const upd=fmt(updateRaw,true);
    const next=stt.cls==='done'?'Tránsito finalizado':(st.find((s,i)=>i>idx)?.name||st[idx]?.name||'-');
    return `<article class="entregaCard ${stt.cls==='done'?'closedTransit':''}">
      <div class="entregaTop">
        <div class="entregaFleet"><span class="entregaFleetIcon">🚚</span><div><h3>Flota ${esc(row.flota||'-')}</h3><span class="entregaStatus ${stt.cls}">${esc(stt.txt)}</span><small>Emb. ${esc((row.embarques||[]).join(', ')||row.numero||'-')}</small></div></div>
        <div class="entregaMeta"><span><i>🗓️</i> Última act.</span><b>${esc(upd)}</b></div>
        <div class="entregaMeta"><span><i>⏱️</i> Tiempo transcurrido</span><b>${esc(elapsed(start,stt.cls==='done'?close:null))}</b></div>
        <div class="entregaMeta"><span><i>🚩</i> Próximo hito</span><b>${esc(abbr(next))}</b></div>
        <div class="entregaMeta"><span><i>📡</i> Estado</span><b class="state ${stt.cls}">${esc(stt.txt)}</b></div>
        <div class="entregaFlags entregaFlagsOnly"><div><b>${esc(code(row.origen))}</b>${flagHtml(row.origen)}<i>›</i><b>${esc(code(row.destino))}</b>${flagHtml(row.destino)}</div></div>
      </div>
      <div class="entregaTimeline" style="--entrega-progress:${pct/100}">
        <div class="entregaLine"><i style="width:${pct}%"></i></div>
        ${st.map((s,i)=>{const d=fmt(s.time,false),has=d&&d!=='-'; const left=i/max*100; const done=(stt.cls==='done'||i<=idx)&&!(stt.cls==='planned'&&i>0); return `<div class="entregaStep ${i===0?'timelineStart':''} ${i===st.length-1?'timelineEnd':''} ${done?'done':''} ${i===idx&&stt.cls!=='done'?'current':''} ${has&&i>0?'hasDate':''}" style="left:${left}%"><b data-full-hito="${esc(s.name)}" title="${esc(s.name)}">${esc(abbr(s.name))}</b><em>${esc(d)}</em></div>`;}).join('')}
        <div class="entregaVehicle" style="left:calc(72px + (100% - 144px) * ${pct/100})"><img class="entregaCarrierImg" src="assets/camion-automobilero.png" alt="Camión automobilero"></div>
      </div>
      <div class="entregaBottom"><span><b>Actualizado:</b> ${esc(upd)}</span><span><b>Tiempo transcurrido:</b> ${esc(elapsed(start,stt.cls==='done'?close:null))}</span><span><b>Próximo hito:</b> ${esc(abbr(next))}</span><span><b>Estado:</b> ${esc(stt.txt)}</span></div>
    </article>`;
  }
  function ensureProgramados(){
    const sel=$('entregaFiltroEstado');
    if(sel&&!Array.from(sel.options).some(o=>o.value==='programado')){
      sel.insertAdjacentHTML('beforeend','<option value="programado">Programados</option>');
    }
  ...
  function fill(id,vals,all){
    ...
    sel.innerHTML=`<option value="">${esc(all)}</option>`+list.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');
    if(list.includes(cur))sel.value=cur;
    sel.onchange=()=>renderEntregaStable(false);
  }
  async function renderEntregaStable(force=false){
    if(typeof window.ensureEntregaPanelStable==='function')try{window.ensureEntregaPanelStable();}catch(e){}
    await ensureData(force);
    ensureProgramados();
    let rows=entregaRows();
    ...
    fill('entregaFiltroDestino',rows.map(r=>r.destino),'Todos los destinos');
    ensureProgramados();
    if($('entregaFiltroEstado'))$('entregaFiltroEstado').onchange=()=>renderEntregaStable(false);
    const fe=N($('entregaFiltroEmbarque')?.value), ff=N($('entregaFiltroFlota')?.value), fc=N($('entregaFiltroCliente')?.value), fd=N($('entregaFiltroDestino')?.value), fs=N($('entregaFiltroEstado')?.value);
    rows=rows.filter(r=>{
      const st=status(r.t,r.e);
      return (!fe||(r.embarques||[]).some(e=>N(e)===fe))&&(!ff||N(r.flota)===ff)&&(!fc||N(r.cliente)===fc)&&(!fd||N(r.destino)===fd)&&(!fs||st.filter===fs);
  ...
  window.renderEntrega.__v219Stable=true;

  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__v219Stable){
    window.tab=function(id){
      if(id==='entrega'){
        document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));
        document.querySelectorAll('.sideNav button').forEach(b=>b.classList.remove('active'));
  ...
  document.addEventListener('click',ev=>{
    ...
    if(/actualizar/i.test(b.textContent||b.title||'')){
      ...
      b.disabled=true;
      b.textContent='Actualizando...';
      renderEntregaStable(true).finally(()=>{b.disabled=false;b.textContent=old||'Actualizar';});
      return false;
    }
  },true);
...
/* ===== v3.3.21 - Entregas: bloqueo mínimo de INICIO sin tocar carga de datos =====
   Base: v3.3.21 estable. No modifica Firebase, refresh ni render principal.
...
(function(){
  ...
  function embNo(card){
    const t=card.querySelector('.entregaFleet small')?.textContent || card.textContent || '';
    const m=t.match(/Emb\.\s*([A-Za-z0-9_-]+)/i);
    return m ? m[1] : '';
  }

  function originFromEmbarque(card){
    const n=embNo(card);
    try{
      const list=Array.isArray(window.embarques)?window.embarques:[];
      const e=list.find(x=>String(x?.numero||x?.embarque||x?.idEmbarque||x?.codigo||x?.id||x?._docId||'')===String(n));
      const o=e&&(e.origen||e.origin);
      if(o) return abbr(o);
    }catch(e){}
    const full=card.querySelector('.entregaStep.timelineStart b')?.getAttribute('data-full-hito') || '';
    if(full && !/^inicio$/i.test(full)) return abbr(full);
    return '';
  }

  function fixInicio(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      const b=card.querySelector('.entregaStep.timelineStart b');
      if(!b) return;
      const current=S(b.textContent);
      const full=S(b.getAttribute('data-full-hito'));
      if(/^inicio$/i.test(current)||/^inicio$/i.test(full)||/^\[object Object\]$/i.test(current)){
        const o=originFromEmbarque(card);
        if(o){
          b.textContent=o;
          b.setAttribute('data-full-hito',o);
          b.setAttribute('title',o);
        }
      }
      card.querySelectorAll('em').forEach(em=>{
        if(/\[object Object\]/i.test(em.textContent||'')) em.textContent='-';
      });
    });
    setV();
  }
  ...
  function burst(){
    [0,80,180,400,900,1600].forEach(ms=>setTimeout(fixInicio,ms));
  }

  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__fixInicio223){
    window.tab=function(id){
      const r=oldTab.apply(this,arguments);
      if(id==='entrega') burst();
      return r;
    };
    window.tab.__fixInicio223=true;
  }

  document.addEventListener('click',ev=>{
    const b=ev.target&&ev.target.closest?ev.target.closest('#entrega button'):null;
    if(b && /actualizar/i.test(b.textContent||b.title||'')) setTimeout(burst,200);
  },true);

  document.addEventListener('DOMContentLoaded',()=>{setV();setTimeout(burst,500);});
  window.addEventListener('load',()=>{setV();setTimeout(burst,500);});
})();




/* ===== v3.3.21 - Entregas: fechas cierre + separación línea punteada =====
   Base v3.3.21. No toca Firebase ni render principal.
...
(function(){
  ...
  function toDate(v){
    try{
      ...
      if(typeof v==='object'){
        ...
        for(const k of ['fecha','fechaHora','date','value','timestamp','createdAt','time','inicio','cierre','end','fin']){
          if(v[k]){ const d=toDate(v[k]); if(d) return d; }
        }
        return null;
      }
      const txt=String(v).trim();
      if(!txt || txt==='-' || /\[object Object\]/i.test(txt)) return null;
      const d=new Date(txt);
      return isNaN(d)?null:d;
    }catch(e){return null;}
  }
  ...
  function isClosed(card){
    const t=(card.querySelector('.entregaStatus')?.textContent+' '+card.querySelector('.state')?.textContent+' '+card.textContent).toUpperCase();
    return /\bCERRADO\b|FINALIZADO/.test(t);
  }
  function collectRecordDates(t){
    const out=[];
    const add=v=>{const d=toDate(v); if(d) out.push(d);};
    ['start','inicio','fechaInicio','createdAt','fechaSalida','salida','startedAt','inicioTransito','fechaInicioTransito',
     'cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin','endedAt',
     'updatedAt','actualizadoEn','lastUpdate','fechaActualizacion'].forEach(k=>add(t&&t[k]));
    ['updates','gps','posiciones','registros','aduanas','hitos','eventos','novedades','checkpoints','aduana_registros','aduanaRegistros'].forEach(k=>{
      const v=t&&t[k];
      const a=Array.isArray(v)?v:(v&&typeof v==='object'?Object.values(v):[]);
      a.forEach(r=>{
        ['time','fecha','fechaHora','createdAt','updatedAt','actualizadoEn','timestamp','date','hora','ingreso','liberacion','fechaIngreso','fechaLiberacion'].forEach(x=>add(r&&r[x]));
      });
    });
  ...
  function dataForCard(card){
    const n=embNo(card);
    const emb=(Array.isArray(window.embarques)?window.embarques:[]).find(e=>String(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId||'')===String(n))||{};
    const trs=(Array.isArray(window.trs)?window.trs:[]);
    const t=trs.find(x=>String(x?.embarque||'')===String(n)) || trs.find(x=>String(x?.flota||x?.fleet||x?.usuario||x?.user?.fleet||'')===String((card.querySelector('.entregaFleet h3')?.textContent.match(/\d+/)||[])[0]||'')) || {};
    return {emb,t,n};
  }
  function bestStart(emb,t){
    const cand=[
      get(t,['start','inicio','fechaInicio','createdAt','fechaSalida','salida','startedAt','inicioTransito','fechaInicioTransito']),
      get(emb,['fecha','fechaHora','createdAt','inicio','fechaInicio','fechaSalida','salida'])
    ];
    for(const c of cand){ if(toDate(c)) return c; }
    const dates=collectRecordDates(t);
    return dates[0]||'';
  }
  function bestEnd(emb,t){
    const cand=[
      get(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin','endedAt']),
      get(emb,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin'])
    ];
    for(const c of cand){ if(toDate(c)) return c; }
    const dates=collectRecordDates(t);
  ...
  function fixClosedDatesAndDots(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      const steps=[...card.querySelectorAll('.entregaStep')];
      if(!steps.length) return;
      const {emb,t}=dataForCard(card);
      const first=steps[0];
      const last=steps[steps.length-1];

      const startText=fmt(bestStart(emb,t));
      const endText=fmt(bestEnd(emb,t));

      if(startText && startText!=='-'){
        const em=first.querySelector('em');
        if(em && (!S(em.textContent) || S(em.textContent)==='-' || /\[object Object\]/i.test(em.textContent||''))){
          em.textContent=startText;
        }
      }

      if(isClosed(card) && endText && endText!=='-'){
        const em=last.querySelector('em');
        if(em && (!S(em.textContent) || S(em.textContent)==='-' || /\[object Object\]/i.test(em.textContent||''))){
          em.textContent=endText;
        }
        card.classList.add('closedTransit');
        steps.forEach(s=>{s.classList.add('done');s.classList.remove('current');});
        const line=card.querySelector('.entregaLine i');
        if(line) line.style.width='100%';
      }

      card.querySelectorAll('.entregaStep.hasDate,.entregaStep.done,.entregaStep.current').forEach(st=>{
        st.classList.add('v224DotSafe');
      });
    });
    setV();
  }
  ...
  function burst(){
    [0,80,180,400,900,1600].forEach(ms=>setTimeout(fixClosedDatesAndDots,ms));
  }

  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__v224Dates){
    window.tab=function(id){
      const r=oldTab.apply(this,arguments);
      if(id==='entrega') burst();
      return r;
    };
    window.tab.__v224Dates=true;
  }

  document.addEventListener('click',ev=>{
    const b=ev.target&&ev.target.closest?ev.target.closest('#entrega button'):null;
    if(b && /actualizar/i.test(b.textContent||b.title||'')) setTimeout(burst,220);
  },true);
...
/* ===== v3.3.21 - Entregas: fechas inicio/cierre desde objetos Firebase + línea corta =====
   Base v3.3.21. No toca la carga de datos. Corrige sólo visualización final. */
(function(){
  const V='3.3.21';
  window.ELTA_APP_VERSION=V;
  window.APP_VERSION_V2=V;
  ...
  function toDate(v){
    try{
      ...
      if(typeof v==='object'){
        ...
        for(const k of ['fecha','fechaHora','date','value','timestamp','createdAt','time','inicio','start','cierre','end','fin','closedAt']){
          const d=toDate(v[k]);
          if(d) return d;
        }
        return null;
  ...
  function raw(o,ks){
    for(const k of ks){
      if(o && Object.prototype.hasOwnProperty.call(o,k) && o[k]!==undefined && o[k]!==null && o[k]!=='') return o[k];
    }
    return '';
  }
  ...
  function flotaNo(card){
    return (card.querySelector('.entregaFleet h3')?.textContent.match(/\d+/)||[])[0] || '';
  }
  ...
  function dataForCard(card){
    const n=embNo(card);
    const f=flotaNo(card);
    const emb=(Array.isArray(window.embarques)?window.embarques:[]).find(e=>String(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId||'')===String(n))||{};
    const trs=(Array.isArray(window.trs)?window.trs:[]);
    const t=trs.find(x=>String(x?.embarque||'')===String(n)) || trs.find(x=>String(x?.flota||x?.fleet||x?.usuario||x?.user?.fleet||'')===String(f)) || {};
    return {emb,t,n,f};
  }

  function allDatesFromTransit(t){
    const out=[];
    const add=v=>{const d=toDate(v); if(d) out.push(d);};
    [
      'start','inicio','fechaInicio','createdAt','fechaSalida','salida','startedAt','inicioTransito','fechaInicioTransito',
      'cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin','endedAt',
      'updatedAt','actualizadoEn','lastUpdate','fechaActualizacion'
    ].forEach(k=>add(t&&t[k]));
    ['updates','gps','posiciones','registros','aduanas','hitos','eventos','novedades','checkpoints','aduana_registros','aduanaRegistros'].forEach(k=>{
      const v=t&&t[k];
      const a=Array.isArray(v)?v:(v&&typeof v==='object'?Object.values(v):[]);
  ...
  function bestStart(emb,t){
    const candidates=[
      raw(t,['start','inicio','fechaInicio','fechaSalida','salida','startedAt','inicioTransito','fechaInicioTransito','createdAt']),
      raw(emb,['fecha','fechaHora','inicio','fechaInicio','fechaSalida','salida','createdAt'])
    ];
    for(const c of candidates){ const d=toDate(c); if(d) return d; }
    return allDatesFromTransit(t)[0] || null;
  }

  function bestEnd(emb,t){
    const candidates=[
      raw(t,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin','endedAt']),
      raw(emb,['cierre','fechaCierre','closedAt','closed','end','finalizadoEn','fechaFin','fin'])
    ];
    for(const c of candidates){ const d=toDate(c); if(d) return d; }
    const d=allDatesFromTransit(t);
  ...
  function forceDates(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      ...
      const sTxt=fmt(bestStart(emb,t));
      const eTxt=fmt(bestEnd(emb,t));

      if(sTxt){
        const em=first.querySelector('em');
        if(em) em.textContent=sTxt;
        first.classList.add('hasDate','v225ShortDot');
      }
      if(isClosed(card) && eTxt){
        const em=last.querySelector('em');
        if(em) em.textContent=eTxt;
        last.classList.add('hasDate','done','v225ShortDot');
        card.classList.add('closedTransit');
        const line=card.querySelector('.entregaLine i');
        if(line) line.style.width='100%';
        steps.forEach(st=>{st.classList.add('done');st.classList.remove('current');});
      }

      card.querySelectorAll('.entregaStep.hasDate,.entregaStep.done,.entregaStep.current').forEach(st=>st.classList.add('v225ShortDot'));
    });
    setV();
  }

  function burst(){
    [0,80,180,400,900,1600,2600].forEach(ms=>setTimeout(forceDates,ms));
  }

  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__v225Dates){
    window.tab=function(id){
      const r=oldTab.apply(this,arguments);
      if(id==='entrega') burst();
      return r;
    };
    window.tab.__v225Dates=true;
  }
...
/* ===== v3.3.21 - Entregas: fechas inicio/cierre robustas + línea vertical mínima =====
...
(function(){
  ...
  function arr(name){
    ...
    try{ const v=eval(name); if(Array.isArray(v)) return v; }catch(e){}
    return [];
  }
  ...
  function toDate(v){
    try{
      ...
      if(typeof v==='object'){
        ...
        for(const k of ['fecha','fechaHora','date','value','timestamp','createdAt','time','inicio','start','cierre','end','fin','closedAt','salida','fechaInicio','fechaCierre']){
          const d=toDate(v[k]);
          if(d) return d;
        }
        return null;
  ...
  function dataForCard(card){
    const n=embNo(card);
    const f=flotaNo(card);
    const embarques = arr('embarques');
    const transitos = arr('trs').concat(arr('transitos'));
    const emb=embarques.find(e=>String(e?.numero||e?.embarque||e?.idEmbarque||e?.codigo||e?.id||e?._docId||'')===String(n))||{};
    const t=transitos.find(x=>String(x?.embarque||'')===String(n)) || transitos.find(x=>String(x?.flota||x?.fleet||x?.usuario||x?.user?.fleet||'')===String(f)) || {};
    return {emb,t,n,f};
  }

  function recordDates(t){
    const out=[];
    const add=v=>{const d=toDate(v); if(d) out.push(d);};
    [
      'start','inicio','fechaInicio','createdAt','fechaSalida','salida','startedAt','inicioTransito','fechaInicioTransito',
  ...
  function completeDates(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      ...
      const startTxt=fmt(bestStart(emb,t));
      const endTxt=fmt(bestEnd(emb,t));

      if(startTxt){
        let em=first.querySelector('em');
        if(!em){ em=document.createElement('em'); first.appendChild(em); }
        em.textContent=startTxt;
        first.classList.add('hasDate','v226TinyDot');
      }

      if(isClosed(card) && endTxt){
        let em=last.querySelector('em');
        if(!em){ em=document.createElement('em'); last.appendChild(em); }
        em.textContent=endTxt;
        last.classList.add('hasDate','done','v226TinyDot');
        card.classList.add('closedTransit');
        steps.forEach(st=>{st.classList.add('done');st.classList.remove('current');});
        const line=card.querySelector('.entregaLine i');
        if(line) line.style.width='100%';
      ...
      card.querySelectorAll('.entregaStep.hasDate,.entregaStep.done,.entregaStep.current').forEach(st=>st.classList.add('v226TinyDot'));
    });
    setV();
  }

  function burst(){
    [0,80,180,400,900,1600,2600,4000].forEach(ms=>setTimeout(completeDates,ms));
  }

  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__v226Dates){
    window.tab=function(id){
      const r=oldTab.apply(this,arguments);
      if(id==='entrega') burst();
      return r;
    };
    window.tab.__v226Dates=true;
  }
...
/* ===== ELTA v3.3.21 - VERSION Y DOCUMENTACION FINAL LIMPIA ===== */
(function(){
  'use strict';
  const VERSION='3.3.21';
  ...
  const LS_KEY='elta_docs_local_v313';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
  ...
  const safe=n=>S(n).replace(/[\/:*?"<>|]+/g,'_').replace(/\s+/g,'_').slice(0,140)||'documento.pdf';
  const icon=t=>{t=S(t).toLowerCase(); if(t.includes('fact'))return '🧾'; if(t.includes('hr'))return '📋'; if(t.includes('mic'))return '📄'; if(t.includes('crt'))return '📄'; if(t.includes('oea'))return '🛡️'; if(t.includes('rem'))return '📦'; if(t.includes('cert'))return '📑'; return '📁';};
  function dbx(){try{return window.db||(typeof db!=='undefined'?db:null)||(window.firebase&&firebase.firestore?firebase.firestore():null);}catch(e){return null;}}
  function nowTs(){try{return firebase.firestore.FieldValue.serverTimestamp();}catch(e){return new Date().toISOString();}}
  function timeout(p,ms,label){return Promise.race([p,new Promise((_,rej)=>setTimeout(()=>rej(new Error(label||'Tiempo agotado')),ms))]);}
  function toMillis(v){try{if(!v)return 0;if(v.toDate)return v.toDate().getTime();if(v.seconds)return v.seconds*1000;const d=new Date(v);return isNaN(d)?0:d.getTime();}catch(e){return 0;}}
  ...
  async function fetchRemote(emb){
    ...
    const jobs=[];
    keys(emb).forEach(k=>['embarque','nroEmbarque','embarqueId'].forEach(f=>jobs.push(timeout(db.collection(COLLECTION).where(f,'==',String(k)).get(),QUERY_TIMEOUT,'Consulta').catch(()=>null))));
    const snaps=await Promise.all(jobs); snaps.forEach(add); return out;
  }
  function localDocs(emb){emb=cleanEmb(emb);return readLocal().filter(x=>keys(emb).includes(String(x.embarque))&&active(x));}

  function renderRows(rows,emb){
    const body=document.getElementById('docsRowsV313');
    if(!body)return;
    window.__docsCacheV313={};
    rows.forEach(r=>window.__docsCacheV313[r.id]=r);
    if(!rows.length){
      body.innerHTML='<div class="docsEmptyV313">No hay documentos cargados.<br><small>Seleccione un PDF para comenzar.</small></div>';
      return;
    }
    body.innerHTML=`<table class="docsTableV313"><thead><tr><th>Tipo</th><th>Documento</th><th>Fecha</th><th>Acciones</th></tr></thead><tbody>${rows.map(d=>`<tr><td><span class="docTypeV313">${icon(d.tipo)} ${E(d.tipo||'Otros')}</span></td><td>${E(d.nombre||d.fileName||'documento.pdf')}</td><td>${fmt(d.fechaCarga||d.createdAt||d.fecha)}</td><td class="docActionsV313"><button type="button" onclick="viewDocV313('${E(d.id)}')" title="Ver">👁</button><button type="button" onclick="downloadDocV313('${E(d.id)}')" title="Descargar">⬇</button><button type="button" onclick="deleteDocV313('${E(d.id)}','${E(emb)}')" title="Eliminar">🗑</button></td></tr>`).join('')}</tbody></table>`;
  }

  async function loadDocs(emb){
    emb=cleanEmb(emb);
    const btn=document.getElementById('docsRefreshV313');
    const body=document.getElementById('docsRowsV313');
    if(btn){btn.disabled=true;btn.textContent='Actualizando...';}
    if(body)body.innerHTML='<div class="docsEmptyV313">Cargando documentación...<br><small>Consultando documentos del embarque.</small></div>';
    let timeoutShown=false;
    const slow=setTimeout(()=>{timeoutShown=true;if(body)body.innerHTML='<div class="docsEmptyV313">La consulta está demorando.<br><small>Puede subir documentos mientras se actualiza la lista.</small></div>'; },1800);
    try{
      const rows=[...await fetchRemote(emb),...localDocs(emb)].sort((a,b)=>toMillis(b.fechaCarga||b.createdAt||b.fecha)-toMillis(a.fechaCarga||a.createdAt||a.fecha));
      clearTimeout(slow); renderRows(rows,emb);
    }catch(e){
      clearTimeout(slow); console.warn('loadDocs',e); renderRows(localDocs(emb),emb);
    }finally{ if(btn){btn.disabled=false;btn.textContent='Actualizar';} }
  }

  function buildModal(emb,flota){
    closeDocs();
    const overlay=document.createElement('div'); overlay.id='docsOverlayV313'; overlay.className='docsOverlayV313';
    overlay.innerHTML=`<div class="docsModalV313"><div class="docsHeadV313"><div><h2>Documentación · Emb. ${E(emb)}</h2><p>Flota ${E(flota||'-')} · PDF del embarque</p></div><button type="button" onclick="closeDocsModalV313()">×</button></div><div class="docsBodyV313"><section class="docsUploadV313"><h3>Subir PDF</h3><label>Tipo</label><select id="docsTipoV313"><option>MIC/DTA</option><option>CRT</option><option>Factura</option><option>HR</option><option>Remito</option><option>Certificado</option><option>Otros</option></select><label>Observación <small>(opcional)</small></label><input id="docsObsV313" placeholder="Ej. MIC N° 123456"><label class="docsDropV313" for="docsFileV313"><span>☁</span><b>Seleccionar PDF</b><small>Máx. 12 MB por archivo</small></label><input id="docsFileV313" type="file" accept="application/pdf,.pdf" multiple style="display:none"><div id="docsSelectedV313" class="docsSelectedV313">Sin archivos seleccionados.</div><div id="docsProgressV313" class="docsProgressV313"></div><button id="docsUploadBtnV313" class="docsPrimaryV313" type="button" onclick="uploadDocsV313('${E(emb)}','${E(flota||'')}')">Subir documento</button></section><section class="docsListV313"><div class="docsListHeadV313"><h3>Documentos cargados</h3><button id="docsRefreshV313" type="button" onclick="loadDocsV313('${E(emb)}')">Actualizar</button></div><div id="docsRowsV313" class="docsRowsV313"><div class="docsEmptyV313">Abriendo vista...</div></div></section></div></div>`;
    document.body.appendChild(overlay);
    const input=document.getElementById('docsFileV313');
    input?.addEventListener('change',()=>{const files=[...(input.files||[])];document.getElementById('docsSelectedV313').textContent=files.length?files.map(f=>`${f.name} (${Math.max(1,Math.round(f.size/1024))} KB)`).join(' · '):'Sin archivos seleccionados.';});
  }
  function openDocs(emb,flota){emb=cleanEmb(emb); if(!emb){alert('No se pudo identificar el embarque.');return false;} buildModal(emb,flota); setTimeout(()=>loadDocs(emb),50); return false;}
  function closeDocs(){document.getElementById('docsOverlayV313')?.remove();document.getElementById('docsOverlayV3')?.remove();document.getElementById('docsOverlay246')?.remove();document.getElementById('docsOverlay228')?.remove();}

  async function uploadDocs(emb,flota){
    emb=cleanEmb(emb);
    const input=document.getElementById('docsFileV313');
    const files=[...(input?.files||[])];
    const btn=document.getElementById('docsUploadBtnV313');
    const prog=document.getElementById('docsProgressV313');
    const selected=document.getElementById('docsSelectedV313');
    const tipo=S(document.getElementById('docsTipoV313')?.value)||'Otros';
    const obs=S(document.getElementById('docsObsV313')?.value);
    ...
    try{
      for(let i=0;i<files.length;i++){
        ...
        if(!/\.pdf$/i.test(file.name) && file.type!=='application/pdf'){fail++; alert(fname+' no es PDF.'); continue;}
        if(file.size>MAX_MB*1024*1024){fail++; alert(fname+' supera '+MAX_MB+' MB.'); continue;}
        if(btn)btn.textContent=`Subiendo archivos... ${i+1}/${files.length}`;
        if(prog)prog.textContent=`Guardando ${i+1}/${files.length}: ${fname}`;
        let base64='';
        try{ base64=await fileToBase64(file); }catch(e){ console.warn(e); }
        const payload={embarque:String(emb),nroEmbarque:String(emb),embarqueId:String(emb),flota:String(flota||''),tipo,nombre:fname,mimeType:file.type||'application/pdf',sizeBytes:file.size,fechaCarga:nowTs(),createdAt:nowTs(),usuario:S(window.ELTA_LOGGED_USER?.nombre||window.ELTA_LOGGED_USER?.user||'Usuario'),observaciones:obs,activo:true,archivoBase64:base64,origen:'firestore_base64'};
        try{
          if(!db) throw new Error('Firestore no disponible');
          await timeout(db.collection(COLLECTION).add(payload),WRITE_TIMEOUT,'Guardar documento'); ok++;
        }catch(e){
          console.warn('Firestore rechazó, guardo local',e);
          const rows=readLocal(); rows.push({...payload,id:'local_'+Date.now()+'_'+i,fechaCarga:new Date().toISOString(),createdAt:new Date().toISOString(),_local:true,origen:'local_fallback'}); writeLocal(rows); local++;
        }
      }
      if(input)input.value=''; const obsEl=document.getElementById('docsObsV313'); if(obsEl)obsEl.value='';
      if(selected)selected.textContent='Sin archivos seleccionados.';
      if(prog)prog.textContent= local?`Carga finalizada: ${ok} en Firestore, ${local} local por permisos.`:`Carga finalizada: ${ok} archivo(s).`;
      await loadDocs(emb);
    }finally{ if(btn){btn.disabled=false;btn.textContent='Subir documento';} setTimeout(()=>{if(prog)prog.textContent='';},5500); }
  }

  function viewDoc(id){const d=(window.__docsCacheV313||{})[id]||{};const url=docUrl(d);if(!url){alert('El documento no tiene archivo disponible.');return;}window.open(url,'_blank','noopener');}
  function downloadDoc(id){const d=(window.__docsCacheV313||{})[id]||{};const url=docUrl(d);if(!url){alert('El documento no tiene archivo disponible.');return;}const a=document.createElement('a');a.href=url;a.download=d.nombre||'documento.pdf';document.body.appendChild(a);a.click();a.remove();}
  async function deleteDoc(id,emb){
    if(!confirm('¿Eliminar este documento del embarque?'))return;
    const d=(window.__docsCacheV313||{})[id];
    if(d?._local){writeLocal(readLocal().map(x=>x.id===id?{...x,activo:false,deleted:true}:x)); await loadDocs(emb); return;}
    const db=dbx(); if(!db){alert('Firestore no está disponible.');return;}
    try{await timeout(db.collection(COLLECTION).doc(id).update({activo:false,deleted:true,fechaEliminacion:nowTs()}),WRITE_TIMEOUT,'Eliminar'); await loadDocs(emb);}catch(e){console.warn(e);alert('No se pudo eliminar el documento.');}
  }

  function embFromButton(btn){let on=btn.getAttribute('onclick')||'';let m=on.match(/openDocsModal\w*\(['"]([^'"]+)/);return cleanEmb(btn.dataset.embarque||btn.dataset.emb||m?.[1]||'');}
  function flotaFromButton(btn){let on=btn.getAttribute('onclick')||'';let m=on.match(/openDocsModal\w*\(['"][^'"]+['"]\s*,\s*['"]([^'"]*)/);return S(btn.dataset.flota||m?.[1]||'');}
  function enableButton(btn){
    if(btn.closest('#docsOverlayV313')) return;
    const emb=embFromButton(btn); const fl=flotaFromButton(btn);
    if(!emb) return;
    btn.disabled=false;btn.removeAttribute('disabled');btn.setAttribute('aria-disabled','false');btn.dataset.embarque=emb;if(fl)btn.dataset.flota=fl;btn.style.pointerEvents='auto';btn.style.opacity='1';btn.classList.remove('disabled','docsNoFiles');btn.title='Abrir documentación PDF';
    if(/docs/i.test(btn.textContent||'')) btn.innerHTML='📄 Docs';
    btn.onclick=function(ev){ev.preventDefault();ev.stopPropagation();return openDocs(embFromButton(btn)||emb,flotaFromButton(btn)||fl);};
  }
  function enableDocsButtons(){
    document.querySelectorAll('button[onclick*="openDocs"],button.docsBtn,button.btnDocs,button[data-embarque][class*="Docs"],button[data-docs]').forEach(enableButton);
    document.querySelectorAll('button').forEach(btn=>{if(btn.closest('#docsOverlayV313'))return; const t=(btn.textContent||'').trim(); if(/^📄?\s*Docs$/i.test(t)||/^Docs$/i.test(t)) enableButton(btn);});
  }
  function injectCss(){
    if(document.getElementById('docsCssV313'))return;
    const st=document.createElement('style');st.id='docsCssV313';st.textContent=`
      .docsOverlayV313{position:fixed;inset:0;background:rgba(0,0,0,.62);z-index:99999;display:flex;align-items:center;justify-content:center;padding:6px}
      .docsModalV313{width:min(1080px,92vw);max-height:76vh;background:#132238;border:1px solid rgba(177,201,230,.22);border-radius:14px;box-shadow:0 14px 40px rgba(0,0,0,.45);overflow:hidden;color:#eef5ff;font-family:inherit}
      .docsHeadV313{background:#2f8d35;padding:10px 18px;display:flex;justify-content:space-between;align-items:flex-start}
      .docsHeadV313 h2{font-size:22px;line-height:1.05;margin:0 0 3px;font-weight:900}
      .docsHeadV313 p{margin:0;font-size:13px;font-weight:800}
      .docsHeadV313 button{font-size:28px;background:transparent;border:0;color:#fff;font-weight:900;cursor:pointer;line-height:1}
      .docsBodyV313{display:grid;grid-template-columns:310px 1fr;gap:10px;padding:10px}
      .docsUploadV313,.docsListV313{background:#14243a;border:1px solid rgba(177,201,230,.22);border-radius:12px;padding:10px}
      .docsUploadV313 h3,.docsListV313 h3{font-size:18px;margin:0 0 8px;font-weight:900}
      .docsUploadV313 label{display:block;font-size:13px;font-weight:900;margin:6px 0 3px}
      .docsUploadV313 label small{font-size:10px;color:#b8c4d5}
      .docsUploadV313 select,.docsUploadV313 input{width:100%;height:32px;border-radius:8px;border:1px solid rgba(177,201,230,.28);background:#26374e;color:#fff;font-size:14px;font-weight:800;padding:0 9px;box-sizing:border-box}
      .docsDropV313{height:70px;border:1px dashed rgba(177,201,230,.35)!important;border-radius:10px;display:flex!important;flex-direction:column;align-items:center;justify-content:center;text-align:center;margin-top:8px!important;background:#132035;cursor:pointer}
      .docsDropV313 span{font-size:16px;line-height:1}
      .docsDropV313 b{font-size:15px;margin-top:3px}
      .docsDropV313 small{font-size:10px;color:#d6deeb}
      .docsSelectedV313{font-size:12px;color:#d8e2f0;min-height:22px;margin:6px 0}
      .docsProgressV313{font-size:12px;color:#9afa8b;font-weight:800;min-height:16px;margin:3px 0}
      .docsPrimaryV313{width:100%;height:38px;border:0;border-radius:10px;background:#42a932;color:#fff;font-size:15px;font-weight:900;cursor:pointer;margin-top:4px}
      .docsPrimaryV313:disabled{opacity:.7;cursor:wait}
      .docsListHeadV313{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}
      .docsListHeadV313 button{height:36px;border-radius:10px;border:1px solid rgba(177,201,230,.3);background:#293a54;color:#fff;font-size:14px;font-weight:900;padding:0 18px;cursor:pointer}
      .docsRowsV313{min-height:210px}
      .docsEmptyV313{border:1px dashed rgba(177,201,230,.35);border-radius:10px;padding:18px;text-align:center;font-size:14px;font-weight:900;color:#e8effa}
      .docsEmptyV313 small{display:block;color:#b8c4d5;margin-top:5px}
      .docsTableV313{width:100%;border-collapse:collapse;font-size:13px}
      .docsTableV313 th{background:#2b3c55;text-align:left;padding:8px;font-weight:900}
      .docsTableV313 td{padding:8px;border-bottom:1px solid rgba(177,201,230,.13);vertical-align:middle}
      .docTypeV313{display:inline-flex;align-items:center;gap:5px;background:#1c5a36;border-radius:14px;color:#9dfd7b;font-weight:900;padding:5px 9px;white-space:nowrap}
      .docActionsV313{white-space:nowrap;text-align:right}
      .docActionsV313 button{background:transparent;border:0;color:#95ff75;font-size:14px;margin:0 3px;cursor:pointer}
      .docActionsV313 button:last-child{color:#ff6b6b}
      @media(max-width:900px){.docsBodyV313{grid-template-columns:1fr}.docsModalV313{max-height:94vh;overflow:auto}}
    `;document.head.appendChild(st);
  }
  window.openDocsModalV313=openDocs; window.closeDocsModalV313=closeDocs; window.loadDocsV313=loadDocs; window.uploadDocsV313=uploadDocs; window.viewDocV313=viewDoc; window.downloadDocV313=downloadDoc; window.deleteDocV313=deleteDoc;
  window.openDocsModalV3=openDocs; window.openDocsModal246=openDocs; window.openDocsModal228=openDocs;
  window.closeDocsModalV3=closeDocs; window.closeDocsModal246=closeDocs; window.closeDocsModal228=closeDocs;
  window.loadDocsV3=loadDocs; window.loadDocs246=loadDocs; window.loadDocs228=loadDocs;
  window.uploadDocsV3=uploadDocs; window.uploadDocs246=uploadDocs; window.uploadDocs228=uploadDocs;
  window.viewDocV3=viewDoc; window.viewDoc246=viewDoc; window.viewDoc228=viewDoc;
  window.downloadDocV3=downloadDoc; window.downloadDoc246=downloadDoc; window.downloadDoc228=downloadDoc;
  window.deleteDocV3=deleteDoc; window.deleteDoc246=deleteDoc; window.deleteDoc228=deleteDoc;
...
/* ===== v3.3.21 - BITACORA OPERATIVA: MODULO UNICO SIN RENDER HEREDADO ===== */
(function(){
  'use strict';
  const VERSION='3.3.21';
  ...
  const list=name=>{try{return Array.isArray(window[name])?window[name]:[]}catch(_){return[]}};
  const setList=(name,val)=>{try{window[name]=val;if(typeof globalThis[name]!=='undefined')globalThis[name]=val}catch(_){}};
  const key=v=>S(v).replace(/^0+/,'')||S(v);
  const embNum=o=>S(o?.embarque||o?.emb||o?.numeroEmbarque||o?.nroEmbarque||o?.numero||o?.id||o?._docId);
  const fleet=o=>S(o?.flota||o?.fleet||o?.user?.fleet||o?.unidad||o?.camion||o?.flotaAsignada||o?.flotas?.[0]);
  const same=(a,b)=>key(a)===key(b);
  const when=v=>{try{if(!v)return null;if(v.toDate)return v.toDate();if(v.seconds)return new Date(v.seconds*1000);const d=new Date(v);return isNaN(d.getTime())?null:d}catch(_){return null}};
  const fmt=v=>{const d=when(v);return d?d.toLocaleString('es-AR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'-'};
  const fmtDate=v=>{const d=when(v);return d?d.toLocaleDateString('es-AR',{day:'2-digit',month:'2-digit'}):'-'};
  const open=t=>{try{if(typeof openT==='function')return !!openT(t)}catch(_){} const s=L(t?.estado||t?.status||t?.state);return s==='abierto'||s==='open'||s==='en tránsito'||s==='en transito'||t?.activo===true};
  const route=t=>{try{if(typeof ruta==='function')return ruta(t)||{}}catch(_){} return t?.route||t?.ruta||{}};
  const locationOf=t=>{try{if(typeof loc==='function')return S(loc(t))}catch(_){}return S(t?.localidad||t?.ubicacion||t?.location)};
  ...
  const bitacoraReverseGeoCache=new Map();
  async function reverseGeocodeBitacoraPosition(pos){
    if(!pos)return'';
    const key=`${Number(pos.lat).toFixed(5)},${Number(pos.lng).toFixed(5)}`;
    if(bitacoraReverseGeoCache.has(key))return bitacoraReverseGeoCache.get(key);
    let label='';
    try{
      const ctl=new AbortController();
      const timer=setTimeout(()=>ctl.abort(),4500);
      const url=`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${encodeURIComponent(pos.lat)}&lon=${encodeURIComponent(pos.lng)}&zoom=14&addressdetails=1`;
      const res=await fetch(url,{headers:{Accept:'application/json'},signal:ctl.signal});
      clearTimeout(timer);
      if(res.ok){
        const data=await res.json();
        const a=data?.address||{};
        const locality=a.neighbourhood||a.suburb||a.city_district||a.city||a.town||a.village||a.municipality||'';
        const region=a.state||a.province||'';
        label=[locality,region].filter(Boolean).filter((v,i,arr)=>arr.indexOf(v)===i).join(', ');
      }
    }catch(_){ }
    // Respaldo local para evitar mostrar '-' cuando el servicio de geocodificación no responde.
    if(!label){
      const lat=Number(pos.lat),lng=Number(pos.lng);
      if(lat>-34.66&&lat<-34.57&&lng>-58.50&&lng<-58.38)label='Caballito, CABA';
      else if(lat>-34.42&&lat<-34.28&&lng>-58.90&&lng<-58.65)label='Belén de Escobar, Buenos Aires';
      else if(lat>-53.90&&lat<-53.68&&lng>-67.85&&lng<-67.55)label='Río Grande, Tierra del Fuego';
      else label=`${lat.toFixed(5)}, ${lng.toFixed(5)}`;
    }
    bitacoraReverseGeoCache.set(key,label);
    return label;
  }
  async function loadExtra(emb){
    ...
    const currentTransit=findTransit(emb);
    const currentFleet=fleet(currentTransit)||fleet(findShipment(emb));
    const belongsToCurrentTransit=x=>{
      if(!same(embNum(x),emb))return false;
      const eventFleet=fleet(x);
      // Los registros sin flota se conservan porque pertenecen al embarque.
      // Si traen flota, deben coincidir con el tránsito seleccionado.
      return !eventFleet||!currentFleet||S(eventFleet)===S(currentFleet);
    };
    const currentChecklists=c.filter(belongsToCurrentTransit);
    const enrichedChecklists=await Promise.all(currentChecklists.map(async item=>{
      const existing=S(item.localidad||item.ubicacion||item.gpsChecklist?.ubicacionTexto||item.gpsChecklist?.localidad||'');
      if(existing&&existing!=='-')return item;
      const pos=coordOfObj(item.gpsChecklist)||coordOfObj(item);
      if(!pos)return item;
      const resolved=await reverseGeocodeBitacoraPosition(pos);
      return {...item,_resolvedLocation:resolved,_resolvedLat:pos.lat,_resolvedLng:pos.lng};
    }));
    state.extra={
      alertas:a.filter(belongsToCurrentTransit),
      aduana:d.filter(belongsToCurrentTransit),
      checklists:enrichedChecklists,
      aduanaCatalogo:cat
    };
  }
  const findTransit=emb=>list('trs').find(t=>same(embNum(t),emb)&&open(t))||null;
  const findShipment=emb=>list('embarques').find(e=>same(embNum(e),emb))||null;
  function userForFleet(f){const u=list('users').find(x=>S(x.fleet||x.flota||x.numero)===S(f));return S(u?.nombre||u?.name||u?.chofer||u?.displayName)}
  function coordOfObj(obj){
    const seen=new Set();
    function valid(lat,lng){lat=Number(lat);lng=Number(lng);return Number.isFinite(lat)&&Number.isFinite(lng)&&Math.abs(lat)<=90&&Math.abs(lng)<=180?{lat,lng}:null}
    function walk(v,depth=0){
      if(v==null||depth>5)return null;
      if(typeof v==='string'){
        const m=v.match(/(-?\d+(?:\.\d+)?)\s*[,;]\s*(-?\d+(?:\.\d+)?)/);if(m)return valid(m[1],m[2]);
        const q=v.match(/[?&](?:q|query|ll)=(-?\d+(?:\.\d+)?)[,%20]+(-?\d+(?:\.\d+)?)/i);if(q)return valid(q[1],q[2]);
        return null;
      }
      if(Array.isArray(v)&&v.length>=2){const a=valid(v[0],v[1]);if(a)return a}
      if(typeof v!=='object'||seen.has(v))return null; seen.add(v);
      const direct=valid(v.lat??v.latitude??v.latitud??v._lat, v.lng??v.lon??v.longitude??v.longitud??v._long);
      if(direct)return direct;
      const priority=['gps','position','posicion','location','ubicacionGps','coordenadasGps','coords','coord','coordenadas','geo','geopoint','punto','start','closed','ultimaPosicion','lastPosition'];
      for(const k of priority){if(k in v){const r=walk(v[k],depth+1);if(r)return r}}
      for(const [k,val] of Object.entries(v)){
        if(/lat|lon|lng|coord|gps|posic|ubicac|geo/i.test(k)){const r=walk(val,depth+1);if(r)return r}
      }
      return null;
    }
    return walk(obj);
  }
  let bitacoraEventMap=null;
  const bitacoraGeoCache=new Map();
  async function geocodeEventLocation(text){
    const q=S(text).trim();
    if(!q||q==='-')return null;
    const key=L(q);
    if(bitacoraGeoCache.has(key))return bitacoraGeoCache.get(key);
    const known=[
      [/caballito/i,{lat:-34.6182,lng:-58.4401}],
      [/bel[eé]n de escobar|escobar/i,{lat:-34.3483,lng:-58.7927}],
      [/r[ií]o grande/i,{lat:-53.7860,lng:-67.7002}],
      [/ushuaia/i,{lat:-54.8019,lng:-68.3030}],
      [/puerto z[aá]rate|z[aá]rate/i,{lat:-34.0981,lng:-59.0286}],
      [/uspallata/i,{lat:-32.5930,lng:-69.3480}],
      [/los andes/i,{lat:-32.8337,lng:-70.5983}]
    ];
    for(const [rx,pos] of known){if(rx.test(q)){bitacoraGeoCache.set(key,pos);return pos}}
    const ctrl=new AbortController(); const timer=setTimeout(()=>ctrl.abort(),4500);
    try{
      const url='https://nominatim.openstreetmap.org/search?format=json&limit=1&accept-language=es&q='+encodeURIComponent(q+', Argentina');
      ...
      if(pos&&Number.isFinite(pos.lat)&&Number.isFinite(pos.lng)){bitacoraGeoCache.set(key,pos);return pos;}
    }catch(_){ } finally {clearTimeout(timer)}
    bitacoraGeoCache.set(key,null);
    return null;
  }
  function eventMapMarkerSpec(type){
    const s=L(type);
    if(s.includes('aduana'))return{emoji:'🏛️',bg:'#2563eb',border:'#93c5fd'};
    if(s.includes('control'))return{emoji:'⚠️',bg:'#f59e0b',border:'#fde68a'};
    if(s.includes('niebla'))return{emoji:'🌧️',bg:'#7c3aed',border:'#c4b5fd'};
    if(s.includes('rotura'))return{emoji:'🔧',bg:'#64748b',border:'#cbd5e1'};
    if(s.includes('accidente'))return{emoji:'🚨',bg:'#dc2626',border:'#fecaca'};
    if(s.includes('check')||s.includes('oea'))return{emoji:'✅',bg:'#16a34a',border:'#bbf7d0'};
    if(s.includes('fin')||s.includes('cierre'))return{emoji:'🏁',bg:'#111827',border:'#f8fafc'};
    if(s.includes('inicio'))return{emoji:'▶',bg:'#16a34a',border:'#bbf7d0'};
    return{emoji:'📍',bg:'#2563eb',border:'#bfdbfe'};
  }
  function eventMapLeafletIcon(ev){
    const spec=eventMapMarkerSpec(ev?.label||ev?.type||'');
    const html=`<div style="width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:${spec.bg};border:3px solid ${spec.border};box-shadow:0 4px 12px rgba(0,0,0,.55);font-size:21px;line-height:1">${spec.emoji}</div>`;
    return window.L.divIcon({className:'b344EventMarker',html,iconSize:[44,44],iconAnchor:[22,40],popupAnchor:[0,-38]});
  }
  async function renderSelectedEventMap(ev){
    const box=byId('b344EventMap'),status=byId('b344MapStatus');
    if(!box)return;
    if(status)status.textContent='Cargando ubicación...';
    let pos=null;
    if(ev?.useDestination)pos=await destinationPosition(ev?.loc);
    if(!pos)pos=coordOfObj(ev?.raw)||coordOfObj(ev);
    ...
    const icon=eventMapLeafletIcon(ev);
  ...
  function optionsData(){
    // La Bitácora solo debe listar embarques con un tránsito actualmente abierto.
    // Se excluyen embarques finalizados y embarques que todavía no tienen tránsito.
    const map=new Map();
    list('trs').filter(open).forEach(t=>{
      const n=embNum(t);if(!n)return;
      const shipment=list('embarques').find(e=>same(embNum(e),n))||{};
      const k=key(n);
      map.set(k,{...shipment,...t,_emb:n,_fleet:fleet(t)||fleet(shipment)||'',_priority:2});
    });
    return [...map.values()].sort((a,b)=>S(a._emb).localeCompare(S(b._emb),'es',{numeric:true}));
  }
  function eventIcon(type){const s=L(type);if(s.includes('aduana'))return'🏛️';if(s.includes('check'))return'✅';if(s.includes('niebla'))return'🌧️';if(s.includes('rotura'))return'🔧';if(s.includes('accidente'))return'🚨';if(s.includes('alert')||s.includes('control'))return'⚠️';if(s.includes('fin')||s.includes('cierre'))return'■';return'▶'}
  function eventSource(obj,fallback='APK'){
    const raw=S(obj?.origenRegistro||obj?.source||obj?.canal||obj?.plataforma||obj?.dispositivo||obj?.origenCarga||'');
    const n=L(raw);
    if(/web|manual|admin|escritorio|desktop/.test(n))return'WEB';
    if(/apk|android|app|movil|móvil|chofer/.test(n))return'APK';
    return fallback;
  }
  function eventUser(obj,t,fl){
    const u=obj?.usuario||obj?.user||obj?.creadoPor||obj?.createdBy||obj?.operador||obj?.chofer;
    const name=typeof u==='object'?S(u?.name||u?.nombre||u?.displayName||u?.email):S(u);
    return name||S(obj?.flota||obj?.fleet||obj?.unidad||fl||fleet(t)||'-');
  }
  function destinationRecord(name){
    const q=L(name);
    return list('destinos').find(x=>{
      const n=S(x?.nombre||x?.destino||x?.name||x?.id||x?._docId);
      const k=L(n); return k===q||q.includes(k)||k.includes(q);
    })||null;
  }
  async function destinationPosition(name){
    const d=destinationRecord(name);
    return coordOfObj(d)||await geocodeEventLocation(S(d?.localidad||d?.ubicacion||d?.nombre||name));
  }
  function buildEvents(t){
    const out=[];const fl=fleet(t)||'-';if(t){const r=route(t),start=t.start?.time||t.start||t.inicio||t.fechaInicio||t.createdAt;out.push({key:'start',type:'Inicio',label:'Inicio de tránsito',time:start,loc:S(r.origen||t.origen||'-'),source:'APK',user:eventUser(t,t,fl)||userForFleet(fl)||fl,obs:'Inicio del tránsito',raw:t});(t.alerts||[]).forEach((a,i)=>out.push({key:'ta'+i,type:S(a.tipo||a.type||'Alerta'),label:S(a.tipo||a.type||'Alerta'),time:a.time||a.fecha||a.createdAt,loc:S(a.localidad||a.ubicacion||locationOf(t)||'-'),source:eventSource(a,'APK'),user:eventUser(a,t,fl),obs:S(a.observacion||a.descripcion||''),raw:a}));if(!open(t))out.push({key:'end',type:'Fin',label:'Fin de tránsito',time:t.closed?.time||t.closed||t.fechaCierre,loc:S(r.destino||t.destino||'-'),source:'APK',user:eventUser(t.closed||t,t,fl),obs:'Cierre del tránsito',raw:t.closed||t,useDestination:!coordOfObj(t.closed)});else out.push({key:'pending',type:'Pendiente',label:'Fin de tránsito',time:null,loc:S(r.destino||t.destino||'-'),source:'-',user:fl,obs:'Pendiente',raw:t,useDestination:true})}
    state.extra.alertas.forEach((a,i)=>out.push({key:'a'+i,type:S(a.tipo||a.nombre||'Alerta'),label:S(a.tipo||a.nombre||'Alerta'),time:a.fecha||a.time||a.createdAt,loc:S(a.localidad||a.ubicacion||'-'),source:eventSource(a,'WEB'),user:eventUser(a,t,fl),obs:S(a.observacion||a.descripcion||''),raw:a}));
    state.extra.aduana.forEach((a,i)=>out.push({key:'d'+i,type:'Aduana',label:S(a.aduana||a.nombre||'Aduana'),time:a.fecha||a.time||a.createdAt,loc:S(a.localidad||a.aduana||'-'),source:eventSource(a,'APK'),user:eventUser(a,t,fl),obs:S(a.observacion||''),raw:a}));
    state.extra.checklists.forEach((a,i)=>out.push({key:'c'+i,type:'Checklist',label:S(a.tipo||'Checklist OEA'),time:a.fechaHoraGuardado||a.creadoEn||a.fecha||a.time||a.createdAt,loc:S(a.localidad||a.ubicacion||a.gpsChecklist?.ubicacionTexto||a.gpsChecklist?.localidad||a._resolvedLocation||((a._resolvedLat!=null&&a._resolvedLng!=null)?`${Number(a._resolvedLat).toFixed(5)}, ${Number(a._resolvedLng).toFixed(5)}`:'-')),source:eventSource(a,'APK'),user:eventUser(a,t,fl),obs:S(a.observacion||''),raw:a}));
    return out.sort((a,b)=>(when(a.time)?.getTime()||9e15)-(when(b.time)?.getTime()||9e15));
  }
  function applyVersion(){window.ELTA_APP_VERSION=VERSION;window.APP_VERSION=VERSION;document.title='ELTA ITS - Versión '+VERSION;document.querySelectorAll('[data-app-version],.appVersion,.loginVersion,.footerVersion').forEach(el=>el.textContent='Versión '+VERSION);document.querySelectorAll('span,small,p,div,footer').forEach(el=>{if(el.childElementCount===0&&/Versi[oó]n\s+\d+\.\d+\.\d+/.test(el.textContent||''))el.textContent=el.textContent.replace(/Versi[oó]n\s+\d+\.\d+\.\d+/g,'Versión '+VERSION)})}
  ...
  function renderEmpty(){const r=byId('bitacoraRoot');if(!r)return;const data=optionsData();r.innerHTML=renderSearch(data)+'<div class="b344Card b344Empty"><h3>Seleccione un embarque</h3><p>La bitácora cargará el resumen, el recorrido y los eventos registrados.</p></div>';bindFilters()}

  function normCountry(v){
    const x=L(v).normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    if(/chile|stli|los andes|libertadores/.test(x))return'chile';
    if(/paraguay|asuncion|falcon|clorinda/.test(x))return'paraguay';
    if(/uruguay|montevideo|fray bentos|sevel/.test(x))return'uruguay';
    if(/brasil|brazil|curitiba|sao paulo/.test(x))return'brasil';
    if(/argentina|zarate|cordoba|clz|clc|ushuaia|tierra del fuego|rio grande|tolhuin|san sebastian/.test(x))return'argentina';
    return x;
  }
  function destinationCountry(destino){
    const d=list('destinos').find(x=>{
      const n=S(x?.nombre||x?.destino||x?.name||x?.id||x?._docId);
      return L(n)===L(destino)||L(destino).includes(L(n))||L(n).includes(L(destino));
    });
    return normCountry(d?.pais||d?.country||destino);
  }
  function customsCatalog(destino){
    const dest=L(destino).normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
    const docs=(state.extra.aduanaCatalogo||[]).filter(x=>x?.activo!==false);
    // La Bitácora no debe traer todas las aduanas de un país. Solo se usan
    // configuraciones vinculadas de forma explícita al destino/corredor del
    // embarque. Los registros reales del tránsito se incorporan aparte.
    return docs
      .filter(x=>{
        const target=L(x?.destino||x?.corredor||x?.ruta||x?.embarqueDestino||'')
          .normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
        if(!target||!dest)return false;
        return target===dest || target.includes(dest) || dest.includes(target);
      })
      .map(x=>({
        id:S(x.id||x._docId),
        label:S(x.nombre||x.aduana||x.id||x._docId||'Aduana'),
        loc:S(x.localidad||x.ubicacion||x.nombre||x.aduana||'-'),
        order:Number(x.orden??x.order??999)
      }))
      .sort((a,b)=>a.order-b.order||a.label.localeCompare(b.label,'es'));
  }
  function sameCustoms(a,b){
    const aa=L(a).replace(/[^a-z0-9]/g,''),bb=L(b).replace(/[^a-z0-9]/g,'');
    return aa&&bb&&(aa===bb||aa.includes(bb)||bb.includes(aa));
  }
  function controlEvents(events){
    return (events||[]).filter(x=>/control\s*de\s*carga|control.*carga/i.test(`${x.type||''} ${x.label||''}`));
  }
  function buildRouteStages(events,origen,destino,estado){
    const real=(events||[]).filter(x=>x&&x.type!=='Pendiente');
    const start=real.find(x=>/inicio/i.test(`${x.type||''} ${x.label||''}`))||real[0]||null;
    const end=real.find(x=>/fin|cierre|final/i.test(`${x.type||''} ${x.label||''}`))||null;
    const regs=state.extra.aduana||[];
    const catalog=customsCatalog(destino);
    const customs=catalog.map(c=>{
      const reg=regs.find(r=>sameCustoms(c.label,S(r.aduana||r.nombre||r.localidad||'')));
      return {kind:'aduana',label:c.label,loc:c.loc,time:reg&&(reg.fecha||reg.time||reg.createdAt),done:!!reg,order:c.order};
    });
    // Si existe un registro de aduana que no pudo vincularse al catálogo,
    // igualmente se incorpora como hito real. Evita perder aduanas por
    // diferencias de nombre, país o datos históricos.
    regs.forEach((r,i)=>{
      const label=S(r.aduana||r.nombre||r.localidad||'Aduana');
      if(!label||customs.some(c=>sameCustoms(c.label,label)))return;
      customs.push({
        kind:'aduana',
        label,
        loc:S(r.localidad||r.ubicacion||label),
        time:r.fecha||r.time||r.createdAt,
        done:true,
        order:Number(r.orden??r.order??(700+i))
      });
    });
    const controls=controlEvents(real).map(x=>({kind:'control',label:'Control de carga',loc:x.loc||'',time:x.time,done:true,order:500}));
    const completedCustoms=customs.filter(x=>x.done).sort((a,b)=>(when(a.time)?.getTime()||0)-(when(b.time)?.getTime()||0));
    const pendingCustoms=customs.filter(x=>!x.done);
    const chronological=[...completedCustoms,...controls].sort((a,b)=>(when(a.time)?.getTime()||0)-(when(b.time)?.getTime()||0));
    return [
      {kind:'start',label:'Inicio',loc:origen,time:start?.time,done:!!start},
      ...chronological,
      ...pendingCustoms,
      {kind:'end',label:'Cierre',loc:destino,time:end?.time,done:!!end||/final/i.test(estado||'')}
    ];
  }
  function renderRouteTimeline(events,origen,destino,estado){
    const stages=buildRouteStages(events,origen,destino,estado);
    const lastDone=Math.max(0,stages.reduce((a,s,i)=>s.done?i:a,0));
    const max=Math.max(1,stages.length-1);
    const pct=Math.max(0,Math.min(100,(lastDone/max)*100));
    return `<div class="b344RouteTimeline">
      <div class="b344RouteLabels"><strong>${E(origen)}</strong><strong>${E(destino)}</strong></div>
      <div class="b344TimelineTrack"><i style="width:${pct}%"></i></div>
      ${stages.map((s,i)=>`<div class="b344TimelineStep ${s.done?'done':''} ${i===lastDone?'current':''} ${i===0?'start':''} ${i===stages.length-1?'end':''} ${s.kind||''}" style="left:${(i/max)*100}%"><span></span><b>${E(s.label)}</b><em>${E(s.time?fmtDate(s.time):(s.done?'Completado':'Pendiente'))}</em></div>`).join('')}
      <div class="b344TimelineTruck" style="left:${pct}%"><img src="assets/camion-automobilero.png" alt="Camión avanzando hacia el cierre"></div>
      <div class="b344TimelineDash"></div>
    </div>`;
  }
  ...
  function renderData(){
    ...
    const cliente=S(r.cliente||e?.cliente||t?.cliente||'-'),origen=S(r.origen||e?.origen||t?.origen||'-'),destino=S(r.destino||e?.destino||t?.destino||'-'),fl=fleet(t)||fleet(e)||'-',chofer=S(t?.user?.name||t?.chofer||userForFleet(fl)||'-'),estado=t?(open(t)?'En tránsito':'Finalizado'):'Sin tránsito';
    const last=(()=>{try{return typeof lastU==='function'?lastU(t):null}catch(_){return null}})(),events=buildEvents(t);
    ...
    root.innerHTML=renderSearch(optionsData())+
      ...
      `</div>`+
      `<div class="b344Card b344Actions"><button class="green"><span aria-hidden="true">▶️</span> Iniciar tránsito</button><button class="blue"><span aria-hidden="true">🏛️</span> Nueva aduana</button><button class="yellow"><span aria-hidden="true">⚠️</span> Nueva alerta</button><button><span aria-hidden="true">✅</span> Checklist OEA</button><button class="red"><span aria-hidden="true">🏁</span> Cerrar tránsito</button></div>`;
    bindFilters();
  ...
  window.buscarBitacoraEmbarque=async()=>{
    const v=S(byId('b344Emb')?.value||byId('b344EmbSelect')?.value);
    if(!v){alert('Ingrese o seleccione un embarque.');return}
    await loadBase(true);
    if(!findTransit(v)){
      alert('El embarque seleccionado no tiene un tránsito abierto.');
      return;
    }
    state.embarque=v;state.eventKey='';await render(true)
  };
  ...
  function init(){ensureCss();ensureMenu();ensurePanel();applyVersion();state.ready=true}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();

/* ===== ELTA v3.3.21 - PANEL OPERATIVO INICIAR TRANSITO ===== */
(function(){
  'use strict';
  const VERSION='3.3.21';
  const $=id=>document.getElementById(id);
  ...
  const key=v=>S(v).replace(/^0+/,'')||S(v);
  const same=(a,b)=>key(a)===key(b);
  const dbx=()=>{try{return window.db||(typeof db!=='undefined'?db:null)||(window.firebase?.firestore?window.firebase.firestore():null)}catch(_){return null}};
  const esc=v=>S(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const embNum=o=>S(o?.embarque||o?.emb||o?.numeroEmbarque||o?.nroEmbarque||o?.numero||o?.id||o?._docId);
  const fleet=o=>S(o?.flota||o?.fleet||o?.unidad||o?.camion||o?.flotaAsignada||o?.user?.fleet||o?.flotas?.[0]);
  ...
  function showPanel(){
    closePanel();
    const overlay=document.createElement('div');overlay.id='startTransitOverlay';overlay.className='st320Overlay';
    overlay.innerHTML=`<aside class="st320Panel" role="dialog" aria-modal="true" aria-label="Iniciar tránsito"><header class="st320Head"><div><h2><span>▶</span> Iniciar tránsito</h2><p>Registrar el inicio de un nuevo tránsito</p></div><button type="button" id="startTransitClose">×</button></header><div class="st320Body"><section class="st320Step"><h3><i>1</i> Paso 1 – Embarque</h3><div class="st320Inline"><label><span>N° Embarque</span><input id="startEmb" inputmode="numeric" placeholder="Ingrese número de embarque"></label><button id="startValidateEmb" class="primary">🔎 Validar</button></div><div id="startEmbCard" class="st320Result muted">Ingrese un embarque para validar sus datos.</div></section><section class="st320Step"><h3><i>2</i> Paso 2 – Flota</h3><div class="st320FleetRow"><label><span>Flota declarada</span><select id="startFleet" disabled><option value="">Seleccione una flota</option></select></label><div id="startFleetState" class="st320FleetState muted">Pendiente de validación</div></div></section><section class="st320Step"><h3><i>3</i> Paso 3 – Datos del tránsito</h3><div id="startTransitData" class="st320Data"></div></section><section class="st320Step"><h3><i>4</i> Paso 4 – Ubicación de inicio</h3><div class="st320Location"><div class="st320Radios"><label><input type="radio" name="startLocation" value="origin" checked> Usar coordenadas del origen</label><label><input type="radio" name="startLocation" value="last"> Usar último GPS de la flota</label><label><input type="radio" name="startLocation" value="manual"> Seleccionar punto en el mapa</label></div><div id="startMap" class="st320Map"><span>Validá el embarque para cargar el mapa.</span></div></div><div id="startCoords" class="st320Coords">Latitud: - &nbsp; Longitud: -</div></section><div id="startTransitStatus" class="st320Status"></div></div><footer class="st320Foot"><button type="button" id="startCancel">Cancelar</button><button type="button" id="startConfirm" class="primary" disabled>▶ Iniciar tránsito</button></footer></aside>`;
    document.body.appendChild(overlay);const pre=S(document.getElementById('b344Emb')?.value||'');if(pre)$('startEmb').value=pre;renderTransitData();bindPanel()
  }
  function closePanel(){try{if(data.map)data.map.remove()}catch(_){}data={shipment:null,fleets:[],fleet:'',driver:'',originPos:null,lastPos:null,selectedPos:null,map:null,marker:null,collections:{}};$('startTransitOverlay')?.remove()}
  ...
  function updateLocationMode(){const mode=document.querySelector('input[name="startLocation"]:checked')?.value||'origin';let pos=null;if(mode==='origin')pos=data.originPos;else if(mode==='last')pos=data.lastPos;else pos=data.selectedPos;if(!pos&&mode!=='manual')setStatus(mode==='origin'?'El origen no tiene coordenadas configuradas. Seleccione otro método.':'La flota no tiene último GPS disponible. Seleccione otro método.','error');setSelectedPos(pos)}
  async function saveTransit(){if(!data.shipment||!data.fleet){setStatus('Valide el embarque y la flota.','error');return}const db=dbx();if(!db?.collection){setStatus('Firebase no está disponible.','error');return}const pos=data.selectedPos;if(!pos){setStatus('Seleccione una ubicación válida para iniciar el tránsito.','error');return}const emb=S($('startEmb').value),cliente=getClient(data.shipment),origen=getOrigin(data.shipment),destino=getDestination(data.shipment),usuario=currentUser();if(!window.confirm(`¿Confirmar inicio de tránsito?\n\nEmbarque: ${emb}\nCliente: ${cliente}\nFlota: ${data.fleet}\nChofer: ${data.driver}\nOrigen: ${origen}\nDestino: ${destino}`))return;const btn=$('startConfirm');btn.disabled=true;btn.textContent='Iniciando tránsito...';setStatus('Validando y guardando en Firebase...','info');try{const fresh=await readCol('transitos');const conflict=fresh.find(t=>S(fleet(t))===data.fleet&&isOpen(t));if(conflict)throw new Error(`La flota ${data.fleet} ya tiene un tránsito abierto.`);const now=new Date().toISOString();const payload={embarque:emb,numeroEmbarque:emb,cliente,flota:data.fleet,chofer:data.driver,origen,destino,estado:'abierto',status:'abierto',activo:true,closed:null,inicio:now,fechaInicio:now,inicioFechaHora:now,createdAt:now,updatedAt:now,creadoDesde:'WEB',origenRegistro:'WEB',creadoPor:usuario,usuario,gps:{lat:pos.lat,lng:pos.lng},inicioGps:{lat:pos.lat,lng:pos.lng},position:{lat:pos.lat,lng:pos.lng},ruta:{cliente,origen,destino},route:{cliente,origen,destino},user:{name:data.driver,fleet:data.fleet}};await db.collection('transitos').add(payload);setStatus('Tránsito iniciado correctamente.','ok');btn.textContent='✓ Tránsito iniciado';setTimeout(()=>{closePanel();try{window.renderBitacoraOperativa?.(true)}catch(_){}},800)}catch(e){console.error(e);setStatus(e.message||'No se pudo iniciar el tránsito.','error');btn.disabled=false;btn.textContent='▶ Iniciar tránsito'}}
  function injectCss(){if($('st320Css'))return;const st=document.createElement('style');st.id='st320Css';st.textContent=`.st320Overlay{position:fixed;inset:0;z-index:999999;background:rgba(2,10,18,.68);display:flex;justify-content:flex-end;backdrop-filter:blur(2px)}.st320Panel{width:min(680px,46vw);height:100vh;background:#f8fafc;color:#0f172a;box-shadow:-16px 0 40px rgba(0,0,0,.38);display:flex;flex-direction:column;animation:st320In .22s ease-out;font-size:14px}@keyframes st320In{from{transform:translateX(100%)}to{transform:none}}.st320Head{padding:14px 18px 12px;border-bottom:1px solid #d7dee8;display:flex;justify-content:space-between;align-items:flex-start}.st320Head h2{margin:0;font-size:21px;line-height:1.15}.st320Head h2 span{color:#2d9e3f}.st320Head p{margin:3px 0 0;color:#475569;font-size:13px}.st320Head button{border:0;background:transparent;font-size:27px;line-height:1;cursor:pointer;padding:0 2px}.st320Body{padding:11px 16px 14px;overflow:auto;flex:1}.st320Step{padding:0 0 10px;margin-bottom:10px;border-bottom:1px solid #d8e0e8}.st320Step h3{font-size:14px;margin:0 0 8px;line-height:1.2}.st320Step h3 i{display:inline-grid;place-items:center;width:22px;height:22px;border-radius:50%;background:#2d9e3f;color:#fff;font-style:normal;margin-right:6px;font-size:12px}.st320Inline,.st320FleetRow{display:grid;grid-template-columns:1fr 132px;gap:8px;align-items:end}.st320Step label span{display:block;font-size:11px;font-weight:800;margin-bottom:4px}.st320Step input,.st320Step select{width:100%;height:36px;border:1px solid #cbd5e1;border-radius:7px;padding:0 10px;background:#fff;color:#0f172a;font-weight:700;font-size:13px}.st320Step button,.st320Foot button{height:36px;border-radius:7px;border:1px solid #b8c3d1;background:#fff;font-weight:900;cursor:pointer;font-size:13px;padding:0 12px}.st320Step .primary,.st320Foot .primary{background:#2d9e3f;color:#fff;border-color:#2d9e3f}.st320Result{margin-top:7px;border-radius:8px;padding:9px 10px;border:1px solid #d9e1ea;font-size:13px}.st320Result.ok{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;background:#e9f7e9;border-color:#b7dfba}.st320Result.ok p{grid-column:1/-1;margin:1px 0 0}.st320Result small,.st320Data small{display:block;color:#64748b;font-size:9px;text-transform:uppercase;font-weight:900}.st320Result b,.st320Data b{display:block;margin-top:2px;font-size:12px;line-height:1.25}.st320Result.error,.st320FleetState.error{background:#fff0f0;border-color:#f3b3b3;color:#991b1b}.muted{color:#64748b;background:#f1f5f9}.st320FleetState{min-height:36px;border:1px solid #d9e1ea;border-radius:7px;padding:6px 8px;font-weight:800;font-size:12px;display:flex;align-items:center}.st320FleetState.ok{background:#e9f7e9;color:#166534;border-color:#b7dfba}.st320FleetState small{font-weight:600}.st320Data{display:grid;grid-template-columns:repeat(3,1fr);border:1px solid #d9e1ea;border-radius:8px;overflow:hidden}.st320Data>div{padding:7px 8px;border-right:1px solid #d9e1ea;border-bottom:1px solid #d9e1ea;min-height:48px}.st320Location{display:grid;grid-template-columns:205px 1fr;gap:9px}.st320Radios label{display:block;margin:0 0 8px;font-weight:700;font-size:12px}.st320Radios input{width:auto;height:auto;margin-right:6px}.st320Map{height:165px;border:1px solid #cbd5e1;border-radius:8px;background:#e2e8f0;overflow:hidden;display:grid;place-items:center;font-size:12px}.st320Coords{margin-top:6px;padding:6px 8px;border-radius:6px;background:#e9f7e9;color:#166534;font-weight:800;font-size:11px}.st320Status{min-height:18px;padding:5px 8px;border-radius:6px;font-weight:800;font-size:11px}.st320Status.error{background:#fee2e2;color:#991b1b}.st320Status.ok{background:#dcfce7;color:#166534}.st320Status.info{background:#e0f2fe;color:#075985}.st320Foot{padding:10px 16px;border-top:1px solid #d7dee8;display:grid;grid-template-columns:1fr 1.25fr;gap:10px}.st320Foot button:disabled{opacity:.55;cursor:not-allowed}@media(max-width:1000px){.st320Panel{width:92vw}.st320Result.ok,.st320Data{grid-template-columns:1fr 1fr}.st320Location{grid-template-columns:1fr}.st320Map{height:150px}}`;document.head.appendChild(st)}
  document.addEventListener('click',e=>{const b=e.target.closest('.b344Actions button');if(!b)return;if(/Iniciar\s+tránsito/i.test(b.textContent||'')){e.preventDefault();showPanel()}},true);injectCss();window.openStartTransitPanel=showPanel;window.closeStartTransitPanel=closePanel;
})();
